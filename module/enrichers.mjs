...
export function getRulesVersion(config={}, options={}) {
  ...
  return options.relativeTo?.parent?.system?.source?.rules
    ...
    || (game.settings.get("naheulbeuk", "rulesVersion") === "modern" ? "2024" : "2014");
}
...
async function enrichAttack(config, label, options) {
  ...
  for ( const value of config.values ) {
    if ( value in CONFIG.NAHEULBEUL.attackModes ) config.attackMode = value;
  ...
  span.innerHTML = game.i18n.format(`EDITOR.NAHEULBEUL.Inline.Attack${config._rules === "2014" ? "Long" : "Short"}`, {
    formula: createRollLink(displayFormula).outerHTML
  });

  if ( config.format === "extended" ) {
    const type = game.i18n.format(`NAHEULBEUL.ATTACK.Formatted.${config._rules}`, {
      type: game.i18n.getListFormatter({ type: "disjunction" }).format(
        Array.from(activity?.validAttackTypes ?? []).map(t => CONFIG.NAHEULBEUL.attackTypes[t]?.label)
      ),
      classification: CONFIG.NAHEULBEUL.attackClassifications[activity?.attack.type.classification]?.label ?? ""
    }).trim();
    ...
    full.innerHTML = game.i18n.format("EDITOR.NAHEULBEUL.Inline.AttackExtended", {
...
async function enrichAward(config, label, options) {
  ...
  block.classList.add("award-block", "naheulbeuk");
  block.dataset.awardCommand = command;

  const entries = [];
  for ( let [key, amount] of Object.entries(parsed.currency) ) {
    const label = CONFIG.NAHEULBEUL.currencies[key].label;
  ...
  if ( parsed.xp ) entries.push(`
    <span class="award-entry">
      ${formatNumber(parsed.xp)} ${game.i18n.localize("NAHEULBEUL.ExperiencePoints.Abbreviation")}
    </span>
  `);

  let award = game.i18n.getListFormatter({ type: "unit" }).format(entries);
  if ( parsed.each ) award = game.i18n.format("EDITOR.NAHEULBEUL.Inline.AwardEach", { award });

  block.innerHTML += `
    ${award}
    <a class="award-link" data-action="awardRequest">
      <i class="fa-solid fa-trophy"></i> ${label ?? game.i18n.localize("NAHEULBEUL.Award.Action")}
    </a>
  `;
...
async function enrichCheck(config, label, options) {
  ...
  for ( let value of config.values ) {
    ...
    if ( slug in CONFIG.NAHEULBEUL.enrichmentLookup.abilities ) config.ability = slug;
    else if ( slug in CONFIG.NAHEULBEUL.enrichmentLookup.skills ) config.skill.push(slug);
    else if ( slug in CONFIG.NAHEULBEUL.enrichmentLookup.tools ) config.tool.push(slug);
  ...
  if ( activity ) {
    ...
    for ( const associated of activity.check.associated ) {
      if ( associated in CONFIG.NAHEULBEUL.skills ) config.skill.push(associated);
      else if ( associated in CONFIG.NAHEULBEUL.tools ) config.tool.push(associated);
    }
    delete config.activity;
  }

  // TODO: Support "spellcasting" ability
  let abilityConfig = CONFIG.NAHEULBEUL.enrichmentLookup.abilities[slugify(config.ability)];
  ...
  for ( let [index, skill] of config.skill.entries() ) {
    const skillConfig = CONFIG.NAHEULBEUL.enrichmentLookup.skills[slugify(skill)];
    if ( skillConfig ) {
  ...
  for ( const tool of config.tool ) {
    const toolConfig = CONFIG.NAHEULBEUL.tools[slugify(tool)];
    const toolUUID = CONFIG.NAHEULBEUL.enrichmentLookup.tools[slugify(tool)];
  ...
  if ( complex ) {
    ...
    for ( const [ability, associated] of groups.entries() ) {
      ...
      if ( associated.length > 1 ) parts.push(
        game.i18n.format("EDITOR.NAHEULBEUL.Inline.SpecificCheck", {
          ability: CONFIG.NAHEULBEUL.enrichmentLookup.abilities[ability].label,
    ...
    if ( config.dc && !config.hideDC ) {
      label = game.i18n.format("EDITOR.NAHEULBEUL.Inline.DC", { dc: config.dc, check: label });
    }
    label = game.i18n.format(`EDITOR.NAHEULBEUL.Inline.Check${config.format === "long" ? "Long" : "Short"}`, { check: label });
    if ( usingTool ) label = game.i18n.format("EDITOR.NAHEULBEUL.Inline.CheckUsing", {
...
function createCheckRequestButtons(dataset) {
  ...
  return [
    ...skills.map(skill => createRequestButton({
      ability: CONFIG.NAHEULBEUL.skills[skill].ability, ...baseDataset, format: "short", skill, type: "skill"
    })),
    ...dataset.usingTool ? [] : tools.map(tool => createRequestButton({
      ability: CONFIG.NAHEULBEUL.tools[tool]?.ability, ...baseDataset, format: "short", tool, type: "tool"
    }))
  ];
}
...
async function enrichSave(config, label, options) {
  ...
  for ( let value of config.values ) {
    const slug = foundry.utils.getType(value) === "string" ? slugify(value) : value;
    if ( slug in CONFIG.NAHEULBEUL.enrichmentLookup.abilities ) config.ability.push(slug);
    else if ( Number.isNumeric(value) ) config.dc = Number(value);
    else config[value] = true;
  }
  config.ability = config.ability
    .filter(a => a in CONFIG.NAHEULBEUL.enrichmentLookup.abilities)
    .map(a => CONFIG.NAHEULBEUL.enrichmentLookup.abilities[a].key ?? a);
  ...
  if ( label ) label = createRollLink(label);
    ...
    label = game.i18n.format(`EDITOR.NAHEULBEUL.Inline.Save${config.format === "long" ? "Long" : "Short"}`, { save: label });
    const template = document.createElement("template");
    template.innerHTML = label;
    label = template;
  }
...
async function enrichDamage(configs, label, options) {
  ...
  for ( const c of configs ) {
    ...
    for ( const value of c.values ) {
      if ( value in CONFIG.NAHEULBEUL.damageTypes ) c.type.push(value);
      else if ( value in CONFIG.NAHEULBEUL.healingTypes ) c.type.push(value);
      else if ( value in CONFIG.NAHEULBEUL.attackModes ) config.attackMode = value;
  ...
  for ( const [idx, formula] of config.formulas.entries() ) {
    ...
    const types = type?.split("|")
      .map(t => CONFIG.NAHEULBEUL.damageTypes[t]?.label ?? CONFIG.NAHEULBEUL.healingTypes[t]?.label)
      .filter(_ => _);
    ...
    parts.push(game.i18n.format(`EDITOR.NAHEULBEUL.Inline.Damage${localizationType}`, localizationData));
  }
  ...
  if ( config.average && (parts.length === 2) ) {
    link.innerHTML = game.i18n.format("EDITOR.NAHEULBEUL.Inline.DamageDouble", { first: parts[0], second: parts[1] });
  } else {
  ...
  if ( config.format === "extended" ) {
    ...
    span.innerHTML = game.i18n.format("EDITOR.NAHEULBEUL.Inline.DamageExtended", { damage: link.outerHTML });
    return span;
  }
...
function enrichLanguage(config, label, options) {
  for ( const value of config.values ) {
    const slug = foundry.utils.getType(value) === "string" ? slugify(value) : value;
    if ( slug in CONFIG.NAHEULBEUL.enrichmentLookup.languages ) config.language = slug;
  }
  delete config.values;

  if ( !(config.language in CONFIG.NAHEULBEUL.enrichmentLookup.languages) ) {
    console.warn(`No language found while enriching ${config._input}.`);
    return null;
  }

  config.type = "language";
  return createPassiveTag(label ?? CONFIG.NAHEULBEUL.enrichmentLookup.languages[config.language], config);
}
 ...
 * ```[[lookup @target.template.size activity=naheulbeukactivity000]]```
 * becomes
 * ```html
 ...
 *      data-uuid="Compendium.naheulbeuk.rules.JournalEntry.w7eitkpD7QQTB6j0.JournalEntryPage.UWw13ISmMxDzmwbd"
...
async function enrichReference(config, label, options) {
  let key;
  let source;
  let type = Object.keys(config).find(k => k in CONFIG.NAHEULBEUL.ruleTypes);
  if ( type ) {
    key = slugify(config[type]);
    const { references } = CONFIG.NAHEULBEUL.ruleTypes[type] ?? {};
    source = foundry.utils.getProperty(CONFIG.NAHEULBEUL, references)?.[key];
  } else if ( config.values.length ) {
    key = slugify(config.values.join(""));
    for ( const [t, { references }] of Object.entries(CONFIG.NAHEULBEUL.ruleTypes) ) {
      source = foundry.utils.getProperty(CONFIG.NAHEULBEUL, references)?.[key];
      if ( source ) {
        type = t;
        break;
  ...
  if ( (type === "condition") && (config.apply !== false) ) {
    ...
    apply.setAttribute("aria-label", game.i18n.localize("EDITOR.NAHEULBEUL.Inline.ApplyStatus"));
...
async function enrichItem(config, label, options) {
  ...
  if ( ["Activity", "Item"].includes(parsed?.type) ) {
    ...
    if ( !label ) {
      ...
      else label = game.i18n.format("EDITOR.NAHEULBEUL.Inline.ItemActivity", { item: doc.item.name, activity: doc.name });
    }
  ...
  if ( foundItem ) {
    if ( config.activity ) {
      ...
      if ( !label ) label = game.i18n.format("EDITOR.NAHEULBEUL.Inline.ItemActivity", {
  ...
  if ( !label ) label = config.activity ? game.i18n.format("EDITOR.NAHEULBEUL.Inline.ItemActivity", {
...
export function createRollLabel(config) {
  const { label: ability, abbreviation } = CONFIG.NAHEULBEUL.abilities[config.ability] ?? {};
  const skill = CONFIG.NAHEULBEUL.skills[config.skill]?.label;
  const toolUUID = CONFIG.NAHEULBEUL.enrichmentLookup.tools[config.tool];
  ...
  switch ( config.type ) {
    ...
    case "tool":
      if ( ability && (skill || tool) ) {
        label = game.i18n.format("EDITOR.NAHEULBEUL.Inline.SpecificCheck", { ability, type: skill ?? tool });
      } else {
        label = ability;
      }
      if ( config.passive ) {
        label = game.i18n.format(
          `EDITOR.NAHEULBEUL.Inline.${showDC ? "DC" : ""}Passive${longSuffix}`, { dc: config.dc, check: label }
        );
      } else {
        if ( showDC ) label = game.i18n.format("EDITOR.NAHEULBEUL.Inline.DC", { dc: config.dc, check: label });
        label = game.i18n.format(`EDITOR.NAHEULBEUL.Inline.Check${longSuffix}`, { check: label });
      }
      break;
    case "concentration":
    case "save":
      if ( config.type === "save" ) label = ability;
      else label = `${game.i18n.localize("NAHEULBEUL.Concentration")} ${ability ? `(${abbreviation})` : ""}`;
      if ( showDC ) label = game.i18n.format("EDITOR.NAHEULBEUL.Inline.DC", { dc: config.dc, check: label });
      label = game.i18n.format(`EDITOR.NAHEULBEUL.Inline.Save${longSuffix}`, { save: label });
      break;
    default:
      return "";
  }
  ...
  if ( config.icon ) {
    switch ( config.type ) {
      case "check":
      case "skill":
        label = `<i class="naheulbeuk-icon" data-src="systems/naheulbeuk/icons/svg/ability-score-improvement.svg"></i>${label}`;
        break;
      case "tool":
...
function createRequestLink(label, dataset) {
  ...
  if ( game.user.isGM ) {
    ...
    gmLink.dataset.tooltip = "EDITOR.NAHEULBEUL.Inline.RequestRoll";
...
async function rollAction(event) {
  ...
  const options = { event };
  if ( ability in CONFIG.NAHEULBEUL.abilities ) options.ability = ability;
  ...
  if ( (action === "roll") || !game.user.isGM ) {
    ...
    try {
      ...
      if ( !actors.length ) {
        ui.notifications.warn("EDITOR.NAHEULBEUL.Inline.Warning.NoActor", { localize: true });
        return;
      }
  ...
  // Roll request
    ...
    const chatData = {
      ...
      content: await foundry.applications.handlebars.renderTemplate(
        "systems/naheulbeuk/templates/chat/roll-request-card.hbs", { buttons }
      ),
      flavor: game.i18n.localize("EDITOR.NAHEULBEUL.Inline.RollRequest"),
...
async function rollAttack(event) {
  ...
  const messageConfig = {
    data: {
      ...
      flavor: game.i18n.localize("NAHEULBEUL.AttackRoll"),
  ...
  if ( rolls?.length ) {
    Hooks.callAll("naheulbeuk.rollAttack", rolls, { subject: null, ammoUpdate: null });
    Hooks.callAll("naheulbeuk.rollAttackV2", rolls, { subject: null, ammoUpdate: null });
    Hooks.callAll("naheulbeuk.postRollAttack", rolls, { subject: null });
  }
}
...
async function rollDamage(event) {
  ...
  const messageConfig = {
    ...
    data: {
      ...
      flavor: game.i18n.localize(`NAHEULBEUL.${rollType === "healing" ? "Healing" : "Damage"}Roll`),
      speaker: ChatMessage.implementation.getSpeaker()
    }
  };
  ...
  Hooks.callAll("naheulbeuk.rollDamage", rolls);
  Hooks.callAll("naheulbeuk.rollDamageV2", rolls);
}
...
async function _fetchActivity(uuid, scaling) {
  ...
  const item = activity.item.clone({ "flags.naheulbeuk.scaling": scaling }, { keepId: true });
...
async function useItem({ rollActivityUuid, rollActivityName, rollItemUuid, rollItemName, rollItemActor }={}) {
  ...
  if ( !item ) {
    ...
    if ( item && canvas.tokens.controlled.length ) ui.notifications.warn(
      game.i18n.format("MACRO.NaheulbeukMissingTargetWarn", {
  ...
  if ( item ) {
    if ( rollActivityName ) {
      ...
      else ui.notifications.warn(game.i18n.format("EDITOR.NAHEULBEUL.Inline.Warning.NoActivityOnItem", {
  ...
  else ui.notifications.warn(game.i18n.format("EDITOR.NAHEULBEUL.Inline.Warning.NoItemOnActor", {
