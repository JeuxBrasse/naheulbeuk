...
export default class ModuleArt {
  ...
  async registerModuleArt() {
    ...
    for ( const { id, mapping, credit } of this.constructor.getArtModules().reverse() ) {
      foundry.utils.logCompatibilityWarning(
        "The Naheulbeuk `ModuleArt` system has been deprecated and replaced with core's `CompendiumArt` system.",
  ...
  async #parseArtMapping(moduleId, mapping, credit) {
    let settings = game.settings.get("naheulbeuk", "moduleArtConfiguration")?.[moduleId];
  ...
  static getModuleArtPath(module) {
    ...
    const artPath = flags?.["naheulbeuk-art"];
  ...
  static getArtModules() {
    const settings = game.settings.get("naheulbeuk", "moduleArtConfiguration");
    const unsorted = [];
    const configs = [];
    ...
    for ( const module of game.modules ) {
      ...
      if ( !mapping ) continue;
      const config = { id: module.id, label: module.title, credit: flags?.["naheulbeuk-art-credit"], mapping };
      configs.push(config);
