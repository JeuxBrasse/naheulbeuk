 ...
 * @param {ItemNaheulbeuk} item          Item to which this advancement belongs.
...
export default class Advancement extends PseudoDocumentMixin(BaseAdvancementData) {
  ...
  static get metadata() {
    return {
      name: "Advancement",
      label: "DOCUMENT.NAHEULBEUK.Advancement",
      order: 100,
      icon: "icons/svg/upgrade.svg",
      typeIcon: "icons/svg/upgrade.svg",
      title: game.i18n.localize("NAHEULBEUK.AdvancementTitle"),
      hint: "",
      multiLevel: false,
   ...
   * @param {ItemNaheulbeuk} item  Item to check against.
  ...
  async delete(options={}) {
    if ( this.item.actor?.system.metadata?.supportsAdvancement
        && !game.settings.get("naheulbeuk", "disableAdvancements") ) {
      const manager = naheulbeuk.applications.advancement.AdvancementManager
  ...
  async createItemData(uuid, id) {
    ...
    return source.clone({
      ...
      "flags.naheulbeuk.sourceId": uuid,
      "flags.naheulbeuk.advancementOrigin": advancementOrigin,
      "flags.naheulbeuk.advancementRoot": this.item.getFlag("naheulbeuk", "advancementRoot") ?? advancementOrigin
  ...
  getContextMenuOptions() {
    if ( this.item.isOwner && !this.item.collection?.locked ) return [
      {
        name: "NAHEULBEUK.ADVANCEMENT.Action.Edit",
        icon: "<i class='fas fa-edit fa-fw'></i>",
        callback: () => this.sheet?.render(true)
      },
      {
        name: "NAHEULBEUK.ADVANCEMENT.Action.Duplicate",
      ...
      {
        name: "NAHEULBEUK.ADVANCEMENT.Action.Delete",
    ...
    return [{
      name: "NAHEULBEUK.ADVANCEMENT.Action.View",
  ...
  static onContextMenu(item, target) {
     ...
     * @function naheulbeuk.getItemAdvancementContext
     * @memberof hookEvents
     ...
     */
    Hooks.callAll("naheulbeuk.getItemAdvancementContext", advancement, target, menuItems);
  ...
  static _createDialogData(type, parent) {
    const Advancement = CONFIG.NAHEULBEUK.advancementTypes[type].documentClass;
    return {
      type,
  ...
  static _createDialogTypes(parent) {
    return Object.entries(CONFIG.NAHEULBEUK.advancementTypes)
