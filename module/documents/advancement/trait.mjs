...
export default class TraitAdvancement extends Advancement {
  ...
  static get metadata() {
    return foundry.utils.mergeObject(super.metadata, {
      ...
      typeIcon: "systems/naheulbeuk/icons/svg/trait.svg",
      title: game.i18n.localize("NAHEULBEUK.ADVANCEMENT.Trait.Title"),
      hint: game.i18n.localize("NAHEULBEUK.ADVANCEMENT.Trait.Hint"),
      apps: {
        config: TraitConfig,
        flow: TraitFlow
      }
  ...
  static localize() {
    ...
    localizeSchema(
      ...
      ["NAHEULBEUK.ADVANCEMENT.Trait.FIELDS.choices"]
    );
  }
  ...
  sortingValueForLevel(levels) {
    const traitOrder = Object.keys(CONFIG.NAHEULBEUK.traits).findIndex(k => k === this.representedTraits().first());
    const modeOrder = Object.keys(CONFIG.NAHEULBEUK.traitModes).findIndex(k => k === this.configuration.mode);
  ...
  async apply(level, data) {
    ...
    for ( const key of data.chosen ) {
      ...
      if ( key.startsWith("tool") ) {
        ...
        const ability = CONFIG.NAHEULBEUK.tools[toolId]?.ability;
  ...
  async actorSelected() {
    ...
    const traitTypes = this.configuration.mode === "default" ? Object.keys(CONFIG.NAHEULBEUK.traits).filter(k => k !== "dm")
      : filteredKeys(CONFIG.NAHEULBEUK.traits, t => t[this.configuration.mode === "mastery" ? "mastery" : "expertise"]);
  ...
  async availableChoices(chosen) {
    ...
    if ( this.configuration.allowReplacements && (unfilteredLength > available.length) ) {
      ...
      if ( rep.size === 1 ) return {
        ...
        label: game.i18n.format("NAHEULBEUK.ADVANCEMENT.Trait.ChoicesRemaining", {
