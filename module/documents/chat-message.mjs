...
export default class ChatMessageNaheulbeuk extends ChatMessage {
   ...
   */
  get canApplyDamage() {
    const type = this.flags.naheulbeuk?.roll?.type;
  ...
  get canSelectTargets() {
    if ( this.flags.naheulbeuk?.roll?.type !== "attack" ) return false;
  ...
  get isRoll() {
    ...
    return super.isRoll && !this.flags.naheulbeuk?.rest;
  }
  ...
  get shouldDisplayChallenge() {
    ...
    switch ( game.settings.get("naheulbeuk", "challengeVisibility") ) {
  ...
  static migrateData(source) {
    ...
    if ( foundry.utils.hasProperty(source, "flags.naheulbeuk.itemData") ) {
      foundry.utils.setProperty(source, "flags.naheulbeuk.item.data", source.flags.naheulbeuk.itemData);
      delete source.flags.naheulbeuk.itemData;
    }
    if ( foundry.utils.hasProperty(source, "flags.naheulbeuk.use") ) {
      const use = source.flags.naheulbeuk.use;
      foundry.utils.setProperty(source, "flags.naheulbeuk.messageType", "usage");
      if ( use.type ) foundry.utils.setProperty(source, "flags.naheulbeuk.item.type", use.type);
      if ( use.itemId ) foundry.utils.setProperty(source, "flags.naheulbeuk.item.id", use.itemId);
      if ( use.itemUuid ) foundry.utils.setProperty(source, "flags.naheulbeuk.item.uuid", use.itemUuid);
    }
    return source;
  }
  ...
  prepareData() {
    super.prepareData();
    if ( !this.flags.naheulbeuk?.item?.data && this.flags.naheulbeuk?.item?.id ) {
      const itemData = this.getFlag("naheulbeuk", "use.consumed.deleted")?.find(i => i._id === this.flags.naheulbeuk.item.id);
      if ( itemData ) Object.defineProperty(this.flags.naheulbeuk.item, "data", { value: itemData });
    }
    naheulbeuk.registry.messages.track(this);
  }
  ...
  async renderHTML(options={}) {
    ...
    if ( game.settings.get("naheulbeuk", "autoCollapseItemCards") ) {
    ...
    naheulbeuk.bastion._activateChatListeners(this, html);

    /**
     * A hook event that fires after naheulbeuk-specific chat message modifications have completed.
     * @function naheulbeuk.renderChatMessage
     * @memberof hookEvents
     * @param {ChatMessageNaheulbeuk} message  Chat message being rendered.
     * @param {HTMLElement} html       HTML contents of the message.
     */
    Hooks.callAll("naheulbeuk.renderChatMessage", this, html);
  ...
  _collapseTrays(html) {
    let collapse;
    switch ( game.settings.get("naheulbeuk", "autoCollapseChatTrays") ) {
  ...
  _highlightCriticalSuccessFailure(html) {
    ...
    const displayAttackResult = game.user.isGM || (game.settings.get("naheulbeuk", "attackRollVisibility") !== "none");
    const forceSuccess = this.flags.naheulbeuk?.roll?.forceSuccess === true;
    ...
    for ( let [index, d20Roll] of this.rolls.entries() ) {
      ...
      d20Roll = naheulbeuk.dice.D20Roll.fromRoll(d20Roll);
      ...
      const canCrit = ["attack", "death"].includes(this.getFlag("naheulbeuk", "roll.type"));
      const isAttack = this.getFlag("naheulbeuk", "roll.type") === "attack";
  ...
  async _enrichChatCard(html) {
    html.querySelectorAll(".naheulbeuk").forEach(el => el.classList.remove("naheulbeuk")); // Legacy
    ...
    anchor.setAttribute("aria-label", game.i18n.localize("NAHEULBEUK.AdditionalControls"));
    ...
    // SVG icons
    html.querySelectorAll("i.naheulbeuk-icon").forEach(el => {
      const icon = document.createElement("naheulbeuk-icon");
      icon.src = el.dataset.src;
      el.replaceWith(icon);
    });

    // Enriched roll flavor
    const roll = this.getFlag("naheulbeuk", "roll");
    ...
    if ( this.isContentVisible && item && roll ) {
      ...
      const subtitle = roll.type === "damage"
        ? isCritical
          ? game.i18n.localize("NAHEULBEUK.CriticalHit")
          : activity?.damageFlavor ?? game.i18n.localize("NAHEULBEUK.DamageRoll")
        : roll.type === "attack"
  ...
  _enrichAttackTargets(html) {
    ...
    if ( !(attackRoll instanceof naheulbeuk.dice.D20Roll) ) return;

    const masteryConfig = CONFIG.NAHEULBEUK.weaponMasteries[attackRoll.options.mastery];
    if ( masteryConfig ) {
      ...
      `;
      p.innerHTML = `<strong>${game.i18n.format("NAHEULBEUK.WEAPON.Mastery.Flavor")}</strong> ${mastery}`;
      (html.querySelector(".chat-card") ?? html.querySelector(".message-content"))?.appendChild(p);
    }

    const visibility = game.settings.get("naheulbeuk", "attackRollVisibility");
    const isVisible = game.user.isGM || (visibility !== "none");
    if ( !isVisible ) return;

    const targets = this.getFlag("naheulbeuk", "targets");
    ...
    tray.innerHTML = `
      <div class="card-tray targets-tray collapsible collapsed">
        <label class="roboto-upper">
          ...
          <span>${game.i18n.localize("NAHEULBEUK.TargetPl")}</span>
  ...
  _enrichDamageTooltip(rolls, html) {
    ...
    const aggregatedRolls = CONFIG.NAHEULBEUK.aggregateDamageDisplay ? aggregateDamageRolls(rolls) : rolls;
    let { formula, total, breakdown } = aggregatedRolls.reduce((obj, r) => {
      obj.formula.push(CONFIG.NAHEULBEUK.aggregateDamageDisplay ? r.formula : ` + ${r.formula}`);
    ...
    const tooltipContents = breakdown.reduce((str, { type, total, constant, dice, icon, method }) => {
      const config = CONFIG.NAHEULBEUK.damageTypes[type] ?? CONFIG.NAHEULBEUK.healingTypes[type];
      return `${str}
    ...
    const damageOnSave = this.getFlag("naheulbeuk", "roll.damageOnSave");
    if ( damageOnSave ) {
      const p = document.createElement("p");
      p.classList.add("supplement");
      p.innerHTML = `<strong>${game.i18n.format("NAHEULBEUK.SAVE.OnSave")}</strong> ${
        game.i18n.localize(`NAHEULBEUK.SAVE.FIELDS.damage.onSave.${damageOnSave.capitalize()}`)
      }`;
  ...
  _enrichEnchantmentTooltip(html) {
    const enchantmentProfile = this.getFlag("naheulbeuk", "use.enchantmentProfile");
    if ( !enchantmentProfile ) return;

    // Ensure concentration is still being maintained
    const concentrationId = this.getFlag("naheulbeuk", "use.concentrationId");
  ...
  _enrichSaveTooltip(html) {
    ...
    if ( roll.forceSuccess ) content.insertAdjacentHTML("beforeend", `
      <p class="supplement">
        <strong>${game.i18n.localize("NAHEULBEUK.ROLL.Status")}</strong>
        ${game.i18n.localize("NAHEULBEUK.LegendaryResistance.Resisted")}
      </p>
    `);
      ...
      content.insertAdjacentHTML("beforeend", `
        <div class="card-buttons">
          <button type="button">
            ...
            ${game.i18n.localize("NAHEULBEUK.LegendaryResistance.Action.Resist")}
          </button>
        </div>
      `);
  ...
  _enrichUsageEffects(html) {
    if ( this.getFlag("naheulbeuk", "messageType") !== "usage" ) return;
    const item = this.getAssociatedItem();
    const effects = this.getFlag("naheulbeuk", "use.effects")
  ...
  static addChatMessageContextOptions(html, options) {
    ...
    options.push(
      {
        name: game.i18n.localize("NAHEULBEUK.ChatContextDamage"),
      ...
      {
        name: game.i18n.localize("NAHEULBEUK.ChatContextHealing"),
      ...
      {
        name: game.i18n.localize("NAHEULBEUK.ChatContextTempHP"),
      ...
      {
        name: game.i18n.localize("NAHEULBEUK.ChatContextDoubleDamage"),
      ...
      {
        name: game.i18n.localize("NAHEULBEUK.ChatContextHalfDamage"),
      ...
      {
        name: game.i18n.localize("NAHEULBEUK.ChatContextSelectHit"),
      ...
      {
        name: game.i18n.localize("NAHEULBEUK.ChatContextSelectMiss"),
        icon: '<i class="fas fa-bullseye"></i>',
        condition: canTarget,
  ...
  applyChatCardDamage(li, multiplier) {
    const damages = aggregateDamageRolls(this.rolls, { respectProperties: true }).map(roll => ({
      value: Math.max(0, roll.total) * (roll.options.type in CONFIG.NAHEULBEUK.healingTypes ? -1 : 1),
      type: roll.options.type,
      properties: new Set(roll.options.properties ?? [])
    }));
  ...
  _onDelete(options, userId) {
    ...
    naheulbeuk.registry.messages.untrack(this);
  }
  ...
  getAssociatedActivity() {
    const activity = fromUuidSync(this.getFlag("naheulbeuk", "activity.uuid"), { strict: false });
    if ( activity ) return activity;
    return this.getAssociatedItem()?.system.activities?.get(this.getFlag("naheulbeuk", "activity.id"));
  }
   ...
   * @returns {ItemNaheulbeuk|void}
   */
  getAssociatedItem() {
    const item = fromUuidSync(this.getFlag("naheulbeuk", "item.uuid"), { strict: false });
    if ( item ) return item;
    const actor = this.getAssociatedActor();
    if ( !actor ) return;
    const storedData = this.getFlag("naheulbeuk", "item.data") ?? this.getOriginatingMessage().getFlag("naheulbeuk", "item.data");
   ...
   * @returns {ChatMessageNaheulbeuk[]}
   */
  getAssociatedRolls(type) {
    return naheulbeuk.registry.messages.get(this.id, type);
  }
   ...
   * @type {ChatMessageNaheulbeuk}
   */
  getOriginatingMessage() {
    return game.messages.get(this.getFlag("naheulbeuk", "originatingMessage")) ?? this;
  }
}
