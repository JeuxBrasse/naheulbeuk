...
import ContextMenuNaheulbeuk from "../applications/context-menu.mjs";
 ...
 */
export default class ItemNaheulbeuk extends SystemDocumentMixin(Item) {

  /** @override */
  static DEFAULT_ICON = "systems/naheulbeuk/icons/svg/documents/item.svg";
  ...
  static compendiumBrowserTypes({ chosen=new Set() }={}) {
    ...
    choices.physical = {
      label: game.i18n.localize("NAHEULBEUK.ITEM.Category.Physical"),
  ...
  _initializeSource(data, options={}) {
     ...
     * @function naheulbeuk.initializeItemSource
     * @memberof hookEvents
     * @param {ItemNaheulbeuk} item     Item for which the data is being initialized.
     * @param {object} data     Source data being initialized.
     * @param {object} options  Additional data initialization options.
     */
    if ( options.pack || options.parent?.pack ) Hooks.callAll("naheulbeuk.initializeItemSource", this, data, options);
   ...
   */
  get canDelete() {
    return !this.flags.naheulbeuk?.cachedFor;
  }
  ...
  get canDuplicate() {
    return !this.system.metadata?.singleton && !["class", "subclass"].includes(this.type)
      && !this.flags.naheulbeuk?.cachedFor;
  }
   ...
   * @type {ItemNaheulbeuk|Promise<ItemNaheulbeuk>|void}
   */
  get container() {
   ...
   */
  get dependentOrigin() {
    return fromUuidSync(this.flags.naheulbeuk?.dependentOn, { relative: this, strict: false }) ?? null;
  }
   ...
   */
  get scalingIncrease() {
    return this.system?.scalingIncrease ?? this.getFlag("naheulbeuk", "scaling") ?? 0;
  }
   ...
   * @yields {ActiveEffectNaheulbeuk}
   * @returns {Generator<ActiveEffectNaheulbeuk, void, void>}
   */
  *allApplicableEffects() {
  ...
  prepareDerivedData() {
    ...
    if ( this.system.properties ) {
      this.labels.properties = this.system.properties.reduce((acc, prop) => {
        ...
        acc.push({
          abbr: prop,
          label: CONFIG.NAHEULBEUK.itemProperties[prop]?.label,
          icon: CONFIG.NAHEULBEUK.itemProperties[prop]?.icon
        });
        return acc;
      }, []);
    }
  ...
  _prepareAdvancement() {
    ...
    this.advancement = {
      ...
      byLevel: Object.fromEntries(
        Array.fromRange(CONFIG.NAHEULBEUK.maxLevel + 1).slice(minAdvancementLevel).map(l => [l, []])
      ),
      byType: {},
   ...
   * Otherwise, it will be called at the end of `ItemNaheulbeuk#prepareDerivedData`.
   */
   ...
   * @returns {Promise<ChatMessageNaheulbeuk|object|void>}
   */
  async displayCard(message={}) {
    const context = {
      actor: this.actor,
      config: CONFIG.NAHEULBEUK,
    ...
    const messageConfig = foundry.utils.mergeObject({
      ...
      data: {
        content: await foundry.applications.handlebars.renderTemplate(
          "systems/naheulbeuk/templates/chat/item-card.hbs", context
        ),
        flags: {
          "naheulbeuk.item": { id: this.id, uuid: this.uuid, type: this.type }
        },
     ...
     * @function naheulbeuk.preDisplayCard
     * @memberof hookEvents
     * @param {ItemNaheulbeuk} item                           Item for which the card will be created.
     ...
     */
    if ( Hooks.call("naheulbeuk.preDisplayCard", this, messageConfig) === false ) return;
    if ( Hooks.call("naheulbeuk.preDisplayCardV2", this, messageConfig) === false ) return;
     ...
     * @function naheulbeuk.displayCard
     * @memberof hookEvents
     * @param {ItemNaheulbeuk} item                Item for which the chat card is being displayed.
     * @param {ChatMessageNaheulbeuk|object} card  The created ChatMessage instance or ChatMessageData depending on whether
     *                                     options.createMessage was set to `true`.
     */
    Hooks.callAll("naheulbeuk.displayCard", this, card);
  ...
  static chatListeners(html) {
    html.addEventListener("click", event => {
      if ( event.target.closest("[data-context-menu]") ) ContextMenuNaheulbeuk.triggerEvent(event);
  ...
  async createActivity(type, data={}, { renderSheet=true }={}) {
    ...
    const config = CONFIG.NAHEULBEUK.activityTypes[type];
    if ( !config ) throw new Error(`${type} not found in CONFIG.NAHEULBEUK.activityTypes`);
   ...
   * @returns {Promise<ItemNaheulbeuk>}  This item with the changes applied.
   */
   ...
   * @returns {Promise<AdvancementConfig>|ItemNaheulbeuk}  Promise for advancement config for new advancement if local
  ...
  createAdvancement(type, data={}, { renderSheet=true, showConfig=renderSheet, source=false }={}) {
    ...
    const config = CONFIG.NAHEULBEUK.advancementTypes[type];
    if ( !config ) throw new Error(`${type} not found in CONFIG.NAHEULBEUK.advancementTypes`);
    const cls = config.documentClass;
   ...
   * @param {object} [options={}]
   * @param {boolean} [options.source=false]  Should a source-only update be performed?
   * @returns {Promise<ItemNaheulbeuk>|ItemNaheulbeuk}        This item with the changes applied, promised if source is `false`.
   */
   ...
   * @param {object} [options={}]
   * @param {boolean} [options.source=false]  Should a source-only update be performed?
   * @returns {Promise<ItemNaheulbeuk>|ItemNaheulbeuk}        This item with the changes applied.
   */
   ...
   * @param {boolean} [options.source=false]        Should a source-only update be performed?
   * @returns {Promise<AdvancementConfig>|ItemNaheulbeuk}   Promise for advancement config for duplicate advancement if source
  ...
  async deleteDialog(options={}) {
    ...
    if ( this.actor?.system.metadata?.supportsAdvancement && !game.settings.get("naheulbeuk", "disableAdvancements") ) {
    ...
    if ( count ) {
      return Dialog.confirm({
        title: `${game.i18n.format("DOCUMENT.Delete", {type: game.i18n.localize("NAHEULBEUK.Container")})}: ${this.name}`,
        content: `<h4>${game.i18n.localize("AreYouSure")}</h4>
          <p>${game.i18n.format("NAHEULBEUK.ContainerDeleteMessage", {count})}</p>
          <label>
            <input type="checkbox" name="deleteContents">
            ${game.i18n.localize("NAHEULBEUK.ContainerDeleteContents")}
          </label>`,
        yes: html => {
  ...
  static addDirectoryContextOptions(app, entryOptions) {
    entryOptions.push({
      name: "NAHEULBEUK.Scroll.CreateScroll",
      ...
      callback: async li => {
        ...
        const scroll = await ItemNaheulbeuk.createScrollFromSpell(spell);
        if ( scroll ) ItemNaheulbeuk.create(scroll);
      },
      condition: li => {
   ...
   * @param {ItemNaheulbeuk[]} items                     Items to create.
   * @param {object} [context={}]                Context for the item's creation.
   * @param {ItemNaheulbeuk} [context.container]         Container in which to create the item.
  ...
  static async createWithContents(items, { container, keepId=false, transformAll, transformFirst }={}) {
    ...
    if ( container ) {
      ...
      if ( depth > PhysicalItemTemplate.MAX_DEPTH ) {
        ui.notifications.warn(game.i18n.format("NAHEULBEUK.ContainerMaxDepth", { depth: PhysicalItemTemplate.MAX_DEPTH }));
        return;
      }
    }
   ...
   * @returns {Promise<ItemNaheulbeuk|void>}                The created scroll consumable item.
   */
  static async createScrollFromSpell(spell, options={}, config={}) {
    ...
    config = foundry.utils.mergeObject({
      explanation: game.user.getFlag("naheulbeuk", "creation.scrollExplanation") ?? "reference",
    ...
    if ( config.dialog !== false ) {
      ...
      await game.user.setFlag("naheulbeuk", "creation.scrollExplanation", config.explanation);
    }

    // Get spell data
    const itemData = (spell instanceof ItemNaheulbeuk) ? spell.toObject() : spell;
    const flags = itemData.flags ?? {};
    if ( Number.isNumeric(config.level) ) {
      flags.naheulbeuk ??= {};
      flags.naheulbeuk.scaling = Math.max(0, config.level - spell.system.level);
      flags.naheulbeuk.spellLevel = {
        value: config.level,
     ...
     * @function naheulbeuk.preCreateScrollFromSpell
     * @memberof hookEvents
     ...
     */
    if ( Hooks.call("naheulbeuk.preCreateScrollFromSpell", itemData, options, config) === false ) return;

    let { activities, level, properties, source } = itemData.system;

    // Get scroll data
    let scrollUuid;
    const id = CONFIG.NAHEULBEUK.spellScrollIds[level];
    if ( foundry.data.validators.isValidId(id) ) {
      scrollUuid = game.packs.get(CONFIG.NAHEULBEUK.sourcePacks.ITEMS).index.get(id).uuid;
    } else {
      scrollUuid = id;
    }
    ...
    for ( const level of Array.fromRange(itemData.system.level + 1).reverse() ) {
      const values = CONFIG.NAHEULBEUK.spellScrollValues[level];
      if ( values ) {
    ...
    const spellScrollData = foundry.utils.mergeObject(scrollData, {
      name: `${game.i18n.localize("NAHEULBEUK.SpellScroll")}: ${itemData.name}`,
     ...
     * @function naheulbeuk.createScrollFromSpell
     * @memberof hookEvents
     * @param {ItemNaheulbeuk|object} spell              The spell or item data to be made into a scroll.
     * @param {object} spellScrollData           The final item data used to make the scroll.
     * @param {SpellScrollConfiguration} config  Configuration options for scroll creation.
     */
    Hooks.callAll("naheulbeuk.createScrollFromSpell", spell, spellScrollData, config);
  ...
  static async createScrollFromCompendiumSpell(uuid, config={}) {
     ...
     * @function naheulbeuk.preCreateScrollFromCompendiumSpell
     * @memberof hookEvents
     * @param {ItemNaheulbeuk} spell                     Spell to add to the scroll.
     * @param {SpellScrollConfiguration} config  Configuration options for scroll creation.
     * @returns {boolean}                        Explicitly return `false` to prevent the scroll to be created.
     */
    if ( Hooks.call("naheulbeuk.preCreateScrollFromCompendiumSpell", spell, config) === false ) return;
    // Get scroll data
    let scrollUuid;
    const id = CONFIG.NAHEULBEUK.spellScrollIds[spell.system.level];
    if ( foundry.data.validators.isValidId(id) ) {
      scrollUuid = game.packs.get(CONFIG.NAHEULBEUK.sourcePacks.ITEMS).index.get(id).uuid;
    } else {
      scrollUuid = id;
    ...
    const activity = {
      _id: staticID("naheulbeukscrollspell"),
      type: "cast",
      consumption: {
    ...
    // Create the spell scroll data
    const spellScrollData = foundry.utils.mergeObject(scrollData, {
      name: `${game.i18n.localize("NAHEULBEUK.SpellScroll")}: ${spell.name}`,
      system: {
     ...
     * @function naheulbeuk.createScrollFromSpell
     * @memberof hookEvents
     * @param {ItemNaheulbeuk} spell                     The spell or item data to be made into a scroll.
     * @param {object} spellScrollData           The final item data used to make the scroll.
     * @param {SpellScrollConfiguration} config  Configuration options for scroll creation.
     */
    Hooks.callAll("naheulbeuk.createScrollFromSpell", spell, spellScrollData, config);
   ...
   * @param {ItemNaheulbeuk} scroll                         Base spell scroll.
   * @param {ItemNaheulbeuk|object} spell                   Spell being added to the scroll.
  ...
  static _createScrollDescription(scroll, spell, spellDescription, config={}) {
    ...
    const isConc = spell.system.properties[spell instanceof ItemNaheulbeuk ? "has" : "includes"]("concentration");
    ...
    switch ( config.explanation ) {
      case "full":
        ...
        return [
          ...
          `<h3>${spell.name} (${game.i18n.format("NAHEULBEUK.LevelNumber", { level })})</h3>`,
          isConc ? `<p><em>${game.i18n.localize("NAHEULBEUK.Scroll.RequiresConcentration")}</em></p>` : null,
          spellDescription,
          `<h3>${game.i18n.localize("NAHEULBEUK.Scroll.Details")}</h3>`,
          scrollDetails || scrollIntro
        ].filterJoin("");
      case "reference":
        return [
          "<p><em>",
          CONFIG.NAHEULBEUK.spellLevels[level] ?? level,
          " &Reference[Spell Scroll]",
          isConc ? `, ${game.i18n.localize("NAHEULBEUK.Scroll.RequiresConcentration")}` : null,
          "</em></p>",
          spellDescription
        ].filterJoin("");
    }
   ...
   * @param {ActorNaheulbeuk} parent  Parent document within which this Item will be created.
   * @returns {string[]}
   * @protected
   */
  ...
  static getDefaultArtwork(itemData={}) {
    ...
    return { img: CONFIG.NAHEULBEUK.defaultArtwork.Item[type] ?? img };
  }
}
