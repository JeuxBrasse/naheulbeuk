 ...
 * @returns {Promise}
 */
export async function createNaheulbeukMacro(dropData, slot) {
  const macroData = { type: "script", scope: "actor" };
  switch ( dropData.type ) {
    case "Activity":
      const activity = await fromUuid(dropData.uuid);
      if ( !activity ) {
        ui.notifications.warn("MACRO.NaheulbeukUnownedWarn", { localize: true });
        return null;
      }
      foundry.utils.mergeObject(macroData, {
        name: `${activity.item.name}: ${activity.name}`,
        img: activity.img,
        command: `naheulbeuk.documents.macro.rollItem("${activity.item._source.name}", { activityName: "${
    ...
    case "Item":
      ...
      foundry.utils.mergeObject(macroData, {
        name: itemData.name,
        img: itemData.img,
        command: `naheulbeuk.documents.macro.rollItem("${itemData._source.name}", { event })`,
        flags: { "naheulbeuk.itemMacro": true }
      });
      break;
    case "ActiveEffect":
      ...
      foundry.utils.mergeObject(macroData, {
        ...
        img: effectData.img,
        command: `naheulbeuk.documents.macro.toggleEffect("${effectData.name}")`,
        flags: { "naheulbeuk.effectMacro": true }
      });
      break;
    default:
      return;
...
function getMacroTarget(name, documentType) {
  ...
  if ( !actor ) {
    ui.notifications.warn("MACRO.NaheulbeukNoActorSelected", {localize: true});
    return null;
  }
  ...
  if ( documents.length === 0 ) {
    ui.notifications.warn(game.i18n.format("MACRO.NaheulbeukMissingTargetWarn", { actor: actor.name, type, name }));
    return null;
  }
  if ( documents.length > 1 ) {
    ui.notifications.warn(game.i18n.format("MACRO.NaheulbeukMultipleTargetsWarn", { actor: actor.name, type, name }));
  }
  return documents[0];
}
