...
export default function SystemFlagsMixin(Base) {
  class SystemFlags extends Base {
    ...
    prepareData() {
      super.prepareData();
      if ( ("naheulbeuk" in this.flags) && this._systemFlagsDataModel ) {
        this.flags.naheulbeuk = new this._systemFlagsDataModel(this._source.flags.naheulbeuk, { parent: this });
      }
    }
    ...
    async setFlag(scope, key, value) {
      if ( (scope === "naheulbeuk") && this._systemFlagsDataModel ) {
        let diff;
        const changes = foundry.utils.expandObject({ [key]: value });
        if ( this.flags.naheulbeuk ) diff = this.flags.naheulbeuk.updateSource(changes, { dryRun: true });
        else diff = new this._systemFlagsDataModel(changes, { parent: this }).toObject();
        return this.update({ flags: { naheulbeuk: diff } });
      }
