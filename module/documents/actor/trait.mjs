 ...
 * @param {ActorNaheulbeuk} actor  Actor for which to get the fields.
 * @param {string} trait   Trait as defined in `CONFIG.NAHEULBEUK.traits`.
 * @returns {object|void}
 */
...
export function actorKeyPath(trait) {
  const traitConfig = CONFIG.NAHEULBEUK.traits[trait];
 ...
 * @param {ActorNaheulbeuk} actor  Actor from which to retrieve the values.
 * @param {string} trait   Trait as defined in `CONFIG.NAHEULBEUK.traits`.
 ...
 * @param {string} [trait]  Trait as defined in `CONFIG.NAHEULBEUK.traits`, only needed if key isn't prefixed.
 * @returns {string|void}
 */
...
export async function categories(trait) {
  const traitConfig = CONFIG.NAHEULBEUK.traits[trait];
  const config = foundry.utils.deepClone(CONFIG.NAHEULBEUK[traitConfig.configKey ?? trait]);
  ...
  for ( const key of Object.keys(config) ) {
    ...
    if ( traitConfig.children?.[key] ) {
      ...
      for ( const [childKey, value] of Object.entries(CONFIG.NAHEULBEUK[traitConfig.children[key]]) ) {
  ...
  if ( traitConfig.subtypes ) {
    const map = CONFIG.NAHEULBEUK[`${trait}ProficienciesMap`];

    // Merge all ID lists together
    const ids = traitConfig.subtypes.ids.reduce((obj, key) => {
      foundry.utils.mergeObject(obj, CONFIG.NAHEULBEUK[key] ?? {});
      return obj;
    }, {});
 ...
 * in `NAHEULBEUK.sourcePacks.ITEMS`.
 ...
 *                                       false.
 * @returns {Promise<ItemNaheulbeuk>|object}     Promise for a `Document` if `indexOnly` is false & `fullItem` is true,
...
export function getBaseItem(identifier, { indexOnly=false, fullItem=false }={}) {
  ...
  // Full ItemNaheulbeuk document required, always async.
...
export function getBaseItemUUID(identifier) {
  ...
  let pack = CONFIG.NAHEULBEUK.sourcePacks.ITEMS;
...
export function traitIndexFields() {
  ...
  for ( const traitConfig of Object.values(CONFIG.NAHEULBEUK.traits) ) {
...
export function traitLabel(trait, count) {
  ...
  if ( !traitConfig ) return game.i18n.localize(`NAHEULBEUK.TraitGenericPlural.${pluralRule}`);
 ...
 * @param {string} [config.trait]   Trait as defined in `CONFIG.NAHEULBEUK.traits` if not using a prefixed key.
...
export function keyLabel(key, config={}) {
  ...
  const traitData = CONFIG.NAHEULBEUK[traitConfig.configKey ?? trait] ?? {};
  ...
  // Wildcards (e.g. "Artisan's Tools", "any Artisan's Tools", "any 2 Artisan's Tools", or "2 other Artisan's Tools")
    ...
    const localization = `NAHEULBEUK.TraitConfigChoose${final ? "Other" : `Any${count ? "Counted" : "Uncounted"}`}`;
    ...
    for ( const childrenKey of Object.values(traitConfig.children ?? {}) ) {
      const childLabel = CONFIG.NAHEULBEUK[childrenKey]?.[lastKey];
      if ( childLabel ) return childLabel;
    }

    // Base item (e.g. "Shortsword")
    for ( const idsKey of traitConfig.subtypes?.ids ?? [] ) {
      let baseItemId = CONFIG.NAHEULBEUK[idsKey]?.[lastKey];
...
export function choiceLabel(choice, { only=false, final=false }={}) {
  ...
  return game.i18n.format("NAHEULBEUK.TraitConfigChooseList", {
    count: choice.count,
...
export function localizedList({ grants=new Set(), choices=[] }) {
  ...
  return game.i18n.format("NAHEULBEUK.TraitConfigChooseWrapper", {
