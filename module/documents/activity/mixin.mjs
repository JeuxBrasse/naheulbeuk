...
export default function ActivityMixin(Base) {
  class Activity extends DependentDocumentMixin(PseudoDocumentMixin(Base)) {
    ...
    static metadata = Object.freeze({
      name: "Activity",
      label: "DOCUMENT.NAHEULBEUK.Activity",
      sheetClass: ActivitySheet,
      usage: {
        actions: {},
        chatCard: "systems/naheulbeuk/templates/chat/activity-card.hbs",
    ...
    static localize() {
      ...
      if ( fields.damage?.fields.parts ) {
        localizeSchema(fields.damage.fields.parts.element, ["NAHEULBEUK.DAMAGE.FIELDS.damage.parts"]);
      }
      if ( fields.consumption ) {
        localizeSchema(fields.consumption.fields.targets.element, ["NAHEULBEUK.CONSUMPTION.FIELDS.consumption.targets"]);
      }
      if ( fields.uses ) localizeSchema(fields.uses.fields.recovery.element, ["NAHEULBEUK.USES.FIELDS.uses.recovery"]);
    }
     ...
     */
    get canConfigure() {
      if ( CONFIG.NAHEULBEUK.activityTypes[this.type]?.configurable === false ) return false;
     ...
     */
    get damageFlavor() {
      return game.i18n.localize("NAHEULBEUK.DamageRoll");
    }
     ...
     */
    get dependentOrigin() {
      return this.item.effects.get(this.flags?.naheulbeuk?.dependentOn) ?? null;
    }
    ...
    get validConsumptionTypes() {
      const types = new Set(Object.keys(CONFIG.NAHEULBEUK.activityConsumptionTypes));
    ...
    async use(usage={}, dialog={}, message={}) {
      ...
      if ( !this.item.isOwner ) {
        ui.notifications.error("NAHEULBEUK.DocumentUseWarn", { localize: true });
        return;
      }
      if ( !this.canUse ) {
        ui.notifications.error("NAHEULBEUK.ACTIVITY.Warning.UsageNotAllowed", { localize: true });
        return;
      }
       ...
       * @function naheulbeuk.preUseActivity
       * @memberof hookEvents
       ...
       */
      if ( Hooks.call("naheulbeuk.preUseActivity", activity, usageConfig, dialogConfig, messageConfig) === false ) return;
      ...
      if ( usageConfig.concentration?.begin ) {
        const effect = await item.actor.beginConcentrating(activity, { "flags.naheulbeuk.scaling": usageConfig.scaling });
        if ( effect ) {
          results.effects ??= [];
          results.effects.push(effect);
          foundry.utils.setProperty(messageConfig.data, "flags.naheulbeuk.use.concentrationId", effect.id);
        }
       ...
       * @function naheulbeuk.postUseActivity
       * @memberof hookEvents
       ...
       */
      if ( Hooks.call("naheulbeuk.postUseActivity", activity, usageConfig, results) === false ) return results;

      // Trigger any primary action provided by this activity
      if ( usageConfig.subsequentActions !== false ) {
        const deltas = results.message?.flags?.naheulbeuk?.use?.consumed
          ?? results.message?.data?.flags?.naheulbeuk?.use?.consumed;
        const consumed = this.createConsumedFlag(this.actor, deltas);
        if ( consumed ) item.updateSource({ "flags.naheulbeuk.consumed": consumed });
    ...
    async consume(usageConfig, messageConfig) {
       ...
       * @function naheulbeuk.preActivityConsumption
       * @memberof hookEvents
       * @param {Activity} activity                           Activity being activated.
       * @param {ActivityUseConfiguration} usageConfig        Configuration data for the activation.
       * @param {ActivityMessageConfiguration} messageConfig  Configuration info for the created chat message.
       ...
       */
      if ( Hooks.call("naheulbeuk.preActivityConsumption", this, usageConfig, messageConfig) === false ) return false;
       ...
       * performed.
       * @function naheulbeuk.activityConsumption
       * @memberof hookEvents
       * @param {Activity} activity                           Activity being activated.
       * @param {ActivityUseConfiguration} usageConfig        Configuration data for the activation.
       * @param {ActivityMessageConfiguration} messageConfig  Configuration info for the created chat message.
       ...
       * @returns {boolean}  Explicitly return `false` to prevent activity from being activated.
       */
      if ( Hooks.call("naheulbeuk.activityConsumption", this, usageConfig, messageConfig, updates) === false ) return false;

      const consumed = await this.#applyUsageUpdates(updates);
      if ( !foundry.utils.isEmpty(consumed) ) {
        foundry.utils.setProperty(messageConfig, "data.flags.naheulbeuk.use.consumed", consumed);
      }
      if ( usageConfig.cause?.activity ) {
        foundry.utils.setProperty(messageConfig, "data.flags.naheulbeuk.use.cause", usageConfig.cause.activity);
      }

      /**
       * A hook event that fires after an item's resource consumption is calculated and applied.
       * @function naheulbeuk.postActivityConsumption
       * @memberof hookEvents
       * @param {Activity} activity                           Activity being activated.
       * @param {ActivityUseConfiguration} usageConfig        Configuration data for the activation.
       * @param {ActivityMessageConfiguration} messageConfig  Configuration info for the created chat message.
       ...
       * @returns {boolean}  Explicitly return `false` to prevent activity from being activated.
       */
      if ( Hooks.call("naheulbeuk.postActivityConsumption", this, usageConfig, messageConfig, updates) === false ) return false;
    ...
    _prepareUsageConfig(config) {
      ...
      if ( config.consume !== false ) {
        const activationConfig = CONFIG.NAHEULBEUK.activityActivationTypes[this.activation.type] ?? {};
      ...
      const levelingFlag = this.item.getFlag("naheulbeuk", "spellLevel");
      if ( levelingFlag ) {
        ...
        if ( this.requiresSpellSlot ) {
          ...
          const model = CONFIG.NAHEULBEUK.spellcasting[method];
          config.spell ??= {};
      ...
      if ( this.requiresConcentration && !game.settings.get("naheulbeuk", "disableConcentration") ) {
        ...
        if ( limit && (limit <= effects.size) ) config.concentration.end ??= effects.find(e => {
          const data = e.flags.naheulbeuk?.item?.data ?? {};
     ...
     * @param {ActivityUseConfiguration} usageConfig        Configuration data for the activation.
     * @param {ActivityMessageConfiguration} messageConfig  Configuration data for the chat message.
     * @param {ItemNaheulbeuk} item                                 Clone of the item that contains this activity.
     * @protected
     */
    async _prepareUsageScaling(usageConfig, messageConfig, item) {
      ...
      if ( levelingFlag ) {
        ...
        if ( level ) {
          ...
          foundry.utils.setProperty(messageConfig, "data.flags.naheulbeuk.use.spellLevel", level);
        }
      }

      if ( usageConfig.scaling ) {
        foundry.utils.setProperty(messageConfig, "data.flags.naheulbeuk.scaling", usageConfig.scaling);
        if ( usageConfig.scaling !== item.flags.naheulbeuk?.scaling ) {
          item.actor._embeddedPreparation = true;
          item.updateSource({ "flags.naheulbeuk.scaling": usageConfig.scaling });
    ...
    async _prepareUsageUpdates(config, { returnErrors=false }={}) {
      ...
      if ( ((config.consume === true) || config.consume.action) && activationConfig?.consume ) {
        ...
        if ( current && !containsConsumption ) {
          let message;
          if ( current.value < 1 ) message = "NAHEULBEUK.ACTIVATION.Warning.NoActions";
          else if ( count > current.value ) message = "NAHEULBEUK.ACTIVATION.Warning.NotEnoughActions";
          if ( message ) {
        ...
        if ( linkedActivity ) {
          ...
          if ( foundry.utils.getType(results) === "Object" ) {
            ...
            if ( updates.delete.includes(linkedActivity.item.id)
              && (this.item.getFlag("naheulbeuk", "cachedFor") === otherLinkedActivity?.relativeUUID) ) {
      ...
      // Handle spell slot consumption
        ...
        const spellcasting = CONFIG.NAHEULBEUK.spellcasting[this.item.system.method];
        ...
        if ( slotData ) {
          if ( slotData.value ) {
            ...
            const err = new ConsumptionError(game.i18n.format("NAHEULBEUK.SpellCastNoSlots", {
      ...
      if ( config.concentration?.begin ) {
        ...
        if ( config.concentration.end ) {
          ...
          if ( !replacedEffect ) errors.push(
            new ConsumptionError(game.i18n.localize("NAHEULBEUK.ConcentratingMissingItem"))
          );
        }
        ...
        // Cannot begin more concentrations than the limit
          ...
          new ConsumptionError(game.i18n.localize("NAHEULBEUK.ConcentratingLimited"))
        );
      }
    ...
    async _usageChatContext(message) {
      ...
      if ( this.activation.condition ) {
        supplements.push(`<strong>${game.i18n.localize("NAHEULBEUK.Trigger")}</strong> ${this.activation.condition}`);
      }
      if ( data.materials?.value ) {
        supplements.push(`<strong>${game.i18n.localize("NAHEULBEUK.Materials")}</strong> ${data.materials.value}`);
      }
      const buttons = this._usageChatButtons(message);

      // Include spell level in the subtitle.
      if ( this.item.type === "spell" ) {
        const spellLevel = foundry.utils.getProperty(message, "data.flags.naheulbeuk.use.spellLevel");
        const { spellLevels, spellSchools } = CONFIG.NAHEULBEUK;
    ...
    _finalizeMessageConfig(usageConfig, messageConfig, results) {
      ...
      if ( effects ) foundry.utils.setProperty(messageConfig.data, "flags.naheulbeuk.use.effects", effects);
    }
    ...
    _usageChatButtons(message) {
      ...
      if ( this.target?.template?.type ) buttons.push({
        label: game.i18n.localize("NAHEULBEUK.TARGET.Action.PlaceTemplate"),
      ...
      if ( message.hasConsumption ) buttons.push({
        label: game.i18n.localize("NAHEULBEUK.CONSUMPTION.Action.ConsumeResource"),
        icon: '<i class="fa-solid fa-cubes-stacked" inert></i>',
        dataset: {
          action: "consumeResource"
        }
      }, {
        label: game.i18n.localize("NAHEULBEUK.CONSUMPTION.Action.RefundResource"),
     ...
     * @param {ChatMessageNaheulbeuk} message     Chat message containing the button.
     * @returns {boolean}
     */
    shouldHideChatButton(button, message) {
      const flag = message.getFlag("naheulbeuk", "use.consumed");
     ...
     * @param {ActivityMessageConfiguration} message  Configuration info for the created message.
     * @returns {Promise<ChatMessageNaheulbeuk|object>}
     * @protected
     */
    async _createUsageMessage(message) {
       ...
       * @function naheulbeuk.preCreateUsageMessage
       * @memberof hookEvents
       * @param {Activity} activity                     Activity for which the card will be created.
       * @param {ActivityMessageConfiguration} message  Configuration info for the created message.
       */
      Hooks.callAll("naheulbeuk.preCreateUsageMessage", this, messageConfig);
       ...
       * @function naheulbeuk.postCreateUsageMessage
       * @memberof hookEvents
       * @param {Activity} activity          Activity for which the card was created.
       * @param {ChatMessageNaheulbeuk|object} card  Created card or configuration data if not created.
       */
      Hooks.callAll("naheulbeuk.postCreateUsageMessage", this, card);
    ...
    async rollDamage(config={}, dialog={}, message={}) {
      ...
      if ( canUpdate && !foundry.utils.isEmpty(lastDamageTypes)
        ...
        await this.item.setFlag("naheulbeuk", `last.${this.id}.damageType`, lastDamageTypes);
      }

      /**
       * A hook event that fires after damage has been rolled.
       * @function naheulbeuk.rollDamage
       * @memberof hookEvents
       * @param {DamageRoll[]} rolls       The resulting rolls.
       * @param {object} [data]
       * @param {Activity} [data.subject]  The activity that performed the roll.
       */
      Hooks.callAll("naheulbeuk.rollDamage", rolls, { subject: this });
      Hooks.callAll("naheulbeuk.rollDamageV2", rolls, { subject: this });
    ...
    getContextMenuOptions() {
      ...
      if ( this.item.isOwner && !compendiumLocked ) {
        entries.push({
          name: "NAHEULBEUK.ContextMenuActionEdit",
          icon: '<i class="fas fa-pen-to-square fa-fw"></i>',
          callback: () => this.sheet.render({ force: true })
        }, {
          name: "NAHEULBEUK.ContextMenuActionDuplicate",
          ...
          name: "NAHEULBEUK.ContextMenuActionDelete",
          icon: '<i class="fas fa-trash fa-fw"></i>',
          callback: () => this.deleteDialog()
        });
      } else {
        entries.push({
          name: "NAHEULBEUK.ContextMenuActionView",
      ...
      if ( "favorites" in (this.actor?.system ?? {}) ) {
        ...
        entries.push({
          name: isFavorited ? "NAHEULBEUK.FavoriteRemove" : "NAHEULBEUK.Favorite",
     ...
     * @param {ChatMessageNaheulbeuk} message  Message associated with the activation.
     */
    async #onChatAction(event, target, message) {
      const consumed = this.createConsumedFlag(message.getAssociatedActor(), message.getFlag("naheulbeuk", "use.consumed"));
      const scaling = message.getFlag("naheulbeuk", "scaling") ?? 0;
      const item = (consumed || scaling) ? this.item.clone({
        "flags.naheulbeuk": { consumed, scaling }
     ...
     * @param {ItemNaheulbeuk} item         The Item the Activity belongs to.
    ...
    static onContextMenu(item, target) {
       ...
       * @function naheulbeuk.getItemActivityContext
       * @memberof hookEvents
       ...
       */
      Hooks.callAll("naheulbeuk.getItemActivityContext", activity, target, menuItems);
    ...
    async #consumeResource(event, target, message) {
      const messageConfig = {};
      const scaling = message.getFlag("naheulbeuk", "scaling");
      const usageConfig = { consume: true, event, scaling };
      const linkedActivity = this.getLinkedActivity(message.getFlag("naheulbeuk", "use.cause"));
    ...
    async #refundResource(event, target, message) {
      const consumed = message.getFlag("naheulbeuk", "use.consumed");
      if ( !foundry.utils.isEmpty(consumed) ) {
        await this.refund(consumed);
        await message.unsetFlag("naheulbeuk", "use.consumed");
      }
    }
    ...
    async #placeTemplate() {
      ...
      try {
        ...
        Hooks.onError("Activity#placeTemplate", err, {
          msg: game.i18n.localize("NAHEULBEUK.TARGET.Warning.PlaceTemplate"),
          log: "error",
          notify: "error"
        });
      }
     ...
     * @returns {Promise<FavoriteDataNaheulbeuk>}
     */
    ...
    getLinkedActivity(relativeUUID) {
      ...
      relativeUUID ??= this.item.getFlag("naheulbeuk", "cachedFor");
    ...
    getRollData(options) {
      ...
      rollData.consumed = this.item.flags.naheulbeuk?.consumed;
     ...
     * @param {ItemNaheulbeuk} item  Candidate item to which the activity might be added.
    ...
    static _createDialogTypes(parent) {
      return Object.entries(CONFIG.NAHEULBEUK.activityTypes)
