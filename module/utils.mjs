/**
 * @import { TargetDescriptorNaheulbeuk, UnitConfiguration } from "./_types.mjs";
 */
 ...
 * @param {string} unit          Length unit as defined in `CONFIG.NAHEULBEUK.movementUnits`.
 * @param {object} [options={}]  Formatting options passed to `formatNumber`.
 * @returns {string}
 */
export function formatLength(value, unit, options={}) {
  return _formatSystemUnits(value, unit, CONFIG.NAHEULBEUK.movementUnits[unit], options);
}
...
export function formatNumber(value, { blank, numerals, ordinal, words, ...options }={}) {
  if ( words && game.i18n.has(`NAHEULBEUK.NUMBER.${value}`, false) ) return game.i18n.localize(`NAHEULBEUK.NUMBER.${value}`);
...
function _formatNumberAsOrdinal(n, options={}) {
  ...
  return game.i18n.has(`NAHEULBEUK.ORDINAL.${pr}`) ? game.i18n.format(`NAHEULBEUK.ORDINAL.${pr}`, { number }) : number;
}
 ...
 * @param {string} unit                     Unit as defined in `CONFIG.NAHEULBEUK.travelUnits`.
 * @param {object} [options={}]             Formatting options passed to `formatNumber`.
...
export function formatTravelSpeed(value, unit, { period="hour", ...options }={}) {
  const unitConfig = CONFIG.NAHEULBEUK.travelUnits[unit];
 ...
 * @param {string} unit          Units as defined in `CONFIG.NAHEULBEUK.timeUnits`.
 * @param {object} [options={}]  Formatting options passed to `formatNumber`.
 * @returns {string}
 */
export function formatTime(value, unit, options={}) {
  ...
  const config = CONFIG.NAHEULBEUK.timeUnits[unit];
  if ( config?.counted ) {
 ...
 * @param {string} unit          Volume unit as defined in `CONFIG.NAHEULBEUK.volumeUnits`.
 * @param {object} [options={}]  Formatting options passed to `formatNumber`.
 * @returns {string}
 */
export function formatVolume(value, unit, options={}) {
  return _formatSystemUnits(value, unit, CONFIG.NAHEULBEUK.volumeUnits[unit], options);
}
 ...
 * @param {string} unit          Weight unit as defined in `CONFIG.NAHEULBEUK.weightUnits`.
 * @param {object} [options={}]  Formatting options passed to `formatNumber`.
 * @returns {string}
 */
export function formatWeight(value, unit, options={}) {
  return _formatSystemUnits(value, unit, CONFIG.NAHEULBEUK.weightUnits[unit], options);
}
...
export function prepareFormulaValue(model, keyPath, label, rollData) {
  ...
  try {
    ...
    if ( item.isEmbedded ) {
      const message = game.i18n.format("NAHEULBEUK.FormulaMalformedError", { property, name: model.name ?? item.name });
 ...
 * @param {object} [options={}]
 * @param {ActorNaheulbeuk} [options.actor]            Actor for which the value is being prepared.
 * @param {ItemNaheulbeuk} [options.item]              Item for which the value is being prepared.
...
export function replaceFormulaData(formula, data, { actor, item, missing="0", property }={}) {
  ...
  if ( (missingReferences.size > 0) && actor && property ) {
    ...
    const message = game.i18n.format("NAHEULBEUK.FormulaMissingReferenceWarn", {
 ...
 * @param {string} action  Keybinding action within the `naheulbeuk` namespace.
...
export function areKeysPressed(event, action) {
  ...
  return game.keybindings.get("naheulbeuk", action).some(b => {
 ...
 * @returns {TargetDescriptorNaheulbeuk[]}
 */
 ...
 * @param {ActorNaheulbeuk} [actor]  Only allow tokens associated with this specific actor.
 * @returns {TokenNaheulbeuk[]}
 */
...
export function convertLength(value, from, to, { strict=true }={}) {
  const message = unit => `Length unit ${unit} not defined in CONFIG.NAHEULBEUK.movementUnits`;
  return _convertSystemUnits(value, from, to, CONFIG.NAHEULBEUL.movementUnits, { message, strict });
}
 ...
 * @param {string} from                     The initial unit as defined in `CONFIG.NAHEULBEUL.timeUnits`.
 * @param {object} [options={}]
...
export function convertTime(value, from, { combat=false, strict=true, to }={}) {
  const base = value * (CONFIG.NAHEULBEUL.timeUnits[from]?.conversion ?? 1);
  if ( !to ) {
    // Find unit with largest conversion value that can still display the value
    const unitOptions = Object.entries(CONFIG.NAHEULBEUL.timeUnits)
      .reduce((arr, [key, v]) => {
  ...
  const message = unit => `Time unit ${unit} not defined in CONFIG.NAHEULBEUL.timeUnits`;
  return { value: _convertSystemUnits(value, from, to, CONFIG.NAHEULBEUL.timeUnits, { message, strict }), unit: to };
}
...
export function convertTravelSpeed(value, from, { strict=false, to }) {
  const message = unit => `Travel speed unit ${unit} not defined in CONFIG.NAHEULBEUL.travelUnits`;
  return { value: _convertSystemUnits(value, from, to, CONFIG.NAHEULBEUL.travelUnits, { message, strict }), unit: to };
}
 ...
 * @param {string} from                    The initial unit as defined in `CONFIG.NAHEULBEUL.weightUnits`.
 * @param {string} to                      The final units.
 * @param {object} [options={}]
 * @param {boolean} [options.strict=true]  Throw an error if either unit isn't found.
...
export function convertWeight(value, from, to, { strict=true }={}) {
  const message = unit => `Weight unit ${unit} not defined in CONFIG.NAHEULBEUL.weightUnits`;
  return _convertSystemUnits(value, from, to, CONFIG.NAHEULBEUL.weightUnits, { message, strict });
}
...
export function defaultUnits(type) {
  ...
  return CONFIG.NAHEULBEUL.defaultUnits[type]?.[game.settings.get("naheulbeuk", settingKey) ? "metric" : "imperial"];
}
...
export async function preloadHandlebarsTemplates() {
  ...
  for ( const path of partials ) {
    ...
    paths[`naheulbeuk.${path.split("/").pop().replace(".hbs", "")}`] = path;
  }
...
export function generateIcon(icon, { alt }={}) {
  ...
  if ( icon?.startsWith("fa") ) {
    ...
    element = document.createElement(icon.endsWith(".svg") ? "naheulbeuk-icon" : "img");
    element.src = icon;
  } else {
    return null;
  }
...
function itemContext(context, options) {
  if ( arguments.length !== 2 ) throw new Error("#naheulbeuk-itemContext requires exactly one argument");
...
function concealSection(conceal, options) {
  ...
  <div class="unidentified-notice">
      <div>
          <strong>${game.i18n.localize("NAHEULBEUK.Unidentified.Title")}</strong>
          <p>${game.i18n.localize("NAHEULBEUK.Unidentified.Notice")}</p>
      </div>
  </div>`;
  return content;
}
...
/**
 * Register custom Handlebars helpers used by Naheulbeuk.
 */
export function registerHandlebarsHelpers() {
  ...
  Handlebars.registerHelper({
    ...
    "naheulbeuk-concealSection": concealSection,
    "naheulbeuk-dataset": dataset,
    "naheulbeuk-icon": (icon, { hash: options }) => {
    ...
    "naheulbeuk-formatCR": (value, options) => formatCR(value, options.hash),
    "naheulbeuk-formatLength": curryUnitFormatter(formatLength),
    "naheulbeuk-formatModifier": formatModifier,
    "naheulbeuk-formatTravelSpeed": curryUnitFormatter(formatTravelSpeed),
    "naheulbeuk-formatTime": curryUnitFormatter(formatTime),
    "naheulbeuk-formatVolume": curryUnitFormatter(formatVolume),
    "naheulbeuk-formatWeight": curryUnitFormatter(formatWeight),
    "naheulbeuk-groupedSelectOptions": groupedSelectOptions,
    "naheulbeuk-itemContext": itemContext,
    "naheulbeuk-linkForUuid": (uuid, options) => linkForUuid(uuid, options.hash),
    "naheulbeuk-numberFormat": (value, options) => formatNumber(value, options.hash),
    "naheulbeuk-numberParts": (value, options) => formatNumberParts(value, options.hash),
    "naheulbeuk-object": makeObject,
    "naheulbeuk-textFormat": formatText
  });
}
 ...
 * @param {string} configKeyPath          Key path within `CONFIG.NAHEULBEUK` to localize.
 * @param {object} [options={}]
 ...
 * @param {object} config  The `CONFIG.NAHEULBEUK` object to localize and sort. *Will be mutated.*
 */
 ...
 * @param {object} [options]
 * @param {ActorNaheulbeuk} [options.actor]  An optional reference actor.
 * @param {ItemNaheulbeuk} [options.item]    An optional reference item.
 * @returns {string|void}
 */
export function getHumanReadableAttributeLabel(attr, { actor, item }={}) {
  ...
  if ( (attr === "details.xp.value") && (actor?.type === "npc") ) {
    return game.i18n.localize("NAHEULBEUK.ExperiencePoints.Value");
  }
  ...
  if ( item && attr.startsWith("activities.") ) {
    ...
    else if ( attr === "uses.spent" ) label = "NAHEULBEUK.Uses";
  }
  ...
  // Item labels
    ...
    else if ( attr === "hd.spent" ) label = "NAHEULBEUK.HitDice";
    else if ( attr === "uses.spent" ) label = "NAHEULBEUK.Uses";
  ...
  // Abilities.
    ...
    label = game.i18n.format("NAHEULBEUK.AbilityScoreL", { ability: CONFIG.NAHEULBEUK.abilities[key].label });
  }

  // Skills.
  else if ( attr.startsWith("skills.") ) {
    const [, key] = attr.split(".");
    label = game.i18n.format("NAHEULBEUK.SkillPassiveScore", { skill: CONFIG.NAHEULBEUK.skills[key].label });
  }

  // Currency
  else if ( attr.startsWith("currency.") ) {
    const [, key] = attr.split(".");
    label = CONFIG.NAHEULBEUK.currencies[key]?.label;
  }
