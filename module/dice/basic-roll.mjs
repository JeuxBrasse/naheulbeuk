...
import ChatMessageNaheulbeuk from "../documents/chat-message.mjs";
...
export default class BasicRoll extends Roll {
  ...
  static async buildConfigure(config={}, dialog={}, message={}) {
     ...
     * method (e.g. `naheulbeuk.preRollSkill`, `naheulbeuk.preRollAbilityCheck`, `naheulbeuk.preRoll`). Exact contents of the
     * configuration object will also change based on the roll type, but the same objects will always be present.
     * @function naheulbeuk.preRoll
     * @memberof hookEvents
    ...
    for ( const hookName of config.hookNames ) {
      if ( Hooks.call(`naheulbeuk.preRoll${hookName.capitalize()}`, config, dialog, message) === false ) return [];
      if ( Hooks.call(`naheulbeuk.preRoll${hookName.capitalize()}V2`, config, dialog, message) === false ) return [];
    }
    ...
    if ( dialog.configure === false ) {
      rolls = config.rolls?.map((r, index) => {
        ...
        for ( const hookName of config.hookNames ) {
          Hooks.callAll(`naheulbeuk.postBuild${hookName.capitalize()}RollConfig`, config, r, index);
        }
    ...
    const rollType = foundry.utils.getProperty(message, "data.flags.naheulbeuk.roll.type");
     ...
     * Multiple hooks may be called depending on the rolling method (e.g. `naheulbeuk.postSkillCheckRollConfiguration`,
     * `naheulbeuk.postAbilityTestRollConfiguration`, and `naheulbeuk.postRollConfiguration` for skill checks). Exact contents of
     * the configuration object will also change based on the roll type, but the same objects will always be present.
     * @function naheulbeuk.postRollConfiguration
     * @memberof hookEvents
     ...
     */
    for ( const hookName of config.hookNames ) {
      const name = `naheulbeuk.post${hookName.capitalize()}RollConfiguration`;
   ...
   * @param {BasicRollProcessConfiguration} config   Configuration for the rolls.
   * @param {BasicRollMessageConfiguration} message  Configuration for message creation.
   * @returns {ChatMessageNaheulbeuk|void}
   */
  static async buildPost(rolls, config, message) {
    ...
    if ( messageId ) foundry.utils.setProperty(message.data, "flags.naheulbeuk.originatingMessage", messageId);
