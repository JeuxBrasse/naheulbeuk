...
/**
 * A type of Roll specific to a damage (or healing) roll in the Naheulbeuk system.
 * @extends {BasicRoll}
 */
export default class DamageRoll extends BasicRoll {
  ...
  static async build(config={}, dialog={}, message={}) {
    config.critical ??= {};
    config.critical.multiplyNumeric ??= game.settings.get("naheulbeuk", "criticalDamageModifiers");
    config.critical.powerfulCritical ??= game.settings.get("naheulbeuk", "criticalDamageMaxDice");
      ...
      if ( term instanceof DiceTerm ) {
        ...
        if ( this.isCritical ) {
          ...
          if ( critical.powerfulCritical ) {
            ...
            if ( bonus > 0 ) {
              const flavor = term.flavor?.toLowerCase().trim() ?? game.i18n.localize("NAHEULBEUK.PowerfulCritical");
