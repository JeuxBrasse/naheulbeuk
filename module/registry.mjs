...
class MessageRegistry {
   ...
   * @param {ChatMessageNaheulbeuk} message  Message to add to the registry.
   */
  static track(message) {
    const origin = message.getFlag("naheulbeuk", "originatingMessage");
    const type = message.getFlag("naheulbeuk", "roll.type");
   ...
   * @param {ChatMessageNaheulbeuk} message  Message to remove from the registry.
   */
...
class SpellListRegistry {
   ...
   */
  static get options() {
    return Object.entries(CONFIG.NAHEULBEUK.spellListTypes).map(([type, group]) => {
   ...
   * @param {string} type          Type of list as defined in `CONFIG.NAHEULBEUK.spellListTypes`. Can also be a combination
...
export class SpellList {
   ...
   */
  get name() {
    return naheulbeuk.registry[SpellList.#REGISTRIES[this.metadata.type]]?.get(this.metadata.identifier)?.name
      ?? this.metadata.name;
  }
   ...
   * @param {ItemNaheulbeuk|string} spell  Spell item or a compendium UUID.
   * @returns {boolean}
   */
  has(spell) {
...
class SummonRegistry {
   ...
   * @param {ActorNaheulbeuk} actor  Actor for which to find the summoned creatures.
   * @returns {ActorNaheulbeuk[]}
   */
