...
import DragDropNaheulbeuk from "../../drag-drop.mjs";
...
export default function DragDropApplicationMixin(Base) {
  return class DragDropApplication extends Base {
    /** @override */
    _onDragOver(event) {
      const data = DragDropNaheulbeuk.getPayload(event);
      DragDropNaheulbeuk.dropEffect = event.dataTransfer.dropEffect = (foundry.utils.getType(data) === "Object")
    ...
    _dropBehavior(event, data) {
      data ??= DragDropNaheulbeuk.getPayload(event);
      const allowed = this._allowedDropBehaviors(event, data);
      let behavior = DragDropNaheulbeuk.dropEffect ?? event.dataTransfer?.dropEffect;
