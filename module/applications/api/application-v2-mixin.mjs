...
/**
 * Mixin method for ApplicationV2-based Naheulbeuk applications.
 * @template {ApplicationV2} T
 * @param {typeof T} Base   Application class being extended.
 * @returns {typeof BaseApplicationNaheulbeuk}
 * @mixin
 */
export default function ApplicationV2Mixin(Base) {
  class BaseApplicationNaheulbeuk extends HandlebarsApplicationMixin(Base) {
    /** @override */
    static DEFAULT_OPTIONS = {
      actions: {
        toggleCollapsed: BaseApplicationNaheulbeuk.#toggleCollapsed
      },
      classes: ["naheulbeuk"],
      window: {
        subtitle: ""
    ...
    *_getHeaderControlContextEntries() {
    ...
    async _prepareContext(options) {
      ...
      context.CONFIG = CONFIG.NAHEULBEUK;
      context.inputs = { ...foundry.applications.fields, ...naheulbeuk.applications.fields };
      return context;
    }
    ...
    async _renderFrame(options) {
      ...
      if ( (options.window?.icon ?? "").includes(".") ) {
        ...
        const newIcon = document.createElement(options.window.icon?.endsWith(".svg") ? "naheulbeuk-icon" : "img");
    ...
    _onRender(context, options) {
      ...
      this.element.querySelectorAll("[data-context-menu]").forEach(control =>
        control.addEventListener("click", naheulbeuk.applications.ContextMenuNaheulbeuk.triggerEvent)
      );
    ...
    _disableFields() {
      const selector = `.window-content :is(${[
        "INPUT", "SELECT", "TEXTAREA", "BUTTON", "NAHEULBEUK-CHECKBOX", "COLOR-PICKER", "DOCUMENT-TAGS",
    ...
    _attachFrameListeners() {
      new ContextMenuNaheulbeuk(this.element, '.header-control[data-action="toggleControls"]', [], {
     ...
     * @this {BaseApplicationNaheulbeuk}
  ...
  return BaseApplicationNaheulbeuk;
}
