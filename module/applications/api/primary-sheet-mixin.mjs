...
import ItemSheetNaheulbeuk from "../item/item-sheet.mjs";

/**
 * @import { FilterStateNaheulbeuk, ItemListComparatorNaheulbeuk } from "../components/_types.mjs";
 * @import { SheetTabDescriptorNaheulbeuk } from "./_types.mjs";
 */

/**
 * Adds V2 sheet functionality shared between primary document sheets (Actors & Items).
 * @param {typeof DocumentSheetNaheulbeuk} Base  The base class being mixed.
 * @returns {typeof PrimarySheetNaheulbeuk}
 */
export default function PrimarySheetMixin(Base) {
  return class PrimarySheetNaheulbeuk extends DragDropApplicationMixin(Base) {
    /** @override */
    static DEFAULT_OPTIONS = {
      actions: {
        editDocument: PrimarySheetNaheulbeuk.#showDocument,
        deleteDocument: PrimarySheetNaheulbeuk.#deleteDocument,
        showDocument: PrimarySheetNaheulbeuk.#showDocument
      }
    };
    ...
    /**
     * Sheet tabs.
     * @type {SheetTabDescriptorNaheulbeuk[]}
     */
    static TABS = [];
     ...
     * @type {Record<string, ItemListComparatorNaheulbeuk>}
     */
    static SORT_MODES = {
     ...
     * @type {Record<string, FilterStateNaheulbeuk>}
     */
    _filters = {};
     ...
     * @type {PrimarySheetNaheulbeuk.MODES|null}
     * @protected
     */
    _mode = null;
    ...
    _renderModeToggle() {
      ...
      if ( this.isEditable && !toggle ) {
        ...
        toggle.dataset.tooltip = "NAHEULBEUK.SheetModeEdit";
        toggle.setAttribute("aria-label", game.i18n.localize("NAHEULBEUK.SheetModeEdit"));
      ...
      elements.innerHTML = `
        <div class="source-book">
          <button type="button" class="unbutton control-button" data-action="showConfiguration" data-config="source"
                  data-tooltip aria-label="${game.i18n.localize("NAHEULBEUK.SOURCE.Action.Configure")}">
    ...
    _renderSource() {
      ...
      elements.querySelector("span").innerText = editable
        ? (source.label || game.i18n.localize("NAHEULBEUK.SOURCE.FIELDS.source.label"))
        : source.label;
    }
    ...
    async _preparePartContext(partId, context, options) {
       ...
       * @function naheulbeuk.prepareSheetContext
       * @memberof hookEvents
       * @param {PrimarySheetNaheulbeuk} sheet  Sheet being rendered.
       ...
       */
      Hooks.callAll("naheulbeuk.prepareSheetContext", this, partId, context, options);
    ...
    _applyItemTooltips(element) {
      ...
      `;
      element.dataset.tooltipClass = "naheulbeuk naheulbeuk-tooltip item-tooltip themed theme-light";
    ...
    async _onChangeSheetMode(event) {
      ...
      const label = game.i18n.localize(`NAHEULBEUK.SheetMode${toggle.checked ? "Play" : "Edit"}`);
    ...
    static async #showDocument(event, target) {
      ...
      doc?.sheet?.render({ force: true, mode: ItemSheetNaheulbeuk.MODES[mode] });
    }
     ...
     * @param {ActiveEffectNaheulbeuk[]} effects  The effects to sort.
     * @param {string} mode               The sort mode.
     * @returns {ActiveEffectNaheulbeuk[]}
     * @protected
     */
     ...
     * @param {ItemNaheulbeuk[]} items  The items to sort.
     * @param {string} mode     The sort mode.
     * @returns {ItemNaheulbeuk[]}
     * @protected
     */
     ...
     * @param {ItemNaheulbeuk} a
     * @param {ItemNaheulbeuk} b
     * @returns {number}
     */
