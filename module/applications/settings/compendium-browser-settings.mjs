...
/**
 * @import { CompendiumSourcePackageConfigNaheulbeuk, CompendiumSourcePackGroupNaheulbeuk } from "../../data/settings/_types.mjs";
 ...
 * @extends ApplicationNaheulbeuk<CompendiumBrowserSourceConfiguration>
 */
export default class CompendiumBrowserSettingsConfig extends ApplicationNaheulbeuk {
  constructor(options) {
    super(options);
  ...
  static DEFAULT_OPTIONS = {
    ...
    tag: "form",
    window: {
      title: "NAHEULBEUK.CompendiumBrowser.Sources.Label",
  ...
  static PARTS = {
    sidebar: {
      id: "sidebar",
      template: "systems/naheulbeuk/templates/compendium/sources-sidebar.hbs"
    },
    packs: {
      id: "packs",
      template: "systems/naheulbeuk/templates/compendium/sources-packs.hbs"
    }
  };
   ...
   * @returns {CompendiumSourcePackageConfigNaheulbeuk}
   * @protected
   */
   ...
   * @param {Set<string>} sources    The packs currently selected for inclusion.
   * @returns {CompendiumSourcePackGroupNaheulbeuk}
   * @protected
   */
  _preparePackGroupContext(documentType, packs, sources) {
    ...
    return {
      ...
      entries: Array.from(packs.map(id => {
        ...
        if ( packageName === "naheulbeuk" ) {
          tag = flags?.naheulbeuk?.sourceBook?.replace("SRD ", "");
        }
        return {
          tag, title,
          id: collection,
  ...
  async _onToggleSource(target) {
    ...
    const setting = { ...game.settings.get("naheulbeuk", "packSourceConfiguration"), ...packs };
    await game.settings.set("naheulbeuk", "packSourceConfiguration", setting);
    this.render();
  }
  ...
  static collateSources() {
    ...
    const setting = game.settings.get("naheulbeuk", "packSourceConfiguration");
