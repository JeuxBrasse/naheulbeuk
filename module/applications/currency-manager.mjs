 ...
 */
export default class CurrencyManager extends ApplicationNaheulbeuk {
  ...
  static DEFAULT_OPTIONS = {
    ...
    tag: "form",
    window: {
      title: "NAHEULBEUK.CurrencyManager.Title"
    }
  };
  ...
  static PARTS = {
    ...
    convert: {
      template: "systems/naheulbeuk/templates/apps/currency-manager-convert.hbs"
    },
    transfer: {
      template: "systems/naheulbeuk/templates/apps/currency-manager-transfer.hbs"
    }
  };
   ...
   * @type {ActorNaheulbeuk|ItemNaheulbeuk}
   */
  get document() {
  ...
  _getTabs() {
    return {
      convert: {
        ...
        label: "NAHEULBEUK.CurrencyManager.Convert.Label",
      ...
      transfer: {
        ...
        label: "NAHEULBEUK.CurrencyManager.Transfer.Label",
   ...
   * See CONFIG.NAHEULBEUK.currencies for configuration.
   * @param {ActorNaheulbeuk|ItemNaheulbeuk} doc  Actor or container item to convert.
   * @returns {Promise<ActorNaheulbeuk|ItemNaheulbeuk>}
   */
  static convertCurrency(doc) {
    const currency = foundry.utils.deepClone(doc.system.currency);

    const currencies = Object.entries(CONFIG.NAHEULBEUK.currencies)
  ...
  static deductActorCurrency(actor, amount, denomination, options={}) {
    ...
    if ( remainder ) throw new Error(game.i18n.format("NAHEULBEUK.CurrencyManager.Error.InsufficientFunds", {
      denomination,
  ...
  static getActorCurrencyUpdates(actor, amount, denomination, { recursive=false, priority="low", exact=true }={}) {
    ...
    const currencies = Object.entries(CONFIG.NAHEULBEUK.currencies).map(([denom, { conversion }]) => {
      return [denom, conversion];
    }).sort(([, a], [, b]) => priority === "high" ? a - b : b - a);
    const baseConversion = CONFIG.NAHEULBEUK.currencies[denomination].conversion;
   ...
   * @param {ActorNaheulbeuk|ItemNaheulbeuk} origin       Document from which to move the currency.
