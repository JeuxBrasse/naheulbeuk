 ...
 */
export default class CreateDocumentDialog extends DialogNaheulbeuk {
  /** @override */
  ...
  static PARTS = {
    ...super.PARTS,
    content: {
      template: "systems/naheulbeuk/templates/apps/document-create.hbs"
    }
  };
  ...
  static async #handleFormSubmission(event, form, formData) {
    if ( !form.checkValidity() ) throw new Error(game.i18n.format("DOCUMENT.NAHEULBEUK.Warning.SelectType", {
      name: game.i18n.localize(documentType.metadata.label ?? `DOCUMENT.NAHEULBEUK.${documentType.documentName}`)
    }));
  ...
  static async prompt(documentType, data={}, { folders, types, ...createOptions }={}, { ok={}, ...config }={}) {
    const label = game.i18n.localize(documentType.metadata.label ?? `DOCUMENT.NAHEULBEUK.${documentType.documentName}`);
