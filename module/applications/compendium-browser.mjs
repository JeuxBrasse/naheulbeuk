 ...
 * @extends ApplicationNaheulbeuk
 * @template CompendiumBrowserConfiguration
 */
export default class CompendiumBrowser extends ApplicationNaheulbeuk {
  constructor(...args) {
    super(...args);
  ...
  static DEFAULT_OPTIONS = {
    ...
    tag: "form",
    window: {
      title: "NAHEULBEUK.CompendiumBrowser.Title",
      minimizable: true,
      resizable: true
    },
    actions: {
  ...
  static PARTS = {
    header: {
      id: "header",
      classes: ["header"],
      template: "systems/naheulbeuk/templates/compendium/browser-header.hbs"
    },
    search: {
      id: "sidebar-search",
      classes: ["filter-element"],
      container: { id: "sidebar", classes: ["sidebar", "flexcol"] },
      template: "systems/naheulbeuk/templates/compendium/browser-sidebar-search.hbs"
    },
    types: {
      id: "sidebar-types",
      container: { id: "sidebar", classes: ["sidebar", "flexcol"] },
      template: "systems/naheulbeuk/templates/compendium/browser-sidebar-types.hbs"
    },
    filters: {
      id: "sidebar-filters",
      container: { id: "sidebar", classes: ["sidebar", "flexcol"] },
      template: "systems/naheulbeuk/templates/compendium/browser-sidebar-filters.hbs",
      templates: ["systems/naheulbeuk/templates/compendium/browser-sidebar-filter-set.hbs"]
    },
    results: {
      id: "results",
      classes: ["results"],
      template: "systems/naheulbeuk/templates/compendium/browser-results.hbs",
      templates: ["systems/naheulbeuk/templates/compendium/browser-entry.hbs"],
      scrollable: [""]
    },
    footer: {
      id: "footer",
      classes: ["footer"],
      template: "systems/naheulbeuk/templates/compendium/browser-footer.hbs"
    },
    tabs: {
      id: "tabs",
      classes: ["tabs", "tabs-left"],
      template: "systems/naheulbeuk/templates/compendium/browser-tabs.hbs"
    }
  };
  ...
  /**
   * Application tabs.
   * @type {CompendiumBrowserTabDescriptorNaheulbeuk[]}
   */
  static TABS = [
    {
      tab: "classes",
      label: "TYPES.Item.classPl",
      svg: "systems/naheulbeuk/icons/svg/items/class.svg",
      documentClass: "Item",
      types: ["class"]
    },
    {
      ...
      svg: "systems/naheulbeuk/icons/svg/items/subclass.svg",
      documentClass: "Item",
      types: ["subclass"]
    },
    {
      ...
      svg: "systems/naheulbeuk/icons/svg/items/race.svg",
      documentClass: "Item",
      types: ["race"]
    },
    {
      tab: "feats",
      label: "NAHEULBEUK.CompendiumBrowser.Tabs.Feat.other",
      icon: "fas fa-star",
      documentClass: "Item",
      types: ["feat"]
    },
    {
      ...
      svg: "systems/naheulbeuk/icons/svg/items/background.svg",
      documentClass: "Item",
      types: ["background"]
    },
    {
      tab: "physical",
      label: "NAHEULBEUK.CompendiumBrowser.Tabs.Item.other",
      svg: "systems/naheulbeuk/icons/svg/backpack.svg",
      documentClass: "Item",
      types: ["physical"]
    },
    {
    ...
    {
      tab: "monsters",
      label: "NAHEULBEUK.CompendiumBrowser.Tabs.Monster.other",
      svg: "systems/naheulbeuk/icons/svg/actors/npc.svg",
      documentClass: "Actor",
      types: ["npc"]
    },
    {
      ...
      svg: "systems/naheulbeuk/icons/svg/actors/vehicle.svg",
      documentClass: "Actor",
      types: ["vehicle"]
    },
    {
  ...
  _configureRenderOptions(options) {
    ...
    if ( options.isFirstRender ) {
      const tab = this.constructor.TABS.find(t => t.tab === this.options.tab);
      if ( tab ) foundry.utils.setProperty(options, "naheulbeuk.browser.types", tab.types);
    }
  }
  ...
  async _prepareContext(options) {
    ...
    context.filterDefinitions.set("source", {
      label: "NAHEULBEUK.SOURCE.FIELDS.source.label",
      type: "set",
      config: {
        ...
        choices: foundry.utils.mergeObject(
          ...
          Object.fromEntries(Object.keys(this.options.filters?.locked?.additional?.source ?? {}).map(k => {
            return [k.slugify({ strict: true }), CONFIG.NAHEULBEUK.sourceBooks[k] ?? k];
          })), { inplace: false }
        )
      }
    });
  ...
  async _prepareFooterContext(context, options) {
    ...
    context.summary = suffix ? game.i18n.format(
      `NAHEULBEUK.CompendiumBrowser.Selection.Summary.${suffix}`, { max, min, value }
    ) : value;
    const pr = getPluralRules();
    context.invalidTooltip = game.i18n.format(`NAHEULBEUK.CompendiumBrowser.Selection.Warning.${suffix}`, {
      max, min, value,
      document: game.i18n.localize(`NAHEULBEUK.CompendiumBrowser.Selection.Warning.Document.${pr.select(max || min)}`)
    });
    return context;
  }
  ...
  async _prepareSidebarContext(partId, context, options) {
    ...
    const types = foundry.utils.getProperty(options, "naheulbeuk.browser.types") ?? [];
      ...
      context.additional = Array.from(context.filterDefinitions?.entries() ?? []).reduce((arr, [key, data]) => {
        ...
        if ( data.type === "set" ) {
          ...
          if ( Object.keys(groups).length > 1 ) Object.entries(groups).forEach(([group, choices]) => pushFilter({
            ...
            label: game.i18n.format("NAHEULBEUK.CompendiumBrowser.Filters.Grouped", {
    ...
    if ( game.user.isGM ) {
      frame.querySelector('[data-action="close"]').insertAdjacentHTML("beforebegin", `
        <button type="button" class="header-control fas fa-cog icon" data-action="configureSources"
                data-tooltip aria-label="${game.i18n.localize("NAHEULBEUK.CompendiumBrowser.Sources.Label")}"></button>
      `);
    }
    return frame;
  }
  ...
  async _renderResult(entry, documentClass) {
    ...
    const html = await foundry.applications.handlebars.renderTemplate(
      "systems/naheulbeuk/templates/compendium/browser-entry.hbs", context
    );
    ...
    `;
    element.dataset.tooltipClass = "naheulbeuk2 naheulbeuk-tooltip item-tooltip";
  ...
  async _renderSourceFilters() {
    ...
    for ( const result of this.#results ) {
      ...
      sources.push({ slug, value: CONFIG.NAHEULBEUK.sourceBooks[value] ?? value });
    }
    ...
    const filter = await foundry.applications.handlebars.renderTemplate(
      "systems/naheulbeuk/templates/compendium/browser-sidebar-filter-set.hbs",
      {
        locked,
        value: locked,
        key: "source",
  ...
  changeTab(tab, group, options={}) {
    ...
    this.render({ parts: ["results", "filters", "types"], naheulbeuk: { browser: { types } }, changedTab: true });
  }
  ...
  #adjustCheckboxStates(htmlElement) {
    for ( const groupArea of htmlElement.querySelectorAll(".type-group") ) {
      const group = groupArea.querySelector(".type-group-header naheulbeuk-checkbox");
      const children = groupArea.querySelectorAll(".wrapper naheulbeuk-checkbox");
  ...
  static async #onHandleSubmit(event, form, formData) {
    ...
    if ( (value < (min || -Infinity)) || (value > (max || Infinity)) ) {
      const suffix = this.#selectionLocalizationSuffix;
      const pr = getPluralRules();
      throw new Error(game.i18n.format(`NAHEULBEUK.CompendiumBrowser.Selection.Warning.${suffix}`, {
        max, min, value,
        document: game.i18n.localize(`NAHEULBEUK.CompendiumBrowser.Selection.Warning.Document.${pr.select(max || min)}`)
      }));
    }
     ...
     * @function naheulbeuk.compendiumBrowserSelection
     * @memberof hookEvents
     ...
     */
    Hooks.callAll("naheulbeuk.compendiumBrowserSelection", this, this.#selected);
  }
  ...
  static async #onSetFilter(event, target) {
    ...
    this.render({ parts: ["filters", "results"], naheulbeuk: { browser: { types: activeTab?.types } } });
  }
  ...
  static async #onSetType(event, target) {
    ...
    if ( target.defaultValue ) {
      ...
      for ( const child of target.closest(".type-group").querySelectorAll("naheulbeuk-checkbox[value]") ) {
  ...
  static #onToggleMode(event, target) {
    ...
    this.render({ parts: ["results", "filters", "types", "tabs"], naheulbeuk: { browser: { types } }, changedTab: true });
  }
  ...
  static async fetch(documentClass, { types=new Set(), filters=[], index=true, indexFields=new Set(), sort=true }={}) {
    ...
    let documents = game.packs
      ...
      .filter(p => (p.metadata.type === documentClass.metadata.name)
        ...
        && (!types.size || !p.metadata.flags.naheulbeuk?.types || new Set(p.metadata.flags.naheulbeuk.types).intersects(types)))
      ...
      .map(async p => await Promise.all((await p.getIndex({ fields: Array.from(indexFields) })
        ...
        .then(index => game.naheulbeuk.moduleArt.apply(index)))
        ...
        .filter(i =>
          (!types.size || (types.has(i.type)
            && (!p.metadata.flags.naheulbeuk?.types || p.metadata.flags.naheulbeuk.types.includes(i.type))))
  ...
  static injectSidebarButton(html) {
    ...
    button.innerHTML = `
      ...
      ${game.i18n.localize("NAHEULBEUK.CompendiumBrowser.Action.Open")}
    `;
