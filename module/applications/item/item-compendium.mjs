...
import ItemSheetNaheulbeuk from "./item-sheet.mjs";

/**
 * Compendium with added support for item containers.
 */
export default class ItemCompendiumNaheulbeuk extends DragDropApplicationMixin(foundry.applications.sidebar.apps.Compendium) {
  ...
  async _handleDroppedEntry(target, data) {
    ...
    if ( (behavior === "copy") || !this._entryAlreadyExists(item) ) {
      ...
      const toCreate = contents?.size
        ? await ItemSheetNaheulbeuk.createWithContents([item], { transformAll: item => item.toCompendium() })
      ...
      if ( toCreate.length ) {
        ...
        [item] = await ItemSheetNaheulbeuk.createDocuments(toCreate, { pack: this.collection.collection, keepId: true });
      }
    }
  ...
  async _onClickEntry(event) {
    ...
    if ( !item ) return;
    const mode = item.sheet?._mode ?? (this.collection.locked ? ItemSheetNaheulbeuk.MODES.PLAY : ItemSheetNaheulbeuk.MODES.EDIT);
