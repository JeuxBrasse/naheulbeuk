...
import ContextMenuNaheulbeuk from "../context-menu.mjs";
 ...
 */
export default class ItemSheetNaheulbeuk extends PrimarySheetMixin(DocumentSheetNaheulbeuk) {
  /** @override */
  static DEFAULT_OPTIONS = {
    actions: {
      addRecovery: ItemSheetNaheulbeuk.#addRecovery,
      deleteCraft: ItemSheetNaheulbeuk.#deleteCraft,
      deleteDocument: ItemSheetNaheulbeuk.#deleteDocument,
      deleteRecovery: ItemSheetNaheulbeuk.#deleteRecovery,
      editDescription: ItemSheetNaheulbeuk.#editDescription,
      modifyAdvancementChoices: ItemSheetNaheulbeuk.#modifyAdvancementChoices,
      showConfiguration: ItemSheetNaheulbeuk.#showConfiguration,
      showDocument: ItemSheetNaheulbeuk.#showDocument,
      showIcon: ItemSheetNaheulbeuk.#showIcon,
      toggleState: ItemSheetNaheulbeuk.#toggleState
    },
    classes: ["item"],
    editingDescriptionTarget: null,
    elements: {
      activities: "naheulbeuk-activities",
      effects: "naheulbeuk-effects"
    },
    form: {
      submitOnChange: true
    },
  ...
  static PARTS = {
    header: {
      template: "systems/naheulbeuk/templates/items/header.hbs"
    },
    tabs: {
      template: "systems/naheulbeuk/templates/shared/horizontal-tabs.hbs",
      templates: ["templates/generic/tab-navigation.hbs"]
    },
    activities: {
      template: "systems/naheulbeuk/templates/items/activities.hbs",
      templates: ["systems/naheulbeuk/templates/shared/activities.hbs"],
      scrollable: [""]
    },
    advancement: {
      template: "systems/naheulbeuk/templates/items/advancement.hbs",
      scrollable: [""]
    },
    description: {
      template: "systems/naheulbeuk/templates/items/description.hbs",
      scrollable: [""]
    },
    details: {
      template: "systems/naheulbeuk/templates/items/details.hbs",
      scrollable: [""]
    },
    effects: {
      template: "systems/naheulbeuk/templates/items/effects.hbs",
      scrollable: [""]
    }
  };
  ...
  /** @override */
  static TABS = [
    { tab: "description", label: "NAHEULBEUK.ITEM.SECTIONS.Description" },
    { tab: "details", label: "NAHEULBEUK.ITEM.SECTIONS.Details", condition: this.isItemIdentified.bind(this) },
    { tab: "activities", label: "NAHEULBEUK.ITEM.SECTIONS.Activities", condition: this.itemHasActivities.bind(this) },
    { tab: "effects", label: "NAHEULBEUK.ITEM.SECTIONS.Effects", condition: this.itemHasEffects.bind(this) },
    { tab: "advancement", label: "NAHEULBEUK.ITEM.SECTIONS.Advancement", condition: this.itemHasAdvancement.bind(this) }
  ];
  ...
  async _prepareContext(options) {
    ...
    context.properties = {
      ...
      options: (this.item.system.validProperties ?? []).reduce((arr, k) => {
        const { label } = CONFIG.NAHEULBEUK.itemProperties[k];
        arr.push({
          label,
          value: k,
  ...
  async _prepareDetailsContext(context, options) {
    ...
    context.coverOptions = Object.entries(CONFIG.NAHEULBEUK.cover).map(([value, label]) => ({ value, label }));
    context.unitsOptions = Object.entries(CONFIG.NAHEULBEUK.movementUnits).map(([value, { label }]) => ({ value, label }));

    // If using modern rules, do not show redundant artificer progression unless it is already selected.
    context.spellProgression = { ...CONFIG.NAHEULBEUK.spellProgression };
    context.spellProgression = Object.entries(context.spellProgression).map(([value, config]) => {
      const group = CONFIG.NAHEULBEUK.spellcasting[config.type]?.label ?? "";
      return { group, value, label: config.label };
    });
    const { progression } = this.item.system.spellcasting ?? {};
    if ( progression && !(progression in CONFIG.NAHEULBEUK.spellProgression) ) {
    ...
    context.recoveryPeriods = CONFIG.NAHEULBEUK.limitedUsePeriods.recoveryOptions;
    context.recoveryTypes = [
      { value: "recoverAll", label: "NAHEULBEUK.USES.Recovery.Type.RecoverAll" },
      { value: "loseAll", label: "NAHEULBEUK.USES.Recovery.Type.LoseAll" },
      { value: "formula", label: "NAHEULBEUK.USES.Recovery.Type.Formula" }
    ];
  ...
  async _prepareEffectsContext(context, options) {
    const effectMap = {};
    const riders = [];
    const riderIds = new Set(this.item.getFlag("naheulbeuk", "riders.effect") ?? []);
  ...
  _getAdvancement() {
    ...
    const advancement = {};
    const configMode = !this.item.parent || (this._mode === ItemSheetNaheulbeuk.MODES.EDIT);
  ...
  _getAdvancementTags(advancement) {
    ...
    if ( advancement.classRestriction === "primary" ) {
      tags.push({
        label: "NAHEULBEUK.AdvancementClassRestrictionPrimary",
        icon: "systems/naheulbeuk/icons/svg/original-class.svg"
      });
    } else if ( advancement.classRestriction === "secondary" ) {
      tags.push({
        label: "NAHEULBEUK.AdvancementClassRestrictionSecondary",
        icon: "systems/naheulbeuk/icons/svg/multiclass.svg"
      });
    }
    return tags;
  }
  ...
  async _getBaseItemOptions(context) {
    const baseIds = this.item.type === "equipment" ? {
      ...CONFIG.NAHEULBEUK.armorIds,
      ...CONFIG.NAHEULBEUK.shieldIds
    } : CONFIG.NAHEULBEUK[`${this.item.type}Ids`];
  ...
  _attachFrameListeners() {
    ...
    new ContextMenuNaheulbeuk(this.element, ".advancement-item[data-id]", [], {
      onOpen: target => naheulbeuk.documents.advancement.Advancement.onContextMenu(this.item, target), jQuery: false
    });
  }
  ...
  async _onRender(context, options) {
    ...
    if ( this._headerToggles.identified ) {
      ...
      const label = isIdentified ? "NAHEULBEUK.Identified" : "NAHEULBEUK.Unidentified.Title";
    ...
    if ( this._headerToggles.equipped ) {
      ...
      const label = isEquipped ? "NAHEULBEUK.Equipped" : "NAHEULBEUK.Unequipped";
  ...
  _addDocument() {
    if ( this.tabGroups.primary === "activities" ) {
      return naheulbeuk.documents.activity.UtilityActivity.createDialog({}, { parent: this.item });
    }

    if ( this.tabGroups.primary === "advancement" ) {
      return naheulbeuk.documents.advancement.Advancement.createDialog({}, { parent: this.item });
    }
  ...
  _onDrop(event) {
    ...
    /**
     * A hook event that fires when some useful data is dropped onto an ItemSheetNaheulbeuk.
     * @function naheulbeuk.dropItemSheetData
     * @memberof hookEvents
     * @param {ItemNaheulbeuk} item                  The ItemNaheulbeuk.
     * @param {ItemSheetNaheulbeuk} sheet    The ItemSheetNaheulbeuk application.
     * @param {object} data                  The data that has been dropped onto the sheet.
     * @returns {boolean}                    Explicitly return `false` to prevent normal drop handling.
     */
    const allowed = Hooks.call("naheulbeuk.dropItemSheetData", item, this, data);
  ...
  async _onDropAdvancement(event, data) {
    ...
    advancements = advancements.filter(a => {
      const validItemTypes = CONFIG.NAHEULBEUK.advancementTypes[a.constructor.typeName]?.validItemTypes
    ...
    if ( this.item.actor?.system.metadata?.supportsAdvancement && !game.settings.get("naheulbeuk", "disableAdvancements") ) {
   ...
   * @param {DragEvent} event  The drag event.
   * @param {ItemNaheulbeuk} item      The dropped item.
   */
   ...
   * @param {ItemNaheulbeuk} item  The Item.
   * @returns {boolean}
   */
