 ...
 * @extends {ApplicationNaheulbeuk<AdvancementManagerOptions>}
 */
export default class AdvancementManager extends ApplicationNaheulbeuk {
  /**
   * @param {ActorNaheulbeuk} actor        Actor on which this advancement is being performed.
  ...
  static DEFAULT_OPTIONS = {
    ...
    window: {
      ...
      title: "NAHEULBEUK.ADVANCEMENT.Manager.Title.Default"
    },
    actions: {
  ...
  get subtitle() {
    ...
    if ( this.step.class && ["class"].includes(item.type) ) level = this.step.class.level;
    if ( level ) parts.push(game.i18n.format("NAHEULBEUK.AdvancementLevelHeader", { level }));

    // Step Count
    const visibleSteps = this.steps.filter(s => !s.automatic);
    const visibleIndex = visibleSteps.indexOf(this.step);
    if ( visibleIndex >= 0 ) parts.push(game.i18n.format("NAHEULBEUK.ADVANCEMENT.Manager.Steps", {
   ...
   * @param {ActorNaheulbeuk} actor               Actor from which the advancement should be updated.
   ...
   * @param {ActorNaheulbeuk} actor         Actor to which the item is being added.
   ...
   * @param {ActorNaheulbeuk} actor         Actor from which the choices should be modified.
   ...
   * @param {ActorNaheulbeuk} actor         Actor from which the advancement should be unapplied.
   ...
   * @param {ActorNaheulbeuk} actor         Actor from which the item should be deleted.
   ...
   * @param {ActorNaheulbeuk} actor         Actor whose level has changed.
   ...
   * @param {ItemNaheulbeuk} item                               Item that has advancement.
   ...
   * @param {ItemNaheulbeuk} item    Item being advanced. If class or subclass, its level will be used.
   * @param {ActorNaheulbeuk} actor  Actor being advanced.
  ...
  async _prepareContext(options) {
    ...
    return {
      ...
      flowClasses: this.step.flow instanceof Application ? "naheulbeuk advancement flow" : "",
  ...
  render(forced=false, options={}) {
     ...
     * @function naheulbeuk.preAdvancementManagerRender
     * @memberof hookEvents
     * @param {AdvancementManager} advancementManager The advancement manager about to be rendered
     */
    const allowed = Hooks.call("naheulbeuk.preAdvancementManagerRender", this);
  ...
  async close(options={}) {
    if ( !options.skipConfirmation ) {
      return new foundry.applications.api.Dialog({
        window: {
          title: `${game.i18n.localize("NAHEULBEUK.ADVANCEMENT.Manager.ClosePrompt.Title")}: ${this.actor.name}`
        },
        position: { width: 400 },
        content: game.i18n.localize("NAHEULBEUK.ADVANCEMENT.Manager.ClosePrompt.Message"),
        buttons: [
          {
            action: "stop",
            icon: "fas fa-times",
            label: game.i18n.localize("NAHEULBEUK.ADVANCEMENT.Manager.ClosePrompt.Action.Stop"),
            default: true,
            callback: () => {
          ...
          },
          {
            ...
            label: game.i18n.localize("NAHEULBEUK.ADVANCEMENT.Manager.ClosePrompt.Action.Continue")
          }
        ]
      }).render({ force: true });
    }
      ...
      do {
        ...
        if ( (type === "delete") && this.step.item ) {
          if ( this.step.flow?.retainedData?.retainedItems ) {
            this.step.flow.retainedData.retainedItems[this.step.item.flags.naheulbeuk?.sourceId] = this.step.item.toObject();
          }
   ...
   * @param {ItemNaheulbeuk[]} preEmbeddedItems  Items present before the current step was applied.
   */
  ...
  async #restart(event) {
    const restart = await foundry.applications.api.Dialog.confirm({
      window: { title: game.i18n.localize("NAHEULBEUK.ADVANCEMENT.Manager.RestartPrompt.Title") },
      content: `<p>${game.i18n.localize("NAHEULBEUK.ADVANCEMENT.Manager.RestartPrompt.Message")}</p>`
    });
    if ( !restart ) return;
  ...
  async #complete(event) {
     ...
     * are applied.
     * @function naheulbeuk.preAdvancementManagerComplete
     * @memberof hookEvents
     ...
     */
    if ( Hooks.call("naheulbeuk.preAdvancementManagerComplete", this, updates, toCreate, toUpdate, toDelete) === false ) {
     ...
     * @function naheulbeuk.advancementManagerComplete
     * @memberof hookEvents
     * @param {AdvancementManager} advancementManager The advancement manager that just completed
     */
    Hooks.callAll("naheulbeuk.advancementManagerComplete", this);
    // Close prompt
    return this.close({ skipConfirmation: true });
  }
}
 ...
 */
class AdvancementVisualizer extends ApplicationNaheulbeuk {
  /** @override */
  static DEFAULT_OPTIONS = {
  ...
  static PARTS = {
    steps: {
      template: "systems/naheulbeuk/templates/advancement/advancement-visualizer.hbs"
    }
  };
