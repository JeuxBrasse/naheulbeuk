...
export default class AdvancementConfig extends PseudoDocumentSheet {
  constructor(advancement={}, options={}) {
    if ( advancement instanceof naheulbeuk.documents.advancement.Advancement ) {
  ...
  async _prepareContext(options) {
    const levels = Array.fromRange(CONFIG.NAHEULBEUK.maxLevel + 1).map(l => ({ value: l, label: l }));
    if ( ["class", "subclass"].includes(this.item.type) ) delete levels[0];
    else levels[0].label = game.i18n.localize("NAHEULBEUK.ADVANCEMENT.Config.AnyLevel");
    const context = {
      ...
      classRestrictionOptions: [
        { value: "", label: game.i18n.localize("NAHEULBEUK.AdvancementClassRestrictionNone") },
        { value: "primary", label: game.i18n.localize("NAHEULBEUK.AdvancementClassRestrictionPrimary") },
        { value: "secondary", label: game.i18n.localize("NAHEULBEUK.AdvancementClassRestrictionSecondary") }
      ],
  ...
  async _onDrop(event) {
    ...
    if ( item.uuid === this.item.uuid ) {
      ui.notifications.error("NAHEULBEUK.ADVANCEMENT.ItemGrant.Warning.Recursive", {localize: true});
      return;
    }

    // Abort if this uuid exists already
    if ( existingItems.find(i => i.uuid === item.uuid) ) {
      ui.notifications.warn("NAHEULBEUK.ADVANCEMENT.ItemGrant.Warning.Duplicate", {localize: true});
      return;
    }
   ...
   * @param {ItemNaheulbeuk} item  The materialized Item that was dropped.
