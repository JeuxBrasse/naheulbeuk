 ...
 */
export default class JournalEntrySheetNaheulbeuk extends foundry.applications.sheets.journal.JournalEntrySheet {
  /** @override */
  static DEFAULT_OPTIONS = {
    classes: ["naheulbeuk", "naheulbeuk-journal", "titlebar"]
  };
  ...
  /**
   * Append the naheulbeuk system styling classes to journal pages inside system journal sheets.
  ...
  static onRenderJournalPageSheet(page, element) {
    if ( page.document.parent?.sheet instanceof JournalEntrySheetNaheulbeuk ) {
      element.classList.add("naheulbeuk", "naheulbeuk-journal", "titlebar", "dialog-lg");
    }
  }
  ...
  static async _injectNavigation(entry, html) {
    const nav = entry.getFlag("naheulbeuk", "navigation");
    if ( !nav ) return;
    ...
    element.append(list);
    list.append(JournalEntrySheetNaheulbeuk.#makeNavigation(previous, "prev"));
    list.append(JournalEntrySheetNaheulbeuk.#makeNavigation(up, "up"));
    list.append(JournalEntrySheetNaheulbeuk.#makeNavigation(next, "next"));
  ...
  static #makeNavigation(doc, dir) {
    ...
    Object.assign(anchor.dataset, { link: "", tooltip: `NAHEULBEUK.JOURNALENTRY.Navigation.${i18n[dir]}`, uuid: doc.uuid });
