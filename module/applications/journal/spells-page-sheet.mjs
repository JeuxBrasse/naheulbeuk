...
import ItemsNaheulbeuk from "../../data/collection/items-collection.mjs";
...
export default class JournalSpellListPageSheet extends JournalEntryPageHandlebarsSheet {
  ...
  static EDIT_PARTS = {
    ...
    config: {
      ...
      template: "systems/naheulbeuk/templates/journal/spell/config.hbs"
    },
    list: {
      classes: ["right", "spell-list"],
      template: "systems/naheulbeuk/templates/journal/spell/list.hbs"
    }
  };
  ...
  static VIEW_PARTS = {
    content: {
      root: true,
      template: "systems/naheulbeuk/templates/journal/spell/view.hbs"
    }
  };
  ...
  static get GROUPING_MODES() {
    return naheulbeuk.dataModels.journal.SpellListJournalPageData.GROUPING_MODES;
  }
  ...
  _configureRenderParts(options) {
    ...
    if ( ("content" in parts) && this.options.displayAsTable ) {
      parts.content.template = "systems/naheulbeuk/templates/journal/spell/table.hbs";
    }
    return parts;
  }
    ...
    for ( const data of context.spells ) {
      ...
      switch ( context.grouping ) {
        case "level":
          ...
          section = context.sections[level] ??= { header: CONFIG.NAHEULBEUK.spellLevels[level], spells: [] };
          break;
        case "school":
          const school = spell.system.school;
          section = context.sections[school] ??= { header: CONFIG.NAHEULBEUK.spellSchools[school]?.label, spells: [] };
          break;
        case "alphabetical":
  ...
  async prepareSpells(grouping) {
    ...
    for ( const baseUuid of uuids ) {
      ...
      if ( collection && !collections.has(collection) ) {
        if ( collection instanceof ItemsNaheulbeuk ) collections.set(collection, collection);
