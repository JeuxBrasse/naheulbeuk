...
export default class JournalClassPageSheet extends JournalEntryPageHandlebarsSheet {
  ...
  static EDIT_PARTS = {
    ...
    config: {
      ...
      template: "systems/naheulbeuk/templates/journal/page-{type}-edit.hbs"
    }
  };
  ...
  static VIEW_PARTS = {
    content: {
      root: true,
      template: "systems/naheulbeuk/templates/journal/page-{type}-view.hbs"
    }
  };
  ...
  async _prepareContext(options) {
    ...
    context.styleOptions = [
      { value: "", label: game.i18n.localize("JOURNALENTRYPAGE.NAHEULBEUK.Class.Style.Inferred") },
      { rule: true },
      { value: "2024", label: game.i18n.localize("JOURNALENTRYPAGE.NAHEULBEUK.Class.Style.Modern") },
      { value: "2014", label: game.i18n.localize("JOURNALENTRYPAGE.NAHEULBEUK.Class.Style.Legacy") }
    ];
    ...
    if ( linked.system.primaryAbility ) {
      ...
      ).format(Array.from(linked.system.primaryAbility.value).map(v => CONFIG.NAHEULBEUK.abilities[v]?.label));
    }
   ...
   * @param {ItemNaheulbeuk} item                  Class item belonging to this journal.
  ...
  async _getTable(item, { initialLevel=1, modernStyle }={}) {
    ...
    const headers = [[{content: game.i18n.localize("NAHEULBEUK.Level")}]];
    if ( item.type === "class" ) headers[0].push({content: game.i18n.localize("NAHEULBEUK.ProficiencyBonus")});
    if ( hasFeatures ) headers[0].push({content: game.i18n.localize("NAHEULBEUK.Features")});
    ...
    const rows = [];
    for ( const level of Array.fromRange((CONFIG.NAHEULBEUK.maxLevel - (initialLevel - 1)), initialLevel) ) {
      let features = [];
  ...
  async _getSpellProgression(item) {
    ...
    const spellcastingModel = CONFIG.NAHEULBEUK.spellcasting[spellcasting.type];
    const table = {};
    ...
    if ( spellcastingModel?.isSingleLevel ) {
      ...
      table.headers = [[
        { content: game.i18n.localize("JOURNALENTRYPAGE.NAHEULBEUK.Class.SpellSlots") },
        { content: game.i18n.localize("JOURNALENTRYPAGE.NAHEULBEUK.Class.SpellSlotLevel") }
      ]];
      table.cols = [{class: "spellcasting", span: 2}];

      // Loop through each level, gathering "Spell Slots" & "Slot Level" for each one
      table.rows = [];
      for ( const level of Array.fromRange(CONFIG.NAHEULBEUK.maxLevel, 1) ) {
        const progression = { [spellSlotKey]: 0 };
        spellcasting.levels = level;
        ActorNaheulbeuk.computeClassProgression(progression, item, { spellcasting });
        ActorNaheulbeuk.prepareSpellcastingSlots(spells, spellcasting.type, progression);
      ...
      const maxSpellLevel = Object.keys(CONFIG.NAHEULBEUK.spellLevels).length - 1;
      ...
      let largestSlot;
      table.rows = [];
      for ( const level of Array.fromRange(CONFIG.NAHEULBEUK.maxLevel, 1).reverse() ) {
      ...
      table.headers = [
        [{content: game.i18n.localize("JOURNALENTRYPAGE.NAHEULBEUK.Class.SpellSlotsPerSpellLevel"), colSpan: largestSlot}],
     ...
     * The actual hook names include the spellcasting type (e.g. `naheulbeuk.buildPsionicSpellcastingTable`).
     * @param {object} table                          Table definition being built. *Will be mutated.*
     * @param {ItemNaheulbeuk} item                           Class for which the spellcasting table is being built.
     * @param {SpellcastingDescription} spellcasting  Spellcasting descriptive object.
     * @function naheulbeuk.buildSpellcastingTable
     * @memberof hookEvents
     */
    Hooks.callAll(`naheulbeuk.build${spellcasting.type.capitalize()}SpellcastingTable`, table, item, spellcasting);
  ...
  async _getOptionalTable(item, { modernStyle }) {
    const headers = [[
      { content: game.i18n.localize("NAHEULBEUK.Level") },
      { content: game.i18n.localize("NAHEULBEUK.Features") }
    ]];
   ...
   * @param {ItemNaheulbeuk} item                       Class or subclass item belonging to this journal.
   * @param {object} options
   * @param {boolean} options.modernStyle       Is the modern style being displayed?
  ...
  async _getFeatures(item, { modernStyle, optional=false }) {
    const prepareFeature = async (f, level) => {
      ...
      return {
        document, level,
        name: modernStyle ? game.i18n.format("JOURNALENTRYPAGE.NAHEULBEUK.Class.Features.Name", {
    ...
    if ( asi.levels.length ) {
      ...
      const name = game.i18n.localize("NAHEULBEUK.ADVANCEMENT.AbilityScoreImprovement.Journal.Name");
      features.push({
        description: game.i18n.format(
          `NAHEULBEUK.ADVANCEMENT.AbilityScoreImprovement.Journal.Description${modernStyle ? "Modern" : "Legacy"}`,
          {
            class: item.name,
            firstLevel: formatNumber(firstLevel),
            firstLevelOrdinal: formatNumber(firstLevel, { ordinal: true }),
            maxAbilityScore: formatNumber(CONFIG.NAHEULBEUK.maxAbilityScore),
    ...
    for ( const advancement of asi.boons ) {
      ...
      features.push({
        description: game.i18n.format(
          `NAHEULBEUK.ADVANCEMENT.AbilityScoreImprovement.Journal.DescriptionEpic${recommendation ? "Recommendation" : ""}`,
          { recommendation: recommendation?.toAnchor().outerHTML }
        ),
        level: advancement.level,
        name: game.i18n.format("JOURNALENTRYPAGE.NAHEULBEUK.Class.Features.Name", {
   ...
   * @param {ItemNaheulbeuk} item  Subclass item being prepared.
