...
export default class OrderUsageDialog extends ActivityUsageDialog {
  ...
  static PARTS = {
    order: {
      template: "systems/naheulbeuk/templates/activity/order-usage.hbs"
    },
    footer: {
  ...
  _prepareBuildContext(context, options) {
    context.build = {
      choices: CONFIG.NAHEULBEUK.facilities.sizes,
      field: new StringField({ nullable: false, blank: false, label: "NAHEULBEUK.FACILITY.FIELDS.size.label" }),
      name: "building.size",
  ...
  _prepareCostsContext(context, { days, gold }) {
    const { duration } = game.settings.get("naheulbeuk", "bastionConfiguration");
    context.costs = {
      days: {
        field: new NumberField({ nullable: true, integer: true, min: 0, label: "NAHEULBEUK.TimeDay" }),
        name: "costs.days",
        value: this.config.costs?.days ?? days ?? duration
      },
      gold: {
        field: new NumberField({ nullable: true, integer: true, min: 0, label: "NAHEULBEUK.CurrencyGP" }),
        name: "costs.gold",
  ...
  async _prepareCraftContext(context, options) {
    ...
    context.craft = {
      legend: game.i18n.localize(`NAHEULBEUK.FACILITY.Orders.${this.activity.order}.present`),
      item: {
    ...
    if ( this.activity.order === "harvest" ) {
      ...
      context.craft.baseItem = {
        field: new BooleanField({
          label: "NAHEULBEUK.FACILITY.Craft.BaseItem.Label",
          hint: "NAHEULBEUK.FACILITY.Craft.BaseItem.Hint"
        }),
  ...
  _prepareEnlargeContext(context, options) {
    const sizes = Object.entries(CONFIG.NAHEULBEUK.facilities.sizes).sort((a, b) => a.value - b.value);
  ...
  async _prepareFooterContext(context, options) {
    context.buttons = [{
      ...
      label: "NAHEULBEUK.FACILITY.Order.Execute"
    }];
    return context;
  }
  ...
  async _prepareOrderContext(context, options) {
    ...
    if ( this.activity.order === "build" ) {
      const { days, value: gold } = CONFIG.NAHEULBEUK.facilities.sizes.cramped;
      this._prepareBuildContext(context, options);
      this._prepareCostsContext(context, { ...options, days, gold });
      return;
    }

    let { duration } = game.settings.get("naheulbeuk", "bastionConfiguration");
    if ( (this.activity.order === "craft") || (this.activity.order === "harvest") ) {
      ...
      const config = CONFIG.NAHEULBEUK.facilities.orders[this.activity.order];
  ...
  async _prepareTradeContext(context, options) {
    ...
    if ( !trade.creatures.max && !trade.stock.max ) {
      context.trade = {
        stocked: {
          field: new BooleanField({
            label: "NAHEULBEUK.FACILITY.Trade.Stocked.Label",
            hint: "NAHEULBEUK.FACILITY.Trade.Stocked.Hint"
          }),
      ...
      context.trade = {
        sell: {
          field: new BooleanField({ label: "NAHEULBEUK.FACILITY.Trade.Sell.Label" }),
          name: "trade.sell",
          value: isSelling
        }
      };
      ...
      if ( trade.stock.max ) {
        ...
        const label = `NAHEULBEUK.FACILITY.Trade.Stock.${isSelling ? "Sell" : "Buy"}`;
        context.trade.stock = {
        ...
        context.trade.creatures = {
          buy, sell,
          hint: "NAHEULBEUK.FACILITY.Trade.Creatures.Buy",
          price: {
            field: new NumberField({
              nullable: false, min: 0, integer: true,
              label: "NAHEULBEUK.FACILITY.Trade.Price.Label",
              hint: "NAHEULBEUK.FACILITY.Trade.Price.Hint"
            }),
  ...
  _prepareBuildData(submitData) {
    if ( (this.config.building?.size ?? "cramped") !== submitData.building?.size ) {
      const { days, value: gold } = CONFIG.NAHEULBEUK.facilities.sizes[submitData.building.size];
