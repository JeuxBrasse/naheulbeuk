...
export default class TransformSheet extends ActivitySheet {
  ...
  static PARTS = {
    ...super.PARTS,
    effect: {
      template: "systems/naheulbeuk/templates/activity/transform-effect.hbs",
      templates: [
        ...super.PARTS.effect.templates,
        "systems/naheulbeuk/templates/activity/parts/transform-profiles.hbs",
        "systems/naheulbeuk/templates/activity/parts/transform-settings.hbs"
      ]
    }
  };
  ...
  async _prepareEffectContext(context, options) {
    ...
    const settings = new TransformationSetting({
      ...(context.source.transform.customize ? context.source.settings
        : CONFIG.NAHEULBEUK.transformation.presets[context.source.transform.preset]?.settings ?? {}),
    ...
    context.presetOptions = [
      { value: "", label: game.i18n.localize("NAHEULBEUK.TRANSFORM.Preset.Default") },
      { rule: true },
      ...Object.entries(CONFIG.NAHEULBEUK.transformation.presets)
        .map(([value, { label }]) => ({ value, label }))
    ];

    context.creatureSizeOptions = Object.entries(CONFIG.NAHEULBEUK.actorSizes)
      .map(([value, { label }]) => ({ value, label }));
    context.creatureTypeOptions = Object.entries(CONFIG.NAHEULBEUK.creatureTypes)
      .map(([value, { label }]) => ({ value, label }));
    context.movementTypeOptions = Object.entries(CONFIG.NAHEULBEUK.movementTypes)
      .map(([value, { label }]) => ({ value, label }));

    context.profileModes = [
      { value: "", label: game.i18n.localize("NAHEULBEUK.TRANSFORM.FIELDS.transform.mode.Direct") },
      { value: "cr", label: game.i18n.localize("NAHEULBEUK.TRANSFORM.FIELDS.transform.mode.CR") }
    ];
  ...
  async _processSubmitData(event, submitData) {
    ...
    if ( submitData.transform?.customize && !this.activity._source.settings ) {
      ...
      submitData.settings = new TransformationSetting(foundry.utils.mergeObject({
        ...(CONFIG.NAHEULBEUK.transformation.presets[preset]?.settings ?? {}),
