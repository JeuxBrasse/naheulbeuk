...
export default class SummonSheet extends ActivitySheet {
  ...
  static PARTS = {
    ...super.PARTS,
    effect: {
      template: "systems/naheulbeuk/templates/activity/summon-effect.hbs",
      templates: [
        ...super.PARTS.effect.templates,
        "systems/naheulbeuk/templates/activity/parts/summon-changes.hbs",
        "systems/naheulbeuk/templates/activity/parts/summon-profiles.hbs"
      ]
    }
  };
    ...
    context.abilityOptions = [
      {
        ...
        label: game.i18n.format("NAHEULBEUK.DefaultSpecific", {
          default: this.activity.isSpell ? game.i18n.localize("NAHEULBEUK.Spellcasting").toLowerCase()
            : CONFIG.NAHEULBEUK.abilities[this.activity.ability]?.label.toLowerCase()
              ?? game.i18n.localize("NAHEULBEUK.None").toLowerCase()
        })
      },
      ...Object.entries(CONFIG.NAHEULBEUK.abilities).map(([value, { label }]) => ({ value, label }))
    ];
    context.creatureSizeOptions = Object.entries(CONFIG.NAHEULBEUK.actorSizes).map(([value, config]) => ({
      value, label: config.label, selected: this.activity.creatureSizes.has(value)
    }));
    context.creatureTypeOptions = Object.entries(CONFIG.NAHEULBEUK.creatureTypes).map(([value, config]) => ({
      value, label: config.label, selected: this.activity.creatureTypes.has(value)
    }));

    context.profileModes = [
      { value: "", label: game.i18n.localize("NAHEULBEUK.SUMMON.FIELDS.summon.mode.Direct") },
      { value: "cr", label: game.i18n.localize("NAHEULBEUK.SUMMON.FIELDS.summon.mode.CR") }
    ];
  ...
  _getTabs() {
    ...
    tabs.effect.label = "NAHEULBEUK.SUMMON.SECTIONS.Summoning";
    ...
    tabs.effect.tabs = this._markTabs({
      profiles: {
        ...
        label: "NAHEULBEUK.SUMMON.SECTIONS.Profiles"
      },
      changes: {
        id: "changes", group: "effect", icon: "fa-solid fa-sliders",
        label: "NAHEULBEUK.SUMMON.SECTIONS.Changes"
      }
    });
    return tabs;
  }
