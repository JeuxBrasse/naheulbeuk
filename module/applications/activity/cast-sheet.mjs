...
export default class CastSheet extends ActivitySheet {
  ...
  static PARTS = {
    ...super.PARTS,
    effect: {
      template: "systems/naheulbeuk/templates/activity/cast-effect.hbs",
      templates: [
        "systems/naheulbeuk/templates/activity/parts/cast-spell.hbs",
        "systems/naheulbeuk/templates/activity/parts/cast-details.hbs"
      ]
    }
  };
  ...
  async _prepareEffectContext(context, options) {
    ...
    if ( context.spell ) {
      ...
      if ( context.spell.system.level > 0 ) context.levelOptions = Object.entries(CONFIG.NAHEULBEUK.spellLevels)
        .filter(([level]) => Number(level) >= context.spell.system.level)
        .map(([value, label]) => ({ value, label }));
    }

    context.abilityOptions = [
      { value: "", label: game.i18n.localize("NAHEULBEUK.Spellcasting") },
      { rule: true },
      ...Object.entries(CONFIG.NAHEULBEUK.abilities).map(([value, { label }]) => ({ value, label }))
    ];
    context.propertyOptions = Array.from(CONFIG.NAHEULBEUK.validProperties.spell).map(value => ({
      value, label: CONFIG.NAHEULBEUK.itemProperties[value]?.label ?? ""
    }));
  ...
  _getTabs() {
    ...
    tabs.effect.label = "NAHEULBEUK.CAST.SECTIONS.Spell";
