...
export default class ActivitySheet extends PseudoDocumentSheet {
  ...
  static PARTS = {
    ...
    identity: {
      template: "systems/naheulbeuk/templates/activity/identity.hbs",
      templates: [
        "systems/naheulbeuk/templates/activity/parts/activity-identity.hbs",
        "systems/naheulbeuk/templates/activity/parts/activity-visibility.hbs"
      ]
    },
    activation: {
      template: "systems/naheulbeuk/templates/activity/activation.hbs",
      templates: [
        "systems/naheulbeuk/templates/activity/parts/activity-time.hbs",
        "systems/naheulbeuk/templates/activity/parts/activity-targeting.hbs",
        "systems/naheulbeuk/templates/activity/parts/activity-consumption.hbs"
      ]
    },
    effect: {
      template: "systems/naheulbeuk/templates/activity/effect.hbs",
      templates: [
        "systems/naheulbeuk/templates/activity/parts/activity-effects.hbs",
        "systems/naheulbeuk/templates/activity/parts/activity-effect-level-limit.hbs",
        "systems/naheulbeuk/templates/activity/parts/activity-effect-settings.hbs"
      ]
    }
  };
  ...
  async _prepareActivationContext(context, options) {
    ...
    context.activationTypes = [
      ...Object.entries(CONFIG.NAHEULBEUK.activityActivationTypes).map(([value, config]) => ({
        value,
        label: game.i18n.localize(config.label),
        group: game.i18n.localize(config.group)
      })),
      { value: "", label: game.i18n.localize("NAHEULBEUK.NoneActionLabel") }
    ];
    context.affectsPlaceholder = game.i18n.localize(
      `NAHEULBEUK.TARGET.Count.${context.data.target?.template?.type ? "Every" : "Any"}`
    );
    context.durationUnits = [
      { value: "inst", label: game.i18n.localize("NAHEULBEUK.TimeInst") },
      ...Object.entries(CONFIG.NAHEULBEUK.scalarTimePeriods).map(([value, label]) => ({
        value, label, group: game.i18n.localize("NAHEULBEUK.DurationTime")
      })),
      ...Object.entries(CONFIG.NAHEULBEUK.permanentTimePeriods).map(([value, label]) => ({
        value, label, group: game.i18n.localize("NAHEULBEUK.DurationPermanent")
      })),
      { value: "spec", label: game.i18n.localize("NAHEULBEUK.Special") }
    ];
    context.rangeUnits = [
      ...
      ...Object.entries(CONFIG.NAHEULBEUK.movementUnits).map(([value, { label }]) => ({
        value, label, group: game.i18n.localize("NAHEULBEUK.RangeDistance")
      }))
    ];
    ...
    const consumptionTypeOptions = Array.from(this.activity.validConsumptionTypes).map(value => ({
      value,
      label: CONFIG.NAHEULBEUK.activityConsumptionTypes[value].label
    }));
    context.consumptionTargets = context.source.consumption.targets.map((data, index) => {
      const typeConfig = CONFIG.NAHEULBEUK.activityConsumptionTypes[data.type] ?? {};
      ...
      return {
        ...
        scalingModes: canScale ? [
          { value: "", label: game.i18n.localize("NAHEULBEUK.CONSUMPTION.Scaling.None") },
          { value: "amount", label: game.i18n.localize("NAHEULBEUK.CONSUMPTION.Scaling.Amount") },
        ...
        selectedTarget: ("validTargets" in typeConfig) && ["itemUses", "material"].includes(data.type)
          ...
          : data.target,
        targetPlaceholder: data.type === "itemUses" ? game.i18n.localize("NAHEULBEUK.CONSUMPTION.Target.ThisItem") : "",
    ...
    context.recoveryTypes = [
      { value: "recoverAll", label: game.i18n.localize("NAHEULBEUK.USES.Recovery.Type.RecoverAll") },
      { value: "loseAll", label: game.i18n.localize("NAHEULBEUK.USES.Recovery.Type.LoseAll") },
      { value: "formula", label: game.i18n.localize("NAHEULBEUK.USES.Recovery.Type.Formula") }
    ];
  ...
  async _prepareEffectContext(context, options) {
    ...
    if ( context.activity.effects ) {
      ...
      context.appliedEffects = context.activity.effects.reduce((arr, data) => {
        ...
        const effect = {
          ...
          additionalSettings: "systems/naheulbeuk/templates/activity/parts/activity-effect-settings.hbs"
        };
    ...
    context.denominationOptions = [
      { value: "", label: "" },
      ...CONFIG.NAHEULBEUK.dieSteps.map(value => ({ value, label: `d${value}` }))
    ];
    if ( context.activity.damage?.parts ) {
      ...
      const typeOptions = Object.entries(CONFIG.NAHEULBEUK.damageTypes).map(([value, { label }]) => ({ value, label }));
  ...
  static #addRecovery(event, target) {
    const periods = new Set(
      Object.entries(CONFIG.NAHEULBEUK.limitedUsePeriods).filter(([, config]) => !config.deprecated).map(([k]) => k)
    );
