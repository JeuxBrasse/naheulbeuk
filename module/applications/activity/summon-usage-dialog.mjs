...
export default class SummonUsageDialog extends ActivityUsageDialog {
  ...
  static PARTS = {
    ...super.PARTS,
    creation: {
      template: "systems/naheulbeuk/templates/activity/summon-usage-creation.hbs"
    }
  };
  ...
  async _prepareCreationContext(context, options) {
    ...
    if ( this._shouldDisplay("create.summons") && (profiles.length || (this.activity.creatureSizes.size > 1)
      ...
      if ( !foundry.utils.hasProperty(this.options.display, "create.summons") ) context.summonsFields.push({
        field: new BooleanField({ label: game.i18n.localize("NAHEULBEUK.SUMMON.Action.Place") }),
        name: "create.summons",
        ...
        if ( profiles.length > 1 ) {
          ...
          context.summonsFields.push({
            field: new StringField({
              required: true, blank: false, label: game.i18n.localize("NAHEULBEUK.SUMMON.Profile.Label")
            }),
    ...
    if ( profile.name ) label = profile.name;
      ...
      switch ( this.activity.summon.mode ) {
        case "cr":
          ...
          label = game.i18n.format("NAHEULBEUK.SUMMON.Profile.ChallengeRatingLabel", { cr: formatCR(cr) });
          break;
        default:
