 ...
 * @extends {DialogNaheulbeuk<ApplicationConfiguration & BasicRollConfigurationDialogOptions>}
 *
 ...
 */
export default class RollConfigurationDialog extends DialogNaheulbeuk {
  ...
  static DEFAULT_OPTIONS = {
    ...
    window: {
      title: "NAHEULBEUK.RollConfiguration.Title",
  ...
  static PARTS = {
    formulas: {
      template: "systems/naheulbeuk/templates/dice/roll-formulas.hbs"
    },
    configuration: {
      template: "systems/naheulbeuk/templates/dice/roll-configuration.hbs"
    },
    buttons: {
      template: "systems/naheulbeuk/templates/dice/roll-buttons.hbs"
    }
  };
  ...
  _identifyDiceTerms() {
    ...
    const identifyTerm = term => {
      ...
      for ( let i = 0; i < term.number; i++ ) dice.push({
        icon: `systems/naheulbeuk/icons/svg/dice/${term.denomination}.svg`,
  ...
  async _prepareButtonsContext(context, options) {
    context.buttons = {
      roll: {
        ...
        label: game.i18n.localize("NAHEULBEUK.Roll")
      }
    };
    return context;
  }
  ...
  async _prepareConfigurationContext(context, options) {
    context.fields = [{
      field: new foundry.data.fields.StringField({
        label: game.i18n.localize("NAHEULBEUK.RollMode"), blank: false, required: true
      }),
      name: "rollMode",
  ...
  #buildConfig(config, formData, index) {
     ...
     * on the rolling method (e.g. `naheulbeuk.buildSkillRollConfig`, `naheulbeuk.buildAbilityCheckRollConfig`,
     * `naheulbeuk.buildRollConfig`).
     * @function naheulbeuk.buildRollConfig
     * @memberof hookEvents
    ...
    for ( const hookName of this.#config.hookNames ?? [""] ) {
      Hooks.callAll(`naheulbeuk.build${hookName.capitalize()}RollConfig`, this, config, formData, index);
    }
     ...
     * depending on the rolling method (e.g. `naheulbeuk.postBuildSkillRollConfig`, `naheulbeuk.postBuildAbilityCheckRollConfig`,
     * `naheulbeuk.postBuildRollConfig`).
     * @function naheulbeuk.postBuildRollConfig
     * @memberof hookEvents
     ...
     */
    for ( const hookName of this.#config.hookNames ?? [""] ) {
      Hooks.callAll(`naheulbeuk.postBuild${hookName.capitalize()}RollConfig`, this.config, config, index, {
        app: this, formData
      });
    }
  ...
  /** @override */
  _onClose(options={}) {
    if ( !options.naheulbeuk?.submitted ) this.#rolls = [];
  }
