 ...
 */
export default class Award extends ApplicationNaheulbeuk {
  ...
  static PARTS = {
    award: {
      template: "systems/naheulbeuk/templates/apps/award.hbs"
    }
  };
  ...
  async _prepareContext(options) {
    ...
    context.currency = Object.entries(CONFIG.NAHEULBEUK.currencies).reduce((obj, [k, { label, icon }]) => {
      obj[k] = {
        label, icon,
    ...
    context.hideXP = game.settings.get("naheulbeuk", "levelingMode") === "noxp";
  ...
  static prepareDestinations(destinations, savedDestinations) {
    const icons = {
      container: '<naheulbeuk-icon class="fa-fw" src="systems/naheulbeuk/icons/svg/backpack.svg"></naheulbeuk-icon>',
   ...
   * @param {ActorNaheulbeuk[]} destinations  Selected destinations to save.
   * @protected
   */
  _saveDestinations(destinations) {
    const target = this.isPartyAward ? this.origin : game.user;
    target.setFlag("naheulbeuk", "awardDestinations", destinations);
  }
   ...
   * @param {(ActorNaheulbeuk|ItemNaheulbeuk)[]} destinations  Documents that should receive the currency.
   * @param {object} [config={}]
   * @param {boolean} [config.each=false]      Award the specified amount to each player, rather than splitting it.
   * @param {ActorNaheulbeuk|ItemNaheulbeuk} [config.origin]   Document from which to move the currency, if not a freeform award.
   * @param {Map<ActorNaheulbeuk|ItemNaheulbeuk, object>} [config.results]  Results of the award operation.
   */
   ...
   * @param {ActorNaheulbeuk[]} destinations   Actors that should receive the XP.
   * @param {object} [config={}]
   * @param {boolean} [config.each=false]      Award the specified amount to each player, rather than splitting it.
   * @param {ActorNaheulbeuk} [config.origin]  Group actor from which to transfer the XP.
   * @param {Map<ActorNaheulbeuk|ItemNaheulbeuk, object>} [config.results]  Results of the award operation.
   */
   ...
   * @param {Map<ActorNaheulbeuk|ItemNaheulbeuk, object>} results  Results of any award operations.
   */
  static async displayAwardMessages(results) {
    ...
    for ( const [destination, result] of results ) {
      ...
      for ( const [key, amount] of Object.entries(result.currency ?? {}) ) {
        const label = CONFIG.NAHEULBEUK.currencies[key].label;
        entries.push(`
      ...
      if ( result.xp ) entries.push(`
        <span class="award-entry">
          ${formatNumber(result.xp)} ${game.i18n.localize("NAHEULBEUK.ExperiencePoints.Abbreviation")}
        </span>
      `);
      if ( !entries.length ) continue;

      const content = game.i18n.format("NAHEULBEUK.Award.Message", {
        name: destination.name, award: `<span class="naheulbeuk">${game.i18n.getListFormatter().format(entries)}</span>`
      });
  ...
  static async handleAward(message) {
    if ( !game.user.isGM ) {
      ui.notifications.error("NAHEULBEUK.Award.NotGMError", { localize: true });
      return;
    }
    ...
    try {
      ...
      // Otherwise show the UI with defaults
        ...
        const savedDestinations = game.user.getFlag("naheulbeuk", "awardDestinations");
  ...
  static parseAwardCommand(message) {
    ...
    for ( const part of command.split(" ") ) {
      ...
      try {
        new Roll(amount);
        if ( label in CONFIG.NAHEULBEUK.currencies ) currency[label] = amount;
    ...
    if ( unrecognized.length ) throw new Error(game.i18n.format("NAHEULBEUK.Award.UnrecognizedWarning", {
