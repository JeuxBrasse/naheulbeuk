import CalendarDataNaheulbeuk from "../../data/calendar/calendar-data.mjs";
...
export default class CalendarHUD extends BaseCalendarHUD {
  ...
  static PARTS = {
    startButtons: {
      ...
      template: "systems/naheulbeuk/templates/apps/calendar-buttons.hbs"
    },
    core: {
      classes: ["calendar-core"],
      template: "systems/naheulbeuk/templates/apps/calendar-core.hbs"
    },
    endButtons: {
      classes: ["calendar-buttons"],
      template: "systems/naheulbeuk/templates/apps/calendar-buttons.hbs"
    ...
    return [
      {
        ...
        position: "start",
        tooltip: game.i18n.format("NAHEULBEUK.CALENDAR.Action.ReverseTime", { amount: defaultAmount }),
        visible: game.user.isGM,
        additional: CalendarHUD.TIME_CONTROL_VALUES.map(({ value, unit }) => ({
          action: "reverse",
          dataset: { value, unit },
          label: `-${formatTime(value, unit, { unitDisplay: "narrow" })}`,
          tooltip: game.i18n.format("NAHEULBEUK.CALENDAR.Action.ReverseTime", {
      ...
      },
      {
        ...
        position: "start",
        tooltip: game.i18n.localize("NAHEULBEUK.CALENDAR.Action.SetDate"),
        visible: game.user.isGM
      },
      {
        ...
        position: "start",
        tooltip: game.i18n.localize("NAHEULBEUK.CALENDAR.Action.OpenCharacterSheet"),
      ...
      },
      {
        ...
        position: "end",
        tooltip: game.i18n.format("NAHEULBEUK.CALENDAR.Action.AdvanceTime", { amount: defaultAmount }),
        visible: game.user.isGM,
        additional: CalendarHUD.TIME_CONTROL_VALUES.map(({ value, unit }) => ({
          action: "advance",
          dataset: { value, unit },
          label: `+${formatTime(value, unit, { unitDisplay: "narrow" })}`,
          tooltip: game.i18n.format("NAHEULBEUK.CALENDAR.Action.AdvanceTime", {
            amount: formatTime(value, unit).titleCase()
          })
        }))
      },
      {
        ...
        position: "end",
        tooltip: game.i18n.localize("NAHEULBEUK.CALENDAR.Action.OpenPartySheet"),
  ...
  async _prepareButtonsContext(context, options) {
     ...
     * @function naheulbeuk.prepareCalendarButtons
     * @memberof hookEvents
    ...
    const controls = this._doEvent(this._getCalendarButtons, {
      ...
      hookName: "naheulbeuk.prepareCalendarButtons",
      hookResponse: true,
  ...
  async renderCore(deltas={}) {
    const prefs = game.settings.get("naheulbeuk", "calendarPreferences");
    const dateFormatter = CONFIG.NAHEULBEUK.calendar.formatters.find(f => f.value === prefs.formatters.date);
    ...
    const timeFormatter = CONFIG.NAHEULBEUK.calendar.formatters.find(f => f.value === prefs.formatters.time);
    ...
    const adjustProgress = (variable, type, delta=0) => {
      ...
      const targetProgress = type in game.time.calendar ? game.time.calendar[type]()
        : CalendarDataNaheulbeuk.prototype[type].call(game.time.calendar);
  ...
  static onUpdateWorldTime(worldTime, deltaTime, options, userId) {
    if ( this.shouldDisplay ) naheulbeuk.ui.calendar?.renderCore(options.naheulbeuk?.deltas);
  }
}
