...
export async function migrateWorld({ bypassVersionCheck=false }={}) {
  ...
  const progress = ui.notifications.info("MIGRATION.NaheulbeukBegin", {
  ...
  const logError = (err, type, name) => {
    err.message = `Failed Naheulbeuk system migration for ${type} ${name}: ${err.message}`;
    console.error(err);
    hasErrors = true;
  };
  ...
  game.settings.set("naheulbeuk", "systemMigrationVersion", game.system.version);
  progress.element?.classList.add(hasErrors ? "warning" : "success");
  progress.update({ message: "MIGRATION.NaheulbeukComplete", format: { version }, pct: 1 });
}
  ...
  try {
    ...
    for ( let doc of documents ) {
      ...
      // Handle migration failures
        ...
        err.message = `Failed naheulbeuk system migration for document ${doc.name} in pack ${pack.collection}: ${err.message}`;
        console.error(err);
        if ( strict ) throw err;
      }
...
export async function refreshCompendium(pack, { bypassVersionCheck, migrate=true }={}) {
  ...
  if ( migrate ) {
    try {
      ...
      err.message = `Failed naheulbeuk system migration pack ${pack.collection}: ${err.message}`;
      console.error(err);
      return;
    }
  }
...
export async function migrateArmorClass(pack) {
  ...
  const updates = [];
  const armor = new Set(Object.keys(CONFIG.NAHEULBEUK.armorTypes));
...
export async function migrateSettings() {
  ...
  const disableExperienceTracking = game.settings.storage.get("world")
    ?.find(s => s.key === "naheulbeuk.disableExperienceTracking")?.value;
  const levelingMode = game.settings.storage.get("world")?.find(s => s.key === "naheulbeuk.levelingMode")?.value;
  if ( (disableExperienceTracking !== undefined) && (levelingMode === undefined) ) {
    await game.settings.set("naheulbeuk", "levelingMode", "noxp");
  }
  ...
  const disableMovementAutomation = game.settings.storage.get("world")
    ?.find(s => s.key === "naheulbeuk.disableMovementAutomation")?.value;
  const movementAutomation = game.settings.storage.get("world")
    ?.find(s => s.key === "naheulbeuk.movementAutomation")?.value;
  if ( (disableMovementAutomation !== undefined) && (movementAutomation === undefined) ) {
    await game.settings.set("naheulbeuk", "movementAutomation", disableMovementAutomation ? "none" : "full");
  }
}
 ...
 * @param {ActorNaheulbeuk} actor               Full actor instance.
 ...
 *
 * @param {ItemNaheulbeuk} item             Full item instance.
...
export function migrateItemData(item, itemData, migrationData, flags={}) {
  ...
  // Migrate embedded effects
  if ( itemData.effects ) {
    const riders = foundry.utils.getProperty(itemData, "flags.naheulbeuk.riders.effect");
    if ( riders?.length ) updateData["flags.naheulbeuk.riders.effect"] = riders;
    const effects = migrateEffects(itemData, migrationData, updateData, flags);
    if ( riders?.length === updateData["flags.naheulbeuk.riders.effect"]?.length ) {
      delete updateData["flags.naheulbeuk.riders.effect"];
    }
    if ( effects.length > 0 ) updateData.effects = effects;
  }
  ...
  // Migrate properties
  const migratedProperties = foundry.utils.getProperty(itemData, "flags.naheulbeuk.migratedProperties");
  if ( migratedProperties?.length ) {
    ...
    updateData["flags.naheulbeuk.-=migratedProperties"] = null;
  }

  if ( foundry.utils.getProperty(itemData, "flags.naheulbeuk.persistSourceMigration") ) {
    flags.persistSourceMigration = true;
    updateData["flags.naheulbeuk.-=persistSourceMigration"] = null;
  }
...
export function migrateEffects(parent, migrationData, itemUpdateData, flags={}) {
  ...
  return parent.effects.reduce((arr, e) => {
    ...
    if ( effectData.flags?.naheulbeuk?.rider ) {
      itemUpdateData["flags.naheulbeuk.riders.effect"] ??= [];
      itemUpdateData["flags.naheulbeuk.riders.effect"].push(effectData._id);
      effectUpdate["flags.naheulbeuk.-=rider"] = null;
    }
    if ( effectData.flags?.naheulbeuk?.persistSourceMigration ) {
      flags.persistSourceMigration = true;
      effectUpdate["flags.naheulbeuk.-=persistSourceMigration"] = null;
    }
...
export const getMigrationData = async function() {
  const data = {};
  try {
    const icons = await fetch("systems/naheulbeuk/json/icon-migration.json");
    const spellIcons = await fetch("systems/naheulbeuk/json/spell-icon-migration.json");
...
function _migrateActorFlags(actorData, updateData) {
  const initiativeAdv = foundry.utils.getProperty(actorData, "flags.naheulbeuk.initiativeAdv");
  if ( initiativeAdv ) {
    const key = "system.attributes.init.roll.mode";
    updateData[key] = Math.min(1, (foundry.utils.getProperty(actorData, key) ?? 0) + 1);
    updateData["flags.naheulbeuk.-=initiativeAdv"] = null;
  }
  return updateData;
}
...
function _migrateTokenImage(actorData, updateData) {
  const oldSystemPNG = /^systems\/naheulbeuk\/tokens\/([a-z]+)\/([A-z]+).png$/;
  for ( const path of ["texture.src", "prototypeToken.texture.src"] ) {
    ...
    if ( oldSystemPNG.test(v) ) {
      ...
      updateData[path] = `systems/naheulbeuk/tokens/${type}/${fileName}.webp`;
    }
  }
  return updateData;
}
...
function _migrateItemUses(item, itemData, updateData, flags) {
  const value = foundry.utils.getProperty(itemData, "flags.naheulbeuk.migratedUses");
  ...
  if ( value !== undefined ) updateData["flags.naheulbeuk.-=migratedUses"] = null;
}
...
/**
 * Migrate macros from the old 'naheulbeuk.rollItemMacro' and 'naheulbeuk.macros' commands to the new location.
 * @param {object} macro       Macro data to migrate.
 * @param {object} updateData  Existing update to expand upon.
 * @returns {object}           The updateData to apply.
 */
function _migrateMacroCommands(macro, updateData) {
  if ( macro.command.includes("game.naheulbeuk.rollItemMacro") ) {
    updateData.command = macro.command.replaceAll("game.naheulbeuk.rollItemMacro", "naheulbeuk.documents.macro.rollItem");
  } else if ( macro.command.includes("game.naheulbeuk.macros.") ) {
    updateData.command = macro.command.replaceAll("game.naheulbeuk.macros.", "naheulbeuk.documents.macro.");
  }
  return updateData;
}
...
export async function purgeFlags(pack) {
  const cleanFlags = flags => {
    const flagsNaheulbeuk = flags.naheulbeuk || null;
    return flagsNaheulbeuk ? {naheulbeuk: flagsNaheulbeuk} : {};
  };
  await pack.configure({locked: false});
