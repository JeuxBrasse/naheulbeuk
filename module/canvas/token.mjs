 ...
 */
export default class TokenNaheulbeuk extends foundry.canvas.placeables.Token {

  /**
   * Update the token ring when this token is targeted.
   * @param {UserNaheulbeuk} user         The user whose targeting has changed.
   * @param {TokenNaheulbeuk} token       The token that was targeted.
  ...
  findMovementPath(waypoints, options) {
    ...
    if ( (game.settings.get("naheulbeuk", "movementAutomation") !== "full") || !this.document.actor?.system.isCreature
      || this.document.actor.statuses.intersects(CONFIG.NAHEULBEUK.neverBlockStatuses) ) {
  ...
  _getMovementCostFunction(options) {
    ...
    if ( game.settings.get("naheulbeuk", "movementAutomation") === "none" ) return costFunction;
  ...
  constrainMovementPath(waypoints, options) {
    ...
    ignoreTokens ||= game.settings.get("naheulbeuk", "movementAutomation") !== "full";
    ignoreTokens ||= !this.actor?.system.isCreature;
    ignoreTokens ||= this.actor?.statuses?.intersects(CONFIG.NAHEULBEUK.neverBlockStatuses);
  ...
  _drawHPBar(number, bar, data) {
    ...
    const hpColor = naheulbeuk.documents.ActorNaheulbeuk.getHPColor(value, effectiveMax);

    // Determine colors to use
    const blk = 0x000000;
    const c = CONFIG.NAHEULBEUK.tokenHPColors;
