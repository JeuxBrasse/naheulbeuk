/**
 * A helper class for building MeasuredTemplates for Naheulbeuk spells and abilities
 */
export default class AbilityTemplate extends foundry.canvas.placeables.MeasuredTemplate {
  ...
  static fromActivity(activity, options={}) {
    ...
    const templateShape = naheulbeuk.config.areaTargetTypes[target.type]?.template;
    ...
    switch ( templateShape ) {
      case "cone":
        ...
        break;
      case "rect": // Naheulbeuk rectangular AoEs are always cubes
        templateData.width = target.size;
        if ( game.settings.get("naheulbeuk", "gridAlignedSquareTemplates") ) {
        ...
        break;
      case "ray": // Naheulbeuk rays are most commonly 1 square (5 ft) in width
     ...
     * @function naheulbeuk.preCreateActivityTemplate
     * @memberof hookEvents
     ...
     */
    if ( Hooks.call("naheulbeuk.preCreateActivityTemplate", activity, templateData) === false ) return null;
     ...
     * @function naheulbeuk.createActivityTemplate
     * @memberof hookEvents
     * @param {Activity} activity            Activity for which the template is being placed.
     * @param {AbilityTemplate[]} templates  The templates being placed.
     */
    Hooks.callAll("naheulbeuk.createActivityTemplate", activity, created);
  ...
  _onMovePlacement(event) {
    ...
    const baseDistance = this.document.flags.naheulbeuk?.dimensions?.size;
    if ( this.document.flags.naheulbeuk?.dimensions?.adjustedSize && baseDistance ) {
