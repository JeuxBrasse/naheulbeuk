export default class TokenLayerNaheulbeuk extends foundry.canvas.layers.TokenLayer {
  /**
   ...
   * @param {TokenNaheulbeuk} token                     The token being moved
  ...
  isOccupiedGridSpaceBlocking(gridSpace, token, { preview=false }={}) {
    const tokenSize = CONFIG.NAHEULBEUK.actorSizes[token.actor?.system.traits.size]?.numerical ?? 2;
    const modernRules = game.settings.get("naheulbeuk", "rulesVersion") === "modern";
    const halflingNimbleness = token.actor?.getFlag("naheulbeuk", "halflingNimbleness");
    const found = this.#getRelevantOccupyingTokens(gridSpace, token, { preview }).filter(t => {
      ...
      if ( t.actor.statuses.intersects(CONFIG.NAHEULBEUK.neverBlockStatuses) ) return false;

      const size = t.actor.system.details?.type?.swarm || t.actor.system.traits?.size;
      const occupiedSize = CONFIG.NAHEULBEUK.actorSizes[size]?.numerical ?? 2;
     ...
     * @function naheulbeuk.determineOccupiedGridSpaceBlocking
     * @memberof hookEvents
     ...
     * @param {Set<TokenNaheulbeuk>} found      The found set of tokens which would block movement. *Will be mutated.*
     */
    Hooks.callAll("naheulbeuk.determineOccupiedGridSpaceBlocking", gridSpace, token, { preview }, found);
    return found.size > 0;
  }
  ...
  isOccupiedGridSpaceDifficult(gridSpace, token, { preview=false }={}) {
    ...
    const found = this.#getRelevantOccupyingTokens(gridSpace, token, { preview }).filter(t => {
      ...
      const occupiedSize = CONFIG.NAHEULBEUK.actorSizes[t.actor?.system.traits.size]?.numerical ?? 2;
     ...
     * @function naheulbeuk.determineOccupiedGridSpaceDifficult
     * @memberof hookEvents
     * @param {GridOffset3D} gridSpace  The grid space being checked.
     * @param {TokenNaheulbeuk} token           The token being moved.
     * @param {object} options          Additional options.
     * @param {boolean} options.preview Whether the movement in question is previewed.
     * @param {Set<TokenNaheulbeuk>} found      The found set of tokens which would cause difficult terrain. *Will be mutated.*
     */
    Hooks.callAll("naheulbeuk.determineOccupiedGridSpaceDifficult", gridSpace, token, { preview }, found);
    return found.size > 0;
  }
   ...
   * @param {object} [options]                  Additional options
   * @param {boolean} [options.preview=false]   Whether the movement in question is previewed
   * @returns {Set<TokenNaheulbeuk>} The set of potentially relevant tokens occupying the provided grid space
   */
