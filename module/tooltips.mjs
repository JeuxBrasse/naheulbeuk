 ...
 */
export default class TooltipsNaheulbeuk {
  /* -------------------------------------------- */
  /*  Properties & Getters                        */
  /* -------------------------------------------- */
  ...
  async _onTooltipActivate() {
    ...
    if ( loading?.dataset.uuid ) {
      ...
      if ( doc instanceof naheulbeuk.documents.ActorNaheulbeuk ) return this._onHoverActor(doc);
      return this._onHoverContentLink(doc);
    }
  ...
  async _onHoverPassiveLanguage(language) {
    const label = CONFIG.NAHEULBEUK.enrichmentLookup.languages[language];
  ...
  async _onHoverPassiveCheck(skill, ability, dc) {
    const skillConfig = CONFIG.NAHEULBEUK.skills[skill];
    const abilityConfig = CONFIG.NAHEULBEUK.abilities[ability ?? skillConfig.ability];

    let label;
    if ( skillConfig ) {
      label = game.i18n.format("NAHEULBEUK.SkillPassiveSpecificHint", { skill: skillConfig.label, ability: abilityConfig.label });
    } else {
      ...
      label = game.i18n.format("NAHEULBEUK.SkillPassiveHint", { skill: abilityConfig.label });
    }
   ...
   * @param {ActorNaheulbeuk} actor                The actor instance.
  ...
  async _onHoverPassive({ label }, memberCallback) {
    ...
    this.tooltip.classList.add("naheulbeuk-tooltip", "passive-tooltip", "naheulbeuk", "themed", "theme-light");
    this.tooltip.classList.remove("theme-dark");
    this.tooltip.innerHTML = await foundry.applications.handlebars.renderTemplate(
      "systems/naheulbeuk/templates/journal/passive-tooltip.hbs", context
    );
