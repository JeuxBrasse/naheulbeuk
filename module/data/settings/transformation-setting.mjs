...
export default class TransformationSetting extends foundry.abstract.DataModel {
  ...
  /** @override */
  static LOCALIZATION_PREFIXES = ["NAHEULBEUK.TRANSFORM.Setting"];
  ...
  static #initial(category) {
    return Object.entries(CONFIG.NAHEULBEUK.transformation[category])
   ...
   * @param {ActorNaheulbeuk} [options.host]   Actor being transformed. Should only be provided if using the dialog.
  ...
  createFormCategories({ host, prefix="" }={}) {
    const disabledFields = TransformationSetting.BOOLEAN_CATEGORIES.reduce((fields, cat) => {
      for ( const value of this[cat] ) {
        for ( const disable of CONFIG.NAHEULBEUK.transformation[cat][value]?.disables ?? [] ) {
          if ( disable.includes("*") ) Object.keys(CONFIG.NAHEULBEUK.transformation[disable.replace(".*", "")] ?? {})
    ...
    return TransformationSetting.BOOLEAN_CATEGORIES.map(cat => ({
      category: cat,
      title: `NAHEULBEUK.TRANSFORM.Setting.FIELDS.${cat}.label`,
      hint: game.i18n.has(`NAHEULBEUK.TRANSFORM.Setting.FIELDS.${cat}.hint`)
        ? `NAHEULBEUK.TRANSFORM.Setting.FIELDS.${cat}.hint` : "",
      settings: [
        ...Object.entries(CONFIG.NAHEULBEUK.transformation[cat]).map(([name, config]) => ({
   ...
   * @param {object} options
   * @param {ActorNaheulbeuk} [options.host]   Actor being transformed.
   * @param {string} [options.prefix]  Prefix before the field name.
   * @returns {object}
   */
  createFormField(name, field, { prefix="", host }) {
    ...
    if ( name === "spellLists" ) descriptor.options = naheulbeuk.registry.spellLists.options.filter(o => {
      if ( !host ) return true;
