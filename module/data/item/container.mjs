...
export default class ContainerData extends ItemDataModel.mixin(
  ...
  /** @override */
  static LOCALIZATION_PREFIXES = ["NAHEULBEUK.CONTAINER", "NAHEULBEUK.SOURCE"];
   ...
   * @type {Collection<ItemNaheulbeuk>|Promise<Collection<ItemNaheulbeuk>>}
   */
  get contents() {
   ...
   * @returns {Promise<Collection<ItemNaheulbeuk>>}
   * @private
   */
   ...
   * @returns {ItemNaheulbeuk|Promise<ItemNaheulbeuk>}  Item if found.
   */
  getContainedItem(id) {
  ...
  /**
   * @typedef {object} ItemNaheulbeukCapacityDescriptor
   ...
   * @returns {Promise<ItemNaheulbeukCapacityDescriptor>}
   */
  async computeCapacity() {
    ...
    if ( this.capacity.count ) {
      ...
      context.units = game.i18n.localize("NAHEULBEUK.Items");
    } else if ( this.capacity.weight.value ) {
      context.value = await this.contentsWeight;
      context.max = this.capacity.weight.value;
      context.units = CONFIG.NAHEULBEUK.weightUnits[this.capacity.weight.units]?.label ?? "";
    }
  ...
  static _migrateWeightlessData(source) {
    if ( foundry.utils.getProperty(source, "system.capacity.weightless") === true ) {
      foundry.utils.setProperty(source, "flags.naheulbeuk.migratedProperties", ["weightlessContents"]);
    }
  }
  ...
  async getSheetData(context) {
    ...
    context.parts = ["naheulbeuk.details-container"];
  }
