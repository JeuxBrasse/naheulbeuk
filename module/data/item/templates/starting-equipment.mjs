...
export default class StartingEquipmentTemplate extends SystemDataModel {
  ...
  static defineSchema() {
    return {
      ...
      wealth: new FormulaField({ label: "NAHEULBEUK.StartingEquipment.Wealth.Label",
        hint: "NAHEULBEUK.StartingEquipment.Wealth.Hint" })
    };
  }
  ...
  getStartingEquipmentDescription({ modernStyle }={}) {
    ...
    modernStyle ??= (this.source.rules === "2024")
      || (!this.source.rules && (game.settings.get("naheulbeuk", "rulesVersion") === "modern"));
    if ( modernStyle ) {
      ...
      if ( entries.length > 1 ) {
        ...
        return `<p>${game.i18n.format("NAHEULBEUK.StartingEquipment.ChooseList", {
...
export class EquipmentEntryData extends foundry.abstract.DataModel {
  ...
  static GROUPING_TYPES = {
    OR: "NAHEULBEUK.StartingEquipment.Operator.OR",
    AND: "NAHEULBEUK.StartingEquipment.Operator.AND"
  };
  ...
  static OPTION_TYPES = {
    // Category types
    armor: "NAHEULBEUK.StartingEquipment.Choice.Armor",
    tool: "NAHEULBEUK.StartingEquipment.Choice.Tool",
    weapon: "NAHEULBEUK.StartingEquipment.Choice.Weapon",
    focus: "NAHEULBEUK.StartingEquipment.Choice.Focus",

    // Currency
    currency: "NAHEULBEUK.StartingEquipment.Currency",

    // Generic item type
    linked: "NAHEULBEUK.StartingEquipment.SpecificItem"
  };
  ...
  /**
   * Where in `CONFIG.NAHEULBEUK` to find the type category labels.
   * @enum {{ label: string, config: string }}
   */
  static CATEGORIES = {
    armor: {
      label: "NAHEULBEUK.Armor",
      config: "armorTypes"
    },
    currency: {
      config: "currencies"
  ...
  static defineSchema() {
    return {
      ...
      requiresProficiency: new BooleanField({ label: "NAHEULBEUK.StartingEquipment.Proficient.Label" })
    };
  }
  ...
  get categoryLabel() {
    ...
    if ( this.type === "weapon" ) label = game.i18n.format("NAHEULBEUK.WeaponCategory", { category: label });
   ...
   */
  get keyOptions() {
    const config = foundry.utils.deepClone(CONFIG.NAHEULBEUK[this.constructor.CATEGORIES[this.type]?.config]);
    if ( this.type === "weapon" ) foundry.utils.mergeObject(config, CONFIG.NAHEULBEUK.weaponTypes);
  ...
  generateLabel({ depth=1, modernStyle }={}) {
    ...
    modernStyle ??= (this.parent.source?.rules === "2024")
      || (!this.parent.source?.rules && (game.settings.get("naheulbeuk", "rulesVersion") === "modern"));
    ...
    switch ( this.type ) {
      ...
      case "currency":
        const currencyConfig = CONFIG.NAHEULBEUK.currencies[this.key];
    ...
    else if ( this.type !== "linked" ) label = game.i18n.format("NAHEULBEUK.TraitConfigChooseAnyUncounted", { type: label });
    if ( (this.type === "linked") && this.requiresProficiency ) {
      label += ` (${game.i18n.localize("NAHEULBEUK.StartingEquipment.IfProficient").toLowerCase()})`;
    }
    return label;
  }
  ...
  static prefixOrEntries(entries, { depth=1, modernStyle=true, usedPrefixes }={}) {
    let letters = game.i18n.localize("NAHEULBEUK.StartingEquipment.Prefixes");
