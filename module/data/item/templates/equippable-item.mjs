...
export default class EquippableItemTemplate extends SystemDataModel {
  ...
  static defineSchema() {
    return {
      attunement: new StringField({required: true, label: "NAHEULBEUK.Attunement"}),
      attuned: new BooleanField({label: "NAHEULBEUK.Attuned"}),
      equipped: new BooleanField({required: true, label: "NAHEULBEUK.Equipped"})
    };
  }
  ...
  get equippableItemCardProperties() {
    return [
      this.attuned ? game.i18n.localize("NAHEULBEUK.AttunementAttuned")
        : CONFIG.NAHEULBEUK.attunementTypes[this.attunement] ?? null,
      game.i18n.localize(this.equipped ? "NAHEULBEUK.Equipped" : "NAHEULBEUK.Unequipped"),
      ("proficient" in this) ? CONFIG.NAHEULBEUK.proficiencyLevels[this.prof?.multiplier || 0] : null
    ];
  }
