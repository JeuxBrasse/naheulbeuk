...
export default class PhysicalItemTemplate extends SystemDataModel {
  ...
  static defineSchema() {
    return {
      container: new ForeignDocumentField(foundry.documents.BaseItem, {
        idOnly: true, label: "NAHEULBEUK.Container"
      }),
      quantity: new NumberField({
        required: true, nullable: false, integer: true, initial: 1, min: 0, label: "NAHEULBEUK.Quantity"
      }),
      weight: new SchemaField({
        value: new NumberField({
          required: true, nullable: false, initial: 0, min: 0, label: "NAHEULBEUK.Weight"
        }),
      ...
      price: new SchemaField({
        value: new NumberField({
          required: true, nullable: false, initial: 0, min: 0, label: "NAHEULBEUK.Price"
        }),
        denomination: new StringField({
          required: true, blank: false, initial: "gp", label: "NAHEULBEUK.Currency"
        })
      }, { label: "NAHEULBEUK.Price" }),
      rarity: new StringField({ required: true, blank: true, label: "NAHEULBEUK.Rarity" })
    };
  }
  ...
  static get compendiumBrowserPhysicalItemFilters() {
    return [
      ...
      ["rarity", {
        label: "NAHEULBEUK.Rarity",
        type: "set",
        config: {
          blank: game.i18n.localize("NAHEULBEUK.ItemRarityMundane").capitalize(),
          choices: Object.entries(CONFIG.NAHEULBEUK.itemRarity).reduce((obj, [key, label]) => {
  ...
  get priceLabel() {
    ...
    const hasPrice = value && (denomination in CONFIG.NAHEULBEUK.currencies);
    return hasPrice ? `${value} ${CONFIG.NAHEULBEUK.currencies[denomination].label}` : null;
  }
  ...
  get physicalItemSheetFields() {
    return [{
      label: CONFIG.NAHEULBEUK.itemRarity[this.rarity],
      value: this._source.rarity,
      requiresIdentification: true,
      field: this.schema.getField("rarity"),
      choices: CONFIG.NAHEULBEUK.itemRarity,
      blank: "NAHEULBEUK.Rarity",
      classes: "item-rarity"
    }];
  }
  ...
  static #migrateRarity(source) {
    if ( !("rarity" in source) || CONFIG.NAHEULBEUK.itemRarity[source.rarity] ) return;
    source.rarity = Object.keys(CONFIG.NAHEULBEUK.itemRarity).find(key =>
      CONFIG.NAHEULBEUK.itemRarity[key].toLowerCase() === source.rarity.toLowerCase()
    ) ?? "";
  }
   ...
   */
  preparePhysicalData() {
    if ( !("gp" in CONFIG.NAHEULBEUK.currencies) ) return;
    const { value, denomination } = this.price;
    const { conversion } = CONFIG.NAHEULBEUK.currencies[denomination] ?? {};
    const { gp } = CONFIG.NAHEULBEUK.currencies;
    if ( conversion ) {
   ...
   * @returns {Promise<ItemNaheulbeuk[]>}
   */
  async allContainers() {
