...
export default class ActivitiesTemplate extends SystemDataModel {
  ...
  /** @override */
  static LOCALIZATION_PREFIXES = ["NAHEULBEUK.USES"];
   ...
   * @type {ActiveEffectNaheulbeuk[]}
   */
  get appliedEnchantments() {
    return naheulbeuk.registry.enchantments.applied(this.parent.uuid);
  }
  ...
  static #migrateUses(source) {
    ...
    // If period is not blank, set an appropriate recovery type
      ...
      if ( CONFIG.NAHEULBEUK.limitedUsePeriods[source.uses.per]?.formula && source.uses.recovery ) {
  ...
  static initializeActivities(source) {
    ...
    if ( source._id && source.type && ("value" in uses) && uses.max ) {
      foundry.utils.setProperty(source, "flags.naheulbeuk.migratedUses", uses.value);
    }
  }
  ...
  static #createInitialActivity(source) {
    ...
    const cls = CONFIG.NAHEULBEUK.activityTypes[type].documentClass;
    cls.createInitialActivity(source);

    if ( (type !== "save") && source.system.save?.ability ) {
      CONFIG.NAHEULBEUK.activityTypes.save.documentClass.createInitialActivity(source, { offset: 1 });
    }
    if ( (source.type !== "weapon") && source.system.damage?.versatile ) {
      CONFIG.NAHEULBEUK.activityTypes.damage.documentClass.createInitialActivity(source, { offset: 2, versatile: true });
    }
    if ( (type !== "utility") && source.system.formula ) {
      CONFIG.NAHEULBEUK.activityTypes.utility.documentClass.createInitialActivity(source, { offset: 3 });
    }
  }
  ...
  async recoverUses(periods, rollData) {
    const updates = {};
    const rolls = [];
    const autoRecharge = game.settings.get("naheulbeuk", "autoRecharge");
  ...
  async preUpdateActivities(changed, options, user) {
    ...
    if ( !riders.activity.size && !riders.effect.size ) {
      foundry.utils.setProperty(changed, "flags.naheulbeuk.-=riders", null);
    } else {
      foundry.utils.setProperty(changed, "flags.naheulbeuk.riders", Object.entries(riders)
    ...
    if ( removed.length ) foundry.utils.setProperty(options, "naheulbeuk.removedCachedItems", removed);
  }
  ...
  async onUpdateActivities(changed, options, userId) {
    ...
    if ( options.naheulbeuk?.removedCachedItems ) {
      await this.parent.actor.deleteEmbeddedDocuments("Item", options.naheulbeuk.removedCachedItems);
    }
