...
export default class SpellData extends ItemDataModel.mixin(ActivitiesTemplate, ItemDescriptionTemplate) {
  ...
  static LOCALIZATION_PREFIXES = [
    "NAHEULBEUK.ACTIVATION", "NAHEULBEUK.DURATION", "NAHEULBEUK.RANGE", "NAHEULBEUK.SOURCE", "NAHEULBEUK.TARGET"
  ];
  ...
  static defineSchema() {
    return this.mergeSchema(super.defineSchema(), {
      ability: new StringField({ label: "NAHEULBEUK.SpellAbility" }),
      activation: new ActivationField(),
      duration: new DurationField(),
      level: new NumberField({ required: true, integer: true, initial: 1, min: 0, label: "NAHEULBEUK.SpellLevel" }),
      materials: new SchemaField({
        value: new StringField({ required: true, label: "NAHEULBEUK.SpellMaterialsDescription" }),
        consumed: new BooleanField({ required: true, label: "NAHEULBEUK.SpellMaterialsConsumed" }),
        cost: new NumberField({ required: true, initial: 0, min: 0, label: "NAHEULBEUK.SpellMaterialsCost" }),
        supply: new NumberField({ required: true, initial: 0, min: 0, label: "NAHEULBEUK.SpellMaterialsSupply" })
      }, { label: "NAHEULBEUK.SpellMaterials" }),
      method: new StringField({ required: true, initial: "", label: "NAHEULBEUK.SpellPreparation.Method" }),
      prepared: new NumberField({ required: true, nullable: false, integer: true, min: 0, initial: 0 }),
      properties: new SetField(new StringField(), { label: "NAHEULBEUK.SpellComponents" }),
      range: new RangeField(),
      school: new StringField({ required: true, label: "NAHEULBEUK.SpellSchool" }),
      sourceClass: new StringField({ label: "NAHEULBEUK.SpellSourceClass" }),
  ...
  static get compendiumBrowserFilters() {
    return new Map([
      ["level", {
        label: "NAHEULBEUK.Level",
        type: "range",
        config: {
          keyPath: "system.level",
          min: 0,
          max: Object.keys(CONFIG.NAHEULBEUK.spellLevels).length - 1
        }
      }],
      ["school", {
        label: "NAHEULBEUK.School",
        type: "set",
        config: {
          choices: CONFIG.NAHEULBEUK.spellSchools,
      ...
      ["spelllist", {
        ...
        createFilter: (filters, value, def) => {
          ...
          for ( const [k, v] of Object.entries(value ?? {}) ) {
            const list = naheulbeuk.registry.spellLists.forType(k);
        ...
        config: {
          choices: naheulbeuk.registry.spellLists.options.reduce((obj, entry) => {
            const [type, identifier] = entry.value.split(":");
            const list = naheulbeuk.registry.spellLists.forType(type, identifier);
            if ( list?.identifiers.size ) obj[entry.value] = {
              label: entry.label, group: CONFIG.NAHEULBEUK.spellListTypes[type]
            };
            return obj;
          }, {}),
          collapseGroup: group => group !== CONFIG.NAHEULBEUK.spellListTypes.class
        }
      }],
   ...
   */
  get canPrepare() {
    return !!CONFIG.NAHEULBEUK.spellcasting[this.method]?.prepares;
  }
  ...
  /** @override */
  get canScale() {
    return (this.level > 0) && !!CONFIG.NAHEULBEUK.spellcasting[this.method]?.slots;
  }
  ...
  get countsPrepared() {
    return !!CONFIG.NAHEULBEUK.spellcasting[this.method]?.prepares
      && (this.level > 0)
      && (this.prepared === CONFIG.NAHEULBEUK.spellPreparationStates.prepared.value);
  }
  ...
  get criticalThreshold() {
    return this.parent?.actor?.flags.naheulbeuk?.spellCriticalThreshold ?? Infinity;
  }
  ...
  get linkedActivity() {
    ...
    const uuid = this.parent.getFlag("naheulbeuk", "cachedFor");
    ...
    // return fromUuidSync(this.parent.getFlag("naheulbeuk", "cachedFor"), { relative, strict: false }) ?? null;
  }
  ...
  /** @override */
  get tooltipSubtitle() {
    return [this.parent.labels.level, CONFIG.NAHEULBEUK.spellSchools[this.school]?.label];
  }
  ...
  static _migrateComponentData(source) {
    ...
    if ( components.length ) {
      foundry.utils.setProperty(source, "flags.naheulbeuk.migratedProperties", components);
    }
  }
  ...
  static #migrateTarget(source) {
    ...
    if ( type in CONFIG.NAHEULBEUK.areaTargetTypes ) {
    ...
    } else if ( type in CONFIG.NAHEULBEUK.individualTargetTypes ) {
  ...
  prepareDerivedData() {
    ...
    labels.level = CONFIG.NAHEULBEUK.spellLevels[this.level];
    labels.school = CONFIG.NAHEULBEUK.spellSchools[this.school]?.label;
    if ( this.properties.has("material") ) labels.materials = this.materials.value;

    labels.components = this.properties.reduce((obj, c) => {
      const config = this.validProperties.has(c) ? CONFIG.NAHEULBEUK.itemProperties[c] : null;
    ...
    Object.defineProperty(labels, "classes", {
      get() {
        return Array.from(naheulbeuk.registry.spellLists.forSpell(uuid))
  ...
  async getSheetData(context) {
    ...
    context.subtitles = [
      ...
      { label: CONFIG.NAHEULBEUK.spellcasting[this.method]?.label }
    ];

    context.parts = ["naheulbeuk.details-spell", "naheulbeuk.field-uses"];

    // Default Ability & Spellcasting Classes
    if ( this.parent.actor ) {
      const ability = CONFIG.NAHEULBEUK.abilities[
        this.parent.actor.spellcastingClasses[this.sourceClass]?.spellcasting.ability
          ?? this.parent.actor.system.attributes?.spellcasting
      ]?.label?.toLowerCase();
      if ( ability ) context.defaultAbility = game.i18n.format("NAHEULBEUK.DefaultSpecific", { default: ability });
      else context.defaultAbility = game.i18n.localize("NAHEULBEUK.Default");
    ...
    context.activationTypes = [
      ...Object.entries(CONFIG.NAHEULBEUK.activityActivationTypes).map(([value, { label, group }]) => {
        return { value, label, group };
      }),
      { value: "", label: "NAHEULBEUK.NoneActionLabel" }
    ];

    // Duration
    context.durationUnits = [
      ...Object.entries(CONFIG.NAHEULBEUK.specialTimePeriods).map(([value, label]) => ({ value, label })),
      ...Object.entries(CONFIG.NAHEULBEUK.scalarTimePeriods).map(([value, label]) => {
        return { value, label, group: "NAHEULBEUK.DurationTime" };
      }),
      ...Object.entries(CONFIG.NAHEULBEUK.permanentTimePeriods).map(([value, label]) => {
        return { value, label, group: "NAHEULBEUK.DurationPermanent" };
      })
    ];

    // Targets
    context.targetTypes = [
      ...Object.entries(CONFIG.NAHEULBEUK.individualTargetTypes).map(([value, { label }]) => {
        return { value, label, group: "NAHEULBEUK.TargetTypeIndividual" };
      }),
      ...Object.entries(CONFIG.NAHEULBEUK.areaTargetTypes).map(([value, { label }]) => {
        return { value, label, group: "NAHEULBEUK.TargetTypeArea" };
      })
    ];
    context.scalarTarget = this.target.affects.type
      && (CONFIG.NAHEULBEUK.individualTargetTypes[this.target.affects.type]?.scalar !== false);
    context.affectsPlaceholder = game.i18n.localize(`NAHEULBEUK.TARGET.Count.${
    ...
    // Range
    context.rangeTypes = [
      ...Object.entries(CONFIG.NAHEULBEUK.rangeTypes).map(([value, label]) => ({ value, label })),
      ...Object.entries(CONFIG.NAHEULBEUK.movementUnits).map(([value, { label }]) => {
        return { value, label, group: "NAHEULBEUK.RangeDistance" };
      })
    ];

    // Spellcasting
    context.canPrepare = this.canPrepare;
    context.spellcastingMethods = Object.values(CONFIG.NAHEULBEUK.spellcasting).map(({ key, label }) => {
      return { label, value: key };
    });
    if ( this.method && !(this.method in CONFIG.NAHEULBEUK.spellcasting) ) {
  ...
  static onDropCreate(event, actor, itemData) {
    ...
    const spellcastKeys = Object.keys(CONFIG.NAHEULBEUK.spellcasting);
    ...
    const methods = CONFIG.NAHEULBEUK.spellcasting;
  ...
  getRollData(...options) {
    ...
    data.item.level = data.item.level + (this.parent.getFlag("naheulbeuk", "scaling") ?? 0);
    return data;
  }
  ...
  async _preCreate(data, options, user) {
    ...
    if ( (system.method !== "spell") && (system.method in CONFIG.NAHEULBEUK.spellcasting) ) {
    ...
    const spellClasses = new Set(
      naheulbeuk.registry.spellLists.forSpell(this.parent._stats.compendiumSource).map(l => l.metadata.identifier)
    );
