...
export default class SpellcastingField extends SchemaField {
  constructor(fields={}, options={}) {
    fields = {
      progression: new StringField({
        ...
        initial: "none",
        blank: false,
        label: "NAHEULBEUK.SpellProgression"
      }),
      ability: new StringField({ label: "NAHEULBEUK.SpellAbility" }),
      preparation: new SchemaField({
        formula: new FormulaField({ label: "NAHEULBEUK.SpellPreparation.Formula" })
      }),
      ...fields
    };
    Object.entries(fields).forEach(([k, v]) => !v ? delete fields[k] : null);
    super(fields, { label: "NAHEULBEUK.Spellcasting", ...options });
  }
  ...
  static prepareData(rollData) {
    ...
    this.spellcasting.type = CONFIG.NAHEULBEUK.spellProgression[this.spellcasting.progression]?.type;
    this.spellcasting.slots = CONFIG.NAHEULBEUK.spellcasting[this.spellcasting.type]?.slots;
