...
export default class ClassData extends ItemDataModel.mixin(
  ...
  /** @override */
  static LOCALIZATION_PREFIXES = ["NAHEULBEUK.CLASS", "NAHEULBEUK.SOURCE"];
  ...
  static get compendiumBrowserFilters() {
    return new Map([
      ["hasSpellcasting", {
        label: "NAHEULBEUK.CompendiumBrowser.Filters.HasSpellcasting",
        type: "boolean",
  ...
  static _migrateTraitAdvancement(source) {
    ...
    if ( needsMigration ) foundry.utils.setProperty(source, "flags.naheulbeuk.persistSourceMigration", true);
  }
  ...
  async getSheetData(context) {
    ...
    context.parts = ["naheulbeuk.details-class", "naheulbeuk.details-spellcasting", "naheulbeuk.details-starting-equipment"];
    context.hitDieOptions = CONFIG.NAHEULBEUK.hitDieTypes.map(d => ({ value: d, label: d }));
    context.primaryAbilities = Object.entries(CONFIG.NAHEULBEUK.abilities).map(([value, data]) => ({
  ...
  async _preUpdate(changed, options, user) {
    ...
    if ( changed.system.levels <= 0 ) {
      ui.notifications.warn("NAHEULBEUK.MaxClassLevelMinimumWarn", { localize: true });
      changed.system.levels = 1;
    }

    // Check to make sure the updated class level doesn't exceed level cap
    if ( changed.system.levels > CONFIG.NAHEULBEUK.maxLevel ) {
      ui.notifications.warn(game.i18n.format("NAHEULBEUK.MaxClassLevelExceededWarn", { max: CONFIG.NAHEULBEUK.maxLevel }));
      changed.system.levels = CONFIG.NAHEULBEUK.maxLevel;
    }
    ...
    if ( newCharacterLevel > CONFIG.NAHEULBEUK.maxLevel ) {
      ui.notifications.warn(game.i18n.format("NAHEULBEUK.MaxCharacterLevelExceededWarn", { max: CONFIG.NAHEULBEUK.maxLevel }));
      changed.system.levels -= newCharacterLevel - CONFIG.NAHEULBEUK.maxLevel;
    }
  }
