...
export default class CommonTemplate extends ActorDataModel.mixin(CurrencyTemplate) {
    ...
    return this.mergeSchema(super.defineSchema(), {
      abilities: new MappingField(new SchemaField({
        value: new NumberField({
          required: true, nullable: false, integer: true, min: 0, initial: 10, label: "NAHEULBEUK.AbilityScore"
        }),
        proficient: new NumberField({
          required: true, integer: true, min: 0, max: 1, initial: 0, label: "NAHEULBEUK.ProficiencyLevel"
        }),
        max: new NumberField({
          required: true, integer: true, nullable: true, min: 0, initial: null, label: "NAHEULBEUK.AbilityScoreMax"
        }),
        bonuses: new SchemaField({
          check: new FormulaField({ required: true, label: "NAHEULBEUK.AbilityCheckBonus" }),
          save: new FormulaField({ required: true, label: "NAHEULBEUK.SaveBonus" })
        }, { label: "NAHEULBEUK.AbilityBonuses" }),
        check: new RollConfigField({ ability: false }),
        save: new RollConfigField({ ability: false })
      }), {
        initialKeys: CONFIG.NAHEULBEUK.abilities, initialValue: this._initialAbilityValue.bind(this),
        initialKeysOnly: true, label: "NAHEULBEUK.Abilities"
      })
    });
  }
  ...
  static _initialAbilityValue(key, initial, existing) {
    const config = CONFIG.NAHEULBEUK.abilities[key];
    if ( config ) {
  ...
  prepareAbilities({ rollData={}, originalSaves }={}) {
    ...
    for ( const [id, abl] of Object.entries(this.abilities) ) {
      ...
      if ( !Number.isFinite(abl.max) ) abl.max = CONFIG.NAHEULBEUK.maxAbilityScore;
  ...
  calculateAbilityCheckProficiency(multiplier, ability) {
    ...
    if ( multiplier < 1 ) {
      ...
      else if ( this.parent.flags.naheulbeuk?.jackOfAllTrades ) multiplier = .5;
    }
  ...
  calculateToolProficiency(multiplier, ability) {
    if ( (multiplier === 1) && this.parent.flags.naheulbeuk?.toolExpertise ) {
