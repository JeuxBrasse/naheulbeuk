import ActiveEffectNaheulbeuk from "../../../documents/active-effect.mjs";
import Proficiency from "../../../documents/actor/proficiency.mjs";
import ItemNaheulbeuk from "../../../documents/item.mjs";
...
export default class AttributesFields {
  ...
  static get armorClass() {
    return {
      calc: new StringField({ initial: "default", label: "NAHEULBEUK.ArmorClassCalculation" }),
      flat: new NumberField({ required: true, integer: true, min: 0, label: "NAHEULBEUK.ArmorClassFlat" }),
      formula: new FormulaField({ deterministic: true, label: "NAHEULBEUK.ArmorClassFormula" })
    };
  }
  ...
  static get hitPoints() {
    return {
      dt: new NumberField({ integer: true, min: 0, label: "NAHEULBEUK.DamageThreshold" }),
      max: new NumberField({ nullable: true, integer: true, min: 0, initial: null, label: "NAHEULBEUK.HitPointsMax" }),
      temp: new NumberField({ integer: true, initial: 0, min: 0, label: "NAHEULBEUK.HitPointsTemp" }),
      tempmax: new NumberField({
        integer: true, initial: 0, label: "NAHEULBEUK.HitPointsTempMax", hint: "NAHEULBEUK.HitPointsTempMaxHint"
      }),
      value: new NumberField({ nullable: true, integer: true, min: 0, initial: null, label: "NAHEULBEUK.HitPointsCurrent" })
    };
  }
  ...
  static get common() {
    return {
      ac: new SchemaField(this.armorClass, { label: "NAHEULBEUK.ArmorClass" }),
      init: new RollConfigField({
        ability: "",
        bonus: new FormulaField({ required: true, label: "NAHEULBEUK.InitiativeBonus" })
      }, { label: "NAHEULBEUK.Initiative" }),
  ...
  static get creature() {
    return {
      attunement: new SchemaField({
        max: new NumberField({
          required: true, nullable: false, integer: true, min: 0, initial: 3, label: "NAHEULBEUK.AttunementMax"
        })
      }, { label: "NAHEULBEUK.Attunement" }),
      senses: new SensesField(),
      spellcasting: new StringField({ required: true, blank: true, label: "NAHEULBEUK.SpellAbility" }),
      exhaustion: new NumberField({
        required: true, nullable: false, integer: true, min: 0, initial: 0, label: "NAHEULBEUK.Exhaustion"
      }),
      concentration: new RollConfigField({
        ability: "",
        bonuses: new SchemaField({
          save: new FormulaField({ required: true, label: "NAHEULBEUK.ConcentrationBonus" })
        }),
        limit: new NumberField({ integer: true, min: 0, initial: 1, label: "NAHEULBEUK.ConcentrationLimit" })
      }, { label: "NAHEULBEUK.Concentration" }),
      loyalty: new SchemaField({
        value: new NumberField({ integer: true, min: 0, max: 20, label: "NAHEULBEUK.Loyalty" })
      })
    };
  }
  ...
  static prepareArmorClass(rollData) {
    ...
    let cfg = CONFIG.NAHEULBEUK.armorClasses[ac.calc];
    if ( !cfg ) {
      ac.calc = "flat";
      if ( Number.isNumeric(ac.value) ) ac.flat = Number(ac.value);
      cfg = CONFIG.NAHEULBEUK.armorClasses.flat;
    }

    // Identify Equipped Items
    const { armors, shields } = this.parent.itemTypes.equipment.reduce((obj, equip) => {
      if ( !equip.system.equipped || !(equip.system.type.value in CONFIG.NAHEULBEUK.armorTypes)) return obj;
    ...
    ac.label = !["custom", "flat"].includes(ac.calc) ? CONFIG.NAHEULBEUK.armorClasses[ac.calc]?.label : null;
      ...
      default:
        ...
        if ( armors.length ) {
          if ( armors.length > 1 ) this.parent._preparationWarnings.push({
            message: game.i18n.localize("NAHEULBEUK.WarnMultipleArmor"), type: "warning"
          });
        ...
        try {
          const replaced = replaceFormulaData(formula, rollData, {
            actor: this, missing: null, property: game.i18n.localize("NAHEULBEUK.ArmorClass")
          });
          ac.base = replaced ? new Roll(replaced).evaluateSync().total : 0;
        } catch(err) {
          this.parent._preparationWarnings.push({
            message: game.i18n.format("NAHEULBEUK.WarnBadACFormula", { formula }), link: "armor", type: "error"
          });
          const replaced = Roll.replaceFormulaData(CONFIG.NAHEULBEUK.armorClasses.default.formula, rollData);
    ...
    if ( shields.length ) {
      if ( shields.length > 1 ) this.parent._preparationWarnings.push({
        message: game.i18n.localize("NAHEULBEUK.WarnMultipleShields"), type: "warning"
      });
  ...
  static prepareConcentration(rollData) {
    ...
    const abilityId = concentration.ability || CONFIG.NAHEULBEUK.defaultAbilities.concentration;
  ...
  static prepareEncumbrance(rollData, { validateItem }={}) {
    const config = CONFIG.NAHEULBEUK.encumbrance;
    const encumbrance = this.attributes.encumbrance ??= {};
    const baseUnits = CONFIG.NAHEULBEUK.encumbrance.baseUnits[this.parent.type]
      ?? CONFIG.NAHEULBEUK.encumbrance.baseUnits.default;
    const unitSystem = game.settings.get("naheulbeuk", "metricWeightUnits") ? "metric" : "imperial";
    ...
    if ( game.settings.get("naheulbeuk", "currencyWeight") && currency ) {
    ...
    const keys = Object.keys(CONFIG.NAHEULBEUK.actorSizes);
    const index = keys.findIndex(k => k === this.traits.size);
    const sizeConfig = CONFIG.NAHEULBEUK.actorSizes[
      keys[this.parent.flags.naheulbeuk?.powerfulBuild ? Math.min(index + 1, keys.length - 1) : index]
    ];
  ...
  static prepareExhaustionLevel() {
    const exhaustion = this.parent.effects.get(ActiveEffectNaheulbeuk.ID.EXHAUSTION);
    const level = exhaustion?.getFlag("naheulbeuk", "exhaustionLevel");
  ...
  static prepareInitiative(rollData) {
    ...
    const flags = this.parent.flags.naheulbeuk ?? {};
    ...
    const isLegacy = game.settings.get("naheulbeuk", "rulesVersion") === "legacy";
    ...
    init.score = CONFIG.NAHEULBEUK.skillPassive.base + init.total + (init.roll.mode * CONFIG.NAHEULBEUK.skillPassive.modifier);
  }
  ...
  static prepareMovement(rollData=this.parent.getRollData()) {
    ...
    for ( const type of Object.keys(CONFIG.NAHEULBEUK.movementTypes) ) {
    ...
    let reduction = game.settings.get("naheulbeuk", "rulesVersion") === "modern"
      ? (this.attributes.exhaustion ?? 0) * (CONFIG.NAHEULBEUK.conditionTypes.exhaustion?.reduction?.speed ?? 0) : 0;
    reduction = convertLength(reduction, CONFIG.NAHEULBEUK.defaultUnits.length.imperial, units);
    ...
    for ( const type of Object.keys(CONFIG.NAHEULBEUK.movementTypes) ) {
      ...
      if ( speed ) {
        ...
        if ( heavilyEncumbered ) {
          speed = Math.max(0, speed - (CONFIG.NAHEULBEUK.encumbrance.speedReduction.heavilyEncumbered[units] ?? 0));
        } else if ( encumbered ) {
          speed = Math.max(0, speed - (CONFIG.NAHEULBEUK.encumbrance.speedReduction.encumbered[units] ?? 0));
        }
        if ( exceedingCarryingCapacity ) {
          speed = Math.min(speed, CONFIG.NAHEULBEUK.encumbrance.speedReduction.exceedingCarryingCapacity[units] ?? 0);
        }
      }
   ...
   * @param {ItemNaheulbeuk} race                    Race item from which to get the stats.
   * @param {object} [options={}]
   * @param {boolean} [options.force=false]  Override any values on the actor.
   * @this {CharacterData|NPCData}
   */
  static prepareRace(race, { force=false }={}) {
    for ( const key of Object.keys(CONFIG.NAHEULBEUK.movementTypes) ) {
    ...
    for ( const key of Object.keys(CONFIG.NAHEULBEUK.senses) ) {
  ...
  static prepareSpellcastingAbility() {
    ...
    this.attributes.spell.abilityLabel = CONFIG.NAHEULBEUK.abilities[this.attributes.spellcasting]?.label ?? "";
