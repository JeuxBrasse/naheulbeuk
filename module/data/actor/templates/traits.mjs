...
export default class TraitsField {
  ...
  static get common() {
    return {
      size: new StringField({ required: true, initial: "med", label: "NAHEULBEUK.Size" }),
      di: new DamageTraitField({}, { label: "NAHEULBEUK.DamImm" }),
      dr: new DamageTraitField({}, { label: "NAHEULBEUK.DamRes" }),
      dv: new DamageTraitField({}, { label: "NAHEULBEUK.DamVuln" }),
      dm: new SchemaField({
        amount: new MappingField(new FormulaField({ deterministic: true }), { label: "NAHEULBEUK.DamMod" }),
        bypasses: new SetField(new StringField(), {
          label: "NAHEULBEUK.DamagePhysicalBypass", hint: "NAHEULBEUK.DamagePhysicalBypassHint"
        })
      }),
      ci: new SimpleTraitField({}, { label: "NAHEULBEUK.ConImm" })
    };
  }
  ...
  static prepareLanguages() {
    ...
    if ( languages.value.has("ALL") ) labels.languages.push(game.i18n.localize("NAHEULBEUK.Language.All"));
    else {
      ...
      for ( const [key, data] of Object.entries(CONFIG.NAHEULBEUK.languages) ) {
    ...
    for ( const [key, { label }] of Object.entries(CONFIG.NAHEULBEUK.communicationTypes) ) {
  ...
  static prepareResistImmune() {
    ...
    if ( this.parent.hasConditionEffect("petrification") ) {
      this.traits.dr.custom = game.i18n.localize("NAHEULBEUK.DamageAll");
      Object.keys(CONFIG.NAHEULBEUK.damageTypes).forEach(type => this.traits.dr.value.add(type));
