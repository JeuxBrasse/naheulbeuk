...
export default class VehicleData extends CommonTemplate {
  ...
  /** @override */
  static LOCALIZATION_PREFIXES = ["NAHEULBEUK.SOURCE"];
    ...
    return this.mergeSchema(super.defineSchema(), {
      attributes: new SchemaField({
        ...
        ac: new SchemaField({
          ...
          calc: new StringField({ initial: "flat", label: "NAHEULBEUK.ArmorClassCalculation" })
        }, { label: "NAHEULBEUK.ArmorClass" }),
        hp: new SchemaField({
          ...AttributesFields.hitPoints,
          mt: new NumberField({
            required: true, integer: true, min: 0, label: "NAHEULBEUK.VEHICLE.Mishap.Threshold.label"
          })
        }, { label: "NAHEULBEUK.HitPoints" }),
        actions: new SchemaField({
          max: new NumberField({
            required: true, nullable: false, integer: true, initial: 3, min: 0, max: 3,
            label: "NAHEULBEUK.VEHICLE.FIELDS.attributes.actions.max.label"
          }),
          spent: new NumberField({
            required: true, nullable: false, integer: true, initial: 0, min: 0, max: 3,
            label: "NAHEULBEUK.VEHICLE.FIELDS.attributes.actions.spent.label"
          }),
          stations: new BooleanField({
            required: true, initial: true, label: "NAHEULBEUK.VEHICLE.FIELDS.attributes.actions.stations.label"
          }),
          thresholds: new SchemaField({
            2: new NumberField({
              required: true, integer: true, min: 0,
              label: "NAHEULBEUK.VEHICLE.FIELDS.attributes.actions.thresholds.full.label"
            }),
            1: new NumberField({
              required: true, integer: true, min: 0,
              label: "NAHEULBEUK.VEHICLE.FIELDS.attributes.actions.thresholds.mid.label"
            }),
            0: new NumberField({
              required: true, integer: true, min: 0,
              label: "NAHEULBEUK.VEHICLE.FIELDS.attributes.actions.thresholds.min.label"
            })
          }, { label: "NAHEULBEUK.VEHICLE.FIELDS.attributes.actions.thresholds.label" })
        }, { label: "NAHEULBEUK.VEHICLE.FIELDS.attributes.actions.label" }),
        capacity: new SchemaField({
          cargo: new SchemaField({
            value: new NumberField({ min: 0, label: "NAHEULBEUK.VEHICLE.FIELDS.attributes.capacity.cargo.value.label" }),
            units: new StringField({
              required: true, blank: false, label: "NAHEULBEUK.UNITS.WEIGHT.Label", initial: () => defaultUnits("weight")
            })
          }, { label: "NAHEULBEUK.VEHICLE.FIELDS.attributes.capacity.cargo.value.label" }),
          creature: new StringField({ required: true, label: "NAHEULBEUK.VehicleCreatureCapacity" }) // FIXME: Leave in the model until we decide how to migrate it.
        }, { label: "NAHEULBEUK.VEHICLE.FIELDS.attributes.capacity.label" }),
        price: new SchemaField({
          value: new NumberField({ initial: null, min: 0, label: "NAHEULBEUK.Price" }),
          denomination: new StringField({ required: true, blank: false, initial: "gp", label: "NAHEULBEUK.Currency" })
        }, { label: "NAHEULBEUK.Price" }),
        ...
        travel: new TravelField({ pace: false, }, {
          initialTime: () => CONFIG.NAHEULBEUK.travelTimes.vehicle, initialUnits: () => defaultUnits("travel")
        })
      }, { label: "NAHEULBEUK.Attributes" }),
      ...
      details: new SchemaField({
        ...
        type: new StringField({ required: true, blank: false, initial: "water", label: "NAHEULBEUK.VEHICLE.Type.label" })
      }, { label: "NAHEULBEUK.Details" }),
      ...
      traits: new SchemaField({
        ...TraitsFields.common,
        size: new StringField({ required: true, blank: false, initial: "lg", label: "NAHEULBEUK.Size" }),
        weight: new SchemaField({
          value: new NumberField({ min: 0, label: "NAHEULBEUK.Weight" }),
          units: new StringField({
            required: true, blank: false, label: "NAHEULBEUK.UNITS.WEIGHT.Label", initial: () => defaultUnits("weight")
          })
        }, { label: "NAHEULBEUK.Weight" }),
        keel: new SchemaField({
          value: new NumberField({ min: 0, label: "NAHEULBEUK.VEHICLE.FIELDS.traits.keel.value.label" }),
          units: new StringField({
            required: true, blank: false, label: "NAHEULBEUK.UNITS.DISTANCE.Label", initial: () => defaultUnits("length")
          })
        }),
        beam: new SchemaField({
          value: new NumberField({ min: 0, label: "NAHEULBEUK.VEHICLE.FIELDS.traits.beam.value.label" }),
          units: new StringField({
            required: true, blank: false, label: "NAHEULBEUK.UNITS.DISTANCE.Label", initial: () => defaultUnits("length")
          })
        }),
        dimensions: new StringField({ required: true, label: "NAHEULBEUK.Dimensions" }) // FIXME: Leave in the model until we decide how to migrate it.
      }, { label: "NAHEULBEUK.Traits" }),
  ...
  static #migrateMovement(source) {
    ...
    let max = 0;
    for ( const p in CONFIG.NAHEULBEUK.movementTypes ) {
   ...
   * @returns {Promise<ActorNaheulbeuk>}    The actor with updates applied.
   */
  ...
  async getEncumbrance() {
    ...
    const { baseUnits, draftMultiplier } = CONFIG.NAHEULBEUK.encumbrance;
