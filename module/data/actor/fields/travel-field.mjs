...
/**
 * @import { TravelPaceNaheulbeuk } from "./_types.mjs";
 */
...
export default class TravelField extends foundry.data.fields.SchemaField {
  constructor(fields={}, { initialTime=8, initialUnits=null, ...options }={}) {
    fields = {
      pace: new StringField({
        required: true, blank: false, initial: "normal", choices: () => CONFIG.NAHEULBEUK.travelPace,
        label: "NAHEULBEUK.TRAVEL.Label"
      }),
      paces: new MappingField(new FormulaField({ deterministic: true }), {
        initialKeys: CONFIG.NAHEULBEUK.travelTypes, initialKeysOnly: true
      }),
      ...
      time: new NumberField({
        ...
        label: "NAHEULBEUK.TRAVEL.FIELDS.time.label", hint: "NAHEULBEUK.TRAVEL.FIELDS.time.hint"
      }),
      units: new StringField({
        required: true, nullable: true, blank: false, initial: initialUnits, label: "NAHEULBEUK.UNITS.TRAVEL.Label"
      }),
      ...fields
    };
  ...
  static prepareData(rollData) {
    ...
    for ( const type of Object.keys(travel.speeds) ) {
      ...
      if ( pace && paceMode ) {
        ...
        travel.paces[type] = TravelField.applyPaceMultiplier(pace, paceMode, CONFIG.NAHEULBEUK.travelUnits[units]?.type);
      }
    ...
    if ( !movement ) return;
    for ( const [type, { travel: travelType="land" }] of Object.entries(CONFIG.NAHEULBEUK.movementTypes) ) {
      ...
      const pace = travel.paces[travelType] ||= TravelField.applyPaceMultiplier(
        speed * travel.time, paceMode, CONFIG.NAHEULBEUK.movementUnits[movement.units]?.type
      );
   ...
   * @param {number} value
   * @param {TravelPaceNaheulbeuk} pace
   * @param {"imperial"|"metric"} [unitType]
   * @returns {number}
   */
  static applyPaceMultiplier(value, pace, unitType) {
    const { normal } = CONFIG.NAHEULBEUK.travelPace;
    const config = CONFIG.NAHEULBEUK.travelPace[pace];
    if ( (unitType === "imperial") && (value === normal.standard) && config?.standard ) return config.standard;
    value *= CONFIG.NAHEULBEUK.travelPace[pace]?.multiplier ?? 1;
  ...
  static convertMovementToTravel(value, initialUnit, finalUnit) {
    const fromConfig = CONFIG.NAHEULBEUK.movementUnits[initialUnit];
    const toConfig = CONFIG.NAHEULBEUK.movementUnits[finalUnit];
  ...
  static getTravelPaceMode(pace, skill) {
    const config = CONFIG.NAHEULBEUK.skills[skill];
    return {
