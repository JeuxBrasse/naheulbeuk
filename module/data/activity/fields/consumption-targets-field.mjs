...
import EmbeddedDataFieldNaheulbeuk from "../../fields/embedded-data-field.mjs";
...
export default class ConsumptionTargetsField extends ArrayField {
  constructor(options={}) {
    super(new EmbeddedDataFieldNaheulbeuk(ConsumptionTargetData), options);
  }
}
...
export class ConsumptionTargetData extends foundry.abstract.DataModel {
  ...
  get combatOnly() {
    ...
    return recovery.every(r => CONFIG.NAHEULBEUK.limitedUsePeriods[r.period]?.type === "combat");
  }
   ...
   */
  get validTargets() {
    const config = CONFIG.NAHEULBEUK.activityConsumptionTypes[this.type];
  ...
  async consume(config, updates) {
    const typeConfig = CONFIG.NAHEULBEUK.activityConsumptionTypes[this.type];
  ...
  static async consumeActivityUses(config, updates) {
    const result = await this._usesConsumption(config, {
      ...
      type: game.i18n.format("NAHEULBEUK.CONSUMPTION.Type.ActivityUses.Warning", {
  ...
  static async consumeAttribute(config, updates) {
    ...
    if ( !foundry.utils.hasProperty(this.actor, keyPath) ) throw new ConsumptionError(
      game.i18n.format("NAHEULBEUK.CONSUMPTION.Warning.MissingAttribute", {
    ...
    let warningMessage;
    if ( (cost > 0) && !current ) warningMessage = "NAHEULBEUK.CONSUMPTION.Warning.None";
    else if ( current < cost ) warningMessage = "NAHEULBEUK.CONSUMPTION.Warning.NotEnough";
    if ( warningMessage ) throw new ConsumptionError(game.i18n.format(warningMessage, {
      ...
      type: game.i18n.format("NAHEULBEUK.CONSUMPTION.Type.Attribute.Warning", { attribute: this.target })
    }));
  ...
  static async consumeHitDice(config, updates) {
    ...
    let warningMessage;
    if ( !validClasses.length ) warningMessage = "NAHEULBEUK.CONSUMPTION.Warning.MissingHitDice";
    else if ( (cost > 0) && !total ) warningMessage = "NAHEULBEUK.CONSUMPTION.Warning.None";
    else if ( total < cost ) warningMessage = "NAHEULBEUK.CONSUMPTION.Warning.NotEnough";
    if ( warningMessage ) {
      ...
      throw new ConsumptionError(game.i18n.format(warningMessage, {
        ...
        type: game.i18n.format("NAHEULBEUK.CONSUMPTION.Type.HitDice.Warning", { denomination })
      }));
    }
  ...
  static async consumeItemUses(config, updates) {
    ...
    if ( !item ) throw new ConsumptionError(game.i18n.format("NAHEULBEUK.CONSUMPTION.Warning.MissingItem", {
      activity: this.activity.name, item: this.item.name
    }));

    const result = await this._usesConsumption(config, {
      uses: item.system.uses,
      type: game.i18n.format("NAHEULBEUK.CONSUMPTION.Type.ItemUses.Warning", { name: this.item.name }),
      rolls: updates.rolls,
  ...
  static async consumeMaterial(config, updates) {
    ...
    let warningMessage;
    if ( cost > 0 && !item.system.quantity ) warningMessage = "NAHEULBEUK.CONSUMPTION.Warning.None";
    else if ( cost > item.system.quantity ) warningMessage = "NAHEULBEUK.CONSUMPTION.Warning.NotEnough";
    if ( warningMessage ) throw new ConsumptionError(game.i18n.format(warningMessage, {
      available: formatNumber(item.system.quantity), cost: formatNumber(cost),
      type: game.i18n.format("NAHEULBEUK.CONSUMPTION.Type.Material.Warning", { name: item.name })
    }));
  ...
  static async consumeSpellSlots(config, updates) {
    const levelNumber = Math.clamp(
      this.resolveLevel({ config, rolls: updates.rolls }), 1, Object.keys(CONFIG.NAHEULBEUK.spellLevels).length - 1
    );
    ...
    let warningMessage;
    if ( !levelData?.max ) warningMessage = "NAHEULBEUK.CONSUMPTION.Warning.MissingSpellSlot";
    else if ( (cost > 0) && !levelData.value ) warningMessage = "NAHEULBEUK.CONSUMPTION.Warning.None";
    else if ( newValue < 0 ) warningMessage = "NAHEULBEUK.CONSUMPTION.Warning.NotEnough";
    if ( warningMessage ) {
      const level = CONFIG.NAHEULBEUK.spellLevels[levelNumber];
      const type = game.i18n.format("NAHEULBEUK.CONSUMPTION.Type.SpellSlots.Warning", { level });
      throw new ConsumptionError(game.i18n.format(warningMessage, {
  ...
  async _usesConsumption(config, { uses, type, rolls, delta }) {
    ...
    let warningMessage;
    if ( cost > 0 && !uses.value ) warningMessage = "NAHEULBEUK.CONSUMPTION.Warning.None";
    else if ( cost > uses.value ) warningMessage = "NAHEULBEUK.CONSUMPTION.Warning.NotEnough";
  ...
  static consumptionLabelsActivityUses(config, { consumed }={}) {
    ...
    return {
      label: game.i18n.localize(`NAHEULBEUK.CONSUMPTION.Type.ActivityUses.Prompt${increaseKey}`),
      hint: game.i18n.format(
        `NAHEULBEUK.CONSUMPTION.Type.ActivityUses.PromptHint${increaseKey}`,
        {
          cost,
          use: game.i18n.localize(`NAHEULBEUK.CONSUMPTION.Type.Use.${pluralRule}`),
          available: formatNumber(uses.value),
          availableUse: game.i18n.localize(`NAHEULBEUK.CONSUMPTION.Type.Use.${usesPluralRule}`)
        }
      ),
  ...
  static consumptionLabelsAttribute(config, { consumed }={}) {
    ...
    return {
      label: game.i18n.localize(`NAHEULBEUK.CONSUMPTION.Type.Attribute.Prompt${increaseKey}`),
      hint: game.i18n.format(
        `NAHEULBEUK.CONSUMPTION.Type.Attribute.PromptHint${increaseKey}`,
        {
          cost,
  ...
  static consumptionLabelsHitDice(config, { consumed }={}) {
    const { cost, simplifiedCost, increaseKey, pluralRule } = this._resolveHintCost(config);
    let denomination;
    if ( this.target === "smallest" ) denomination = game.i18n.localize("NAHEULBEUK.ConsumeHitDiceSmallest");
    else if ( this.target === "largest" ) denomination = game.i18n.localize("NAHEULBEUK.ConsumeHitDiceLargest");
    ...
    return {
      label: game.i18n.localize(`NAHEULBEUK.CONSUMPTION.Type.HitDice.Prompt${increaseKey}`),
      hint: game.i18n.format(
        `NAHEULBEUK.CONSUMPTION.Type.HitDice.PromptHint${increaseKey}`,
        {
          cost, denomination: denomination.toLowerCase(),
          die: game.i18n.localize(`NAHEULBEUK.CONSUMPTION.Type.HitDie.${pluralRule}`),
  ...
  static consumptionLabelsItemUses(config, { consumed }={}) {
    const { cost, simplifiedCost, increaseKey, pluralRule } = this._resolveHintCost(config);
    const item = this.actor.items.get(this.target);
    const itemName = item ? item.name : game.i18n.localize("NAHEULBEUK.CONSUMPTION.Target.ThisItem").toLowerCase();
    ...
    if ( simplifiedCost > uses.value ) warn = true;
      ...
      type: "warn",
      message: game.i18n.format("NAHEULBEUK.CONSUMPTION.Warning.WillDestroy", { item: itemName })
    });

    return {
      label: game.i18n.localize(`NAHEULBEUK.CONSUMPTION.Type.ItemUses.Prompt${increaseKey}`),
      hint: game.i18n.format(
        `NAHEULBEUK.CONSUMPTION.Type.ItemUses.PromptHint${increaseKey}`,
        {
          cost,
          use: game.i18n.localize(`NAHEULBEUK.CONSUMPTION.Type.Use.${pluralRule}`),
          available: formatNumber(uses.value),
  ...
  static consumptionLabelsMaterial(config, { consumed }={}) {
    ...
    return {
      label: game.i18n.localize(`NAHEULBEUK.CONSUMPTION.Type.Material.Prompt${increaseKey}`),
      hint: game.i18n.format(
        `NAHEULBEUK.CONSUMPTION.Type.Material.PromptHint${increaseKey}`,
        {
          cost,
          item: item ? `<em>${item.name}</em>`
            : game.i18n.localize("NAHEULBEUK.CONSUMPTION.Target.UnknownItem").toLowerCase(),
  ...
  static consumptionLabelsSpellSlots(config, { consumed }={}) {
    const { cost, simplifiedCost, increaseKey, pluralRule } = this._resolveHintCost(config);
    const levelNumber = Math.clamp(this.resolveLevel({ config }), 1, Object.keys(CONFIG.NAHEULBEUK.spellLevels).length - 1);
    const level = CONFIG.NAHEULBEUK.spellLevels[levelNumber].toLowerCase();
    const available = this.actor.system.spells?.[`spell${levelNumber}`]?.value ?? 0;
    return {
      label: game.i18n.localize(`NAHEULBEUK.CONSUMPTION.Type.SpellSlots.Prompt${increaseKey}`),
      hint: game.i18n.format(
        `NAHEULBEUK.CONSUMPTION.Type.SpellSlots.PromptHint${increaseKey}`,
        {
          cost,
          slot: game.i18n.format(`NAHEULBEUK.CONSUMPTION.Type.SpellSlot.${pluralRule}`, { level }),
          available: formatNumber(available)
        }
      ),
      warn: simplifiedCost > available
  ...
  static validAttributeTargets() {
    ...
    return TokenDocument.implementation.getConsumedAttributes(this.actor.type).map(attr => {
      let group;
      if ( attr.startsWith("abilities.") ) group = game.i18n.localize("NAHEULBEUK.AbilityScorePl");
      else if ( attr.startsWith("currency.") ) group = game.i18n.localize("NAHEULBEUK.Currency");
      else if ( attr.startsWith("spells.") ) group = game.i18n.localize("NAHEULBEUK.CONSUMPTION.Type.SpellSlots.Label");
      else if ( attr.startsWith("attributes.movement.") ) group = game.i18n.localize("NAHEULBEUK.Speed");
      else if ( attr.startsWith("attributes.senses.") ) group = game.i18n.localize("NAHEULBEUK.Senses");
      else if ( attr.startsWith("attributes.actions.") ) group = game.i18n.localize("NAHEULBEUK.Vehicle");
      else if ( attr.startsWith("resources.") ) group = game.i18n.localize("NAHEULBEUK.Resources");
  ...
  static validHitDiceTargets() {
    return [
      { value: "smallest", label: game.i18n.localize("NAHEULBEUK.ConsumeHitDiceSmallest") },
      ...CONFIG.NAHEULBEUK.hitDieTypes.map(d => ({ value: d, label: d })),
      { value: "largest", label: game.i18n.localize("NAHEULBEUK.ConsumeHitDiceLargest") }
    ];
  }
  ...
  static validItemUsesTargets() {
    const makeLabel = (name, item) => {
      ...
      if ( uses.max && (uses.recovery?.length === 1) && (uses.recovery[0].type === "recoverAll")
        ...
        const per = CONFIG.NAHEULBEUK.limitedUsePeriods[uses.recovery[0].period]?.abbreviation;
        label = game.i18n.format("NAHEULBEUK.AbilityUseConsumableLabel", { max: uses.max, per });
      }
      else label = game.i18n.format("NAHEULBEUK.AbilityUseChargesLabel", { value: uses.value });
      return `${name} (${label})`;
    };
    return [
      { value: "", label: makeLabel(game.i18n.localize("NAHEULBEUK.CONSUMPTION.Target.ThisItem"), this.item) },
      { rule: true },
      ...(this.actor?.items ?? [])
  ...
  static validSpellSlotsTargets() {
    return Object.entries(CONFIG.NAHEULBEUK.spellLevels).reduce((arr, [value, label]) => {
