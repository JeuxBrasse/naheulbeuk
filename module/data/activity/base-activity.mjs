...
export default class BaseActivityData extends foundry.abstract.DataModel {
  ...
  static LOCALIZATION_PREFIXES = [
    "NAHEULBEUK.ACTIVITY", "NAHEULBEUK.ACTIVATION", "NAHEULBEUK.CONSUMPTION",
    "NAHEULBEUK.DURATION", "NAHEULBEUK.RANGE", "NAHEULBEUK.TARGET", "NAHEULBEUK.USES"
  ];
   ...
   */
  get isRider() {
    return !!this.item.getFlag("naheulbeuk", "riders.activity")?.includes(this.id);
  }
   ...
   */
  get isScaledScroll() {
    return !!this.item.getFlag("naheulbeuk", "spellLevel");
  }
   ...
   * @type {string}
   */
  static INITIAL_ID = staticID("naheulbeukactivity");
  ...
  static transformDamagePartData(source, [formula, type]) {
    ...
    if ( parsed && CONFIG.NAHEULBEUK.dieSteps.includes(Number(parsed[2])) ) {
  ...
  static transformEffectsData(source, options) {
    return source.effects
      .filter(e => !e.transfer && (e.type !== "enchantment") && (e.flags?.naheulbeuk?.type !== "enchantment"))
      .map(e => ({ _id: e._id }));
  }
  ...
  static transformTargetData(source, options) {
    ...
    if ( source.system.target?.type in CONFIG.NAHEULBEUK.areaTargetTypes ) foundry.utils.mergeObject(data, {
      template: {
  ...
  prepareFinalData(rollData) {
    ...
    for ( const target of this.consumption.targets ) {
      ...
      if ( !actor.items.has(target.target) ) {
        const message = game.i18n.format("NAHEULBEUK.CONSUMPTION.Warning.MissingItem", {
    ...
    this.labels.damage = this.labels.damages = rolls.map(roll => {
      ...
      if ( types.length ) {
        label = `${formula} ${game.i18n.getListFormatter({ type: "conjunction" }).format(
          types.map(p => CONFIG.NAHEULBEUK.damageTypes[p]?.label ?? CONFIG.NAHEULBEUK.healingTypes[p]?.label).filter(_ => _)
        )}`;
      }
  ...
  _processDamagePart(damage, rollConfig, rollData, index=0) {
    ...
    const lastType = this.item.getFlag("naheulbeuk", `last.${this.id}.damageType.${index}`);
    ...
    return {
      ...
      options: {
        ...
        properties: Array.from(this.item.system.properties ?? [])
          .filter(p => CONFIG.NAHEULBEUK.itemProperties[p]?.isPhysical)
      }
    };
  }
