...
export default class TargetField extends SchemaField {
  ...
  static prepareData(rollData, labels) {
    ...
    if ( this.target.affects.scalar ) {
      prepareFormulaValue(this, "target.affects.count", "NAHEULBEUK.TARGET.FIELDS.target.affects.count.label", rollData);
    ...
    if ( this.target.template.type ) {
      ...
      prepareFormulaValue(this, "target.template.count", "NAHEULBEUK.TARGET.FIELDS.target.template.count.label", rollData);
      prepareFormulaValue(this, "target.template.size", "NAHEULBEUK.TARGET.FIELDS.target.template.size.label", rollData);
      prepareFormulaValue(this, "target.template.width", "NAHEULBEUK.TARGET.FIELDS.target.template.width.label", rollData);
      prepareFormulaValue(this, "target.template.height", "NAHEULBEUK.TARGET.FIELDS.target.template.height.label", rollData);
    } else {
    ...
    const templateConfig = CONFIG.NAHEULBEUK.areaTargetTypes[this.target.template.type];
    if ( templateConfig ) {
      const parts = [];
      if ( this.target.template.count > 1 ) parts.push(`${this.target.template.count} ×`);
      if ( this.target.template.units in CONFIG.NAHEULBEUK.movementUnits ) {
    ...
    const affectsConfig = CONFIG.NAHEULBEUK.individualTargetTypes[this.target.affects.type];
      ...
      sheet: affectsConfig?.counted ? game.i18n.format(
        `${affectsConfig.counted}.${this.target.affects.count ? pr.select(this.target.affects.count) : "other"}`, {
          number: this.target.affects.count ? formatNumber(this.target.affects.count)
            : game.i18n.localize(`NAHEULBEUK.TARGET.Count.${this.target.template.type ? "Every" : "Any"}`)
        }
      ).trim().capitalize() : (affectsConfig?.label ?? ""),
      statblock: game.i18n.format(
        `${affectsConfig?.counted ?? "NAHEULBEUK.TARGET.Type.Target.Counted"}.${pr.select(this.target.affects.count || 1)}`,
  ...
  static templateDimensions(type) {
    const sizes = CONFIG.NAHEULBEUK.areaTargetTypes[type]?.sizes;
    const dimensions = { size: "NAHEULBEUK.AreaOfEffect.Size.Label" };
    if ( sizes ) {
      dimensions.width = sizes.includes("width") && (sizes.includes("length") || sizes.includes("radius"));
      dimensions.height = sizes.includes("height");
      if ( sizes.includes("radius") ) dimensions.size = "NAHEULBEUK.AreaOfEffect.Size.Radius";
      else if ( sizes.includes("length") ) dimensions.size = "NAHEULBEUK.AreaOfEffect.Size.Length";
      else if ( sizes.includes("width") ) dimensions.size = "NAHEULBEUK.AreaOfEffect.Size.Width";
      if ( sizes.includes("thickness") ) dimensions.width = "NAHEULBEUK.AreaOfEffect.Size.Thickness";
      else if ( dimensions.width ) dimensions.width = "NAHEULBEUK.AreaOfEffect.Size.Width";
      if ( dimensions.height ) dimensions.height = "NAHEULBEUK.AreaOfEffect.Size.Height";
    }
    return dimensions;
  }
}
