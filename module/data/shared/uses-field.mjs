...
export default class UsesField extends SchemaField {
  ...
  static prepareData(rollData, labels) {
    prepareFormulaValue(this, "uses.max", "NAHEULBEUK.USES.FIELDS.uses.max.label", rollData);
    ...
    for ( const recovery of this.uses.recovery ) {
      if ( recovery.period === "recharge" ) {
        ...
        if ( labels ) labels.recharge ??= `${game.i18n.localize("NAHEULBEUK.Recharge")} [${
          recovery.formula}${parseInt(recovery.formula) < 6 ? "+" : ""}]`;
      } else if ( recovery.period in CONFIG.NAHEULBEUK.limitedUsePeriods ) {
        const config = CONFIG.NAHEULBEUK.limitedUsePeriods[recovery.period];
  ...
  static get rechargeOptions() {
    return Array.fromRange(5, 2).reverse().map(min => ({
      value: min,
      label: game.i18n.format("NAHEULBEUK.USES.Recovery.Recharge.Range", {
  ...
  static getStatblockLabel() {
    ...
    if ( (this.activation?.type === "legendary") || (this.activation?.type === "mythic") ) {
      ...
      return game.i18n.format(`NAHEULBEUK.NPC.ActionCostCounted.${pr.select(this.activation.value)}`, {
    ...
    const type = CONFIG.NAHEULBEUK.limitedUsePeriods[recovery.period]?.type;
    if ( (type === "combat") || (type === "special") ) return "";

    // Recharge X–Y
    if ( recovery.period === "recharge" ) {
      const value = parseInt(recovery.formula);
      return `${game.i18n.localize("NAHEULBEUK.Recharge")} ${value === 6 ? "6" : `${value}–6`}`;
    }

    // Recharge after a Short or Long Rest
    if ( ["lr", "sr"].includes(recovery.period) && (this.uses.max === 1) ) {
      return game.i18n.localize(`NAHEULBEUK.Recharge${recovery.period === "sr" ? "Short" : "Long"}`);
    }

    // X/Day
    const period = CONFIG.NAHEULBEUK.limitedUsePeriods[recovery.period === "sr" ? "sr" : "day"]?.label ?? "";
    if ( !period ) return "";
    ...
    if ( profile.type === "recoverAll" ) updates.spent = 0;
      ...
      try {
        ...
        if ( ["day", "dawn", "dusk"].includes(profile.period)
          && (game.settings.get("naheulbeuk", "restVariant") === "gritty") ) {
          roll.alter(7, 0, { multiplyNumeric: true });
        }
        total = (await roll.evaluate()).total;
      } catch(err) {
        Hooks.onError("UsesField#recoverUses", err, {
          msg: game.i18n.format("NAHEULBEUK.ItemRecoveryFormulaWarning", {
  ...
  static async rollRecharge(config={}, dialog={}, message={}) {
    ...
    messageConfig.data.flavor = game.i18n.format("NAHEULBEUK.ItemRechargeCheck", {
      name: this.name,
      result: game.i18n.localize(`NAHEULBEUK.ItemRecharge${rolls[0].isSuccess ? "Success" : "Failure"}`)
    });
     ...
     * been made.
     * @function naheulbeuk.rollRecharge
     * @memberof hookEvents
     * @param {BasicRoll[]} rolls             The resulting rolls.
     * @param {object} data
     * @param {ItemNaheulbeuk|Activity} data.subject  Item or Activity for which the roll was performed.
     * @param {object} data.updates           Updates to be applied to the subject.
     * @returns {boolean}                     Explicitly return `false` to prevent updates from being performed.
     */
    if ( Hooks.call("naheulbeuk.rollRecharge", rolls, { subject: this, updates }) === false ) return rolls;
    if ( Hooks.call("naheulbeuk.rollRechargeV2", rolls, { subject: this, updates }) === false ) return rolls;
     ...
     * @function naheulbeuk.postRollRecharge
     * @memberof hookEvents
     * @param {BasicRoll[]} rolls     The resulting rolls.
     * @param {object} data
     * @param {ActorNaheulbeuk} data.subject  Item or Activity for which the roll was performed.
     */
    Hooks.callAll("naheulbeuk.postRollRecharge", rolls, { subject: this });
