...
export default class DurationField extends SchemaField {
  ...
  static prepareData(rollData, labels) {
    this.duration.scalar = this.duration.units in CONFIG.NAHEULBEUK.scalarTimePeriods;
    if ( this.duration.scalar ) {
      prepareFormulaValue(this, "duration.value", "NAHEULBEUK.DURATION.FIELDS.duration.value.label", rollData);
    } else this.duration.value = null;

    if ( labels && this.duration.units ) {
      if ( this.duration.value && (this.duration.units in CONFIG.NAHEULBEUK.timeUnits) ) {
        labels.duration = formatTime(this.duration.value, this.duration.units);
      } else labels.duration = CONFIG.NAHEULBEUK.timePeriods[this.duration.units] ?? "";
      labels.concentrationDuration = this.duration.concentration || this.properties?.has("concentration")
        ? game.i18n.format("NAHEULBEUK.ConcentrationDuration", { duration: labels.duration }) : labels.duration;
    }
