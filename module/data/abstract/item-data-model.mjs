...
/**
 * @import { FavoriteDataNaheulbeuk, ItemDataModelMetadata } from "./_types.mjs";
 */
...
export default class ItemDataModel extends SystemDataModel {
   ...
   * @type {string}
   */
  static ITEM_TOOLTIP_TEMPLATE = "systems/naheulbeuk/templates/items/parts/item-tooltip.hbs";
   ...
   * @type {ItemNaheulbeuk|void}
   */
  get advancementRootItem() {
    return this.parent?.actor?.items.get(this.parent.getFlag("naheulbeuk", "advancementRoot")?.split(".")?.[0]);
  }
  ...
  prepareBaseData() {
    if ( this.parent.isEmbedded && this.parent.actor?.items.has(this.parent.id) ) {
      ...
      const sourceId = this.parent._stats.compendiumSource ?? this.parent.flags.naheulbeuk?.sourceId;
   ...
   * @param {ActorNaheulbeuk} actor    Actor onto which the item was dropped.
  ...
  async richTooltip(enrichmentOptions={}) {
    return {
      ...
      classes: ["naheulbeuk", "naheulbeuk-tooltip", "item-tooltip", "themed", "theme-light"]
    };
  }
  ...
  async getCardData({ activity, ...enrichmentOptions }={}) {
    ...
    const context = {
      ...
      controlHints: game.settings.get("naheulbeuk", "controlHints"),
      ...
      description: {
        ...
        concealed: game.user.isGM && game.settings.get("naheulbeuk", "concealItemDescriptions") && !description.chat
      }
    };
  ...
  async getCraftCost({ baseItem="craft" }={}) {
    ...
    if ( !this.properties.has("mgc") || !rarity ) {
      const { mundane } = CONFIG.NAHEULBEUK.crafting;
    ...
    const { magic } = CONFIG.NAHEULBEUK.crafting;
   ...
   * @returns {Promise<FavoriteDataNaheulbeuk>}  Favorite data for this item.
   */
