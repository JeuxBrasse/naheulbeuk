 ...
 */
export default class ItemsNaheulbeuk extends foundry.documents.collections.Items {
  /** @override */
  ...
  async importFromCompendium(pack, id, updateData={}, options={}) {
    ...
    if ( contents ) {
      ...
      const toCreate = await ItemNaheulbeuk.createWithContents(contents, {
        container: created, keepId: options.keepId, transformAll: item => this.fromCompendium(item, fromOptions)
      });
      await ItemNaheulbeuk.createDocuments(toCreate, {fromCompendium: true, keepId: true});
    }
